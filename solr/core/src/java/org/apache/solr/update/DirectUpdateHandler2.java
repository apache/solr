...
public class DirectUpdateHandler2 extends UpdateHandler implements SolrCoreState.IndexWriterCloser {
  ...
  public DirectUpdateHandler2(SolrCore core, UpdateHandler updateHandler) {
    ...
    if (this.ulog != null && updateHandler != null) {
    ...
    IOUtils.closeQuietly(updateHandler); // must cast to DirectUpdateHandler2 to ensure it's close is called
  ...
  public int addDoc(AddUpdateCommand cmd) throws IOException {
    ...
    try {
      ...
      throw new SolrException(SolrException.ErrorCode.BAD_REQUEST, "Exception writing document to the index; possible analysis error, id=" +
  ...
  private int addDoc0(AddUpdateCommand cmd) throws IOException {
    ...
    if ((cmd.getFlags() & UpdateCommand.IGNORE_INDEXWRITER) != 0) {
      // TLOG non-leader replica: write to tlog only, do not touch the IndexWriter.
      // numDocsPending tracks uncommitted IW writes, so must NOT be incremented here.
  ...
  public void deleteByQuery(DeleteUpdateCommand cmd) throws IOException {
    ...
    try {
      ...
      if (delAll && cmd.getVersion() == -Long.MAX_VALUE) {
        ReentrantLock lock = solrCoreState.getUpdateLock();
        lock.lock();
        try {
          deleteAll();
          ulog.deleteAll();
  ...
  public void closeWriter(IndexWriter writer) throws IOException {
    ...
    try {
      ...
      if (TestInjection.injectSkipIndexWriterCommitOnClose(writer) || Boolean.getBoolean("solr.skipCommitOnClose")) {
        ...
        log.warn("Skipping commit for IndexWriter.close() due to TestInjection or system property dueToInjection={}", TestInjection.skipIndexWriterCommitOnClose);
        if (writer != null) {
          writer.rollback();
        }
      ...
      // Lock commitLock = solrCoreState.getCommitLock();
     // commitLock.lock();
      try {
        try {
          if (log.isDebugEnabled()) {
            log.debug("Committing on IndexWriter.close() {}.", (tryToCommit ? "" : " ... SKIPPED (unnecessary)"));
          }
          if (tryToCommit) {
...
//            }
  ...
  private void updateDocOrDocValues(AddUpdateCommand cmd, IndexWriter writer) throws IOException {
    ...
    if (cmd.isInPlaceUpdate()) {
      ...
      writer.updateDocValues(updateTerm, fieldsToUpdate.toArray(new Field[0]));

        ...
        if (sortedDocValues != null) {
          ...
          resultValues[i] = new LongValues() {
            ...
            public long longValue() throws IOException {
              ...
              // NOTE: must use a deterministic, JVM-independent hash here. BytesRef.hashCode() uses
              // Lucene's StringHelper.GOOD_FAST_HASH_SEED which is seeded from System.currentTimeMillis()
              // per-JVM, so different nodes would hash the same term differently. Parallel stream workers
              // run on different nodes and each applies {!hash worker=N}; an inconsistent hash makes the
              // worker partitions non-complementary (a doc matches multiple workers or none), corrupting
              // partitionKeys-based parallelism (e.g. over-reads in parallel topic/priority streams).
              // java.lang.String.hashCode() is specified and stable across JVMs.
              return atDoc ? values.lookupOrd(values.ordValue()).utf8ToString().hashCode() : 0;
            }
          };
          continue;
        }

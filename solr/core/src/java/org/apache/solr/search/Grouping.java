...
public class Grouping {
  ...
  /**
   * Invokes search with the specified filter and collector. If a time limit has been specified, set
   * timeout on the searcher before search.
   */
  private void searchWithTimeLimiter(final Query filterQuery, Collector collector)
    ...
    try {
      ...
      if (cmd.getTimeAllowed() > 0 && searcher.timedOut()) {
  ...
  public abstract class Command<T> {
    ...
    protected void addDocList(NamedList<? super DocList> rsp, GroupDocs<?> groups) {
  ...
  public class CommandQuery extends Command<Object> {
    ...
    public Query query;
    TopDocsCollector<? extends ScoreDoc> topCollector;
    ...
    protected Collector createFirstPassCollector() throws IOException {
      ...
      if (withinGroupSort == null || withinGroupSort.equals(Sort.RELEVANCE)) {
        subCollector =
            topCollector =
                new TopScoreDocCollectorManager(groupDocsToCollect, null, Integer.MAX_VALUE)
                    .newCollector();
      } else {
        topCollector =

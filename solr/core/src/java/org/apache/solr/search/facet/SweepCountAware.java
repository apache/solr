...
interface SweepCountAware {
  ...
  static class SegCountPerSeg implements SegCounter {
    ...
    public void register(CountSlotAcc[] countAccs, LongValues toGlobal, int maxSegOrd) {
      ...
      // domains supplies a <code>null</code>
      // DocIdSetIterator -- matches no docs -- for a given segment). But the relevant contents of
      // `countAccs` are re-initialized
      // per-segment, so we can (and must!) safely set `maxIdx` based on `activeSegCounts.length`
      // (as opposed to `countAccs.length`).

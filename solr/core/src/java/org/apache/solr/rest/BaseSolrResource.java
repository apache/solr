...
public abstract class BaseSolrResource {
  ...
  private volatile SolrQueryRequest solrRequest;
  private volatile SolrQueryResponse solrResponse;
  private volatile QueryResponseWriter responseWriter;
  private volatile String contentType;
  private volatile int updateTimeoutSecs = -1;
  ...
  protected void handleException(Logger log) {
    Throwable exception = solrResponse.getException();
    if (null != exception) {
      ...
      if (null != message && ! message.trim().isEmpty()) {
        solrResponse.getToLog().add("msg", "{" + message.trim() + "}");
      }
    }
  }

 ...
 *
 * Format and behavior based on the exception handling in Solr's v1 requestHandler's.  Also sets metrics if present on
 * the request context.
 */
public class CatchAllExceptionMapper implements ExceptionMapper<Exception> {
    ...
    public Response toResponse(Exception exception) {
        final SolrQueryRequest solrQueryRequest = (SolrQueryRequest) containerRequestContext.getProperty(SOLR_QUERY_REQUEST_KEY);

        // First, handle any exception-related metrics
        final Exception normalizedException = RequestHandlerBase.normalizeReceivedException(solrQueryRequest, exception);
        final RequestHandlerBase.HandlerMetrics metrics = (RequestHandlerBase.HandlerMetrics) containerRequestContext.getProperty(HANDLER_METRICS_KEY);
        if (metrics != null) {
            RequestHandlerBase.processErrorMetricsOnException(normalizedException, metrics);
        }

        // Then, actually convert this to a response
        final SolrJerseyResponse response = (SolrJerseyResponse) containerRequestContext.getProperty(SOLR_JERSEY_RESPONSE_KEY);

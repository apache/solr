...
/**
 * Utility methods for creating and populating a {@link org.glassfish.jersey.server.ContainerRequest} for use with Jersey {@link org.glassfish.jersey.server.ApplicationHandler}s
 */
public class ContainerRequestUtils {
    private ContainerRequestUtils() { /* Private ctor prevents instantiation */ }
     ...
     *
     * Implementation guided by code in 'jersey-container-jetty-http's JettyHttpContainer class.
     */
    public static ContainerRequest createContainerRequest(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, Configuration appConfig) {
        final ContainerResponseWriter responseWriter = new JettyBridgeResponseWriter(httpServletResponse);
        try {
    ...
    private static URI getBaseUri(HttpServletRequest httpServletRequest) {
        try {
            return new URI(httpServletRequest.getScheme(), null, httpServletRequest.getServerName(),
                    httpServletRequest.getServerPort(), getBasePath(httpServletRequest), null, null);
    ...
    private static URI getRequestUri(HttpServletRequest httpServletRequest, URI baseUri) throws URISyntaxException {
        ...
        // Jersey is only used for v2 APIs so we have no need of the janky v2 suffixing (and it impedes matching) - remove if present.

...
public class SplitCoreAPI extends CoreAdminAPIBase implements SplitCoreApi {
  ...
  public SplitCoreResponse splitCore(String coreName, SplitCoreRequestBody requestBody)
      throws Exception {
    ensureRequiredParameterProvided("coreName", coreName);
    final SplitCoreRequestBody body =
        requestBody != null ? requestBody : new SplitCoreRequestBody();
    final SplitCoreResponse response = instantiateJerseyResponse(SplitCoreResponse.class);

    // splitByPrefix uses this internally to calculate document ranges for the sub-shards, via a
    // synchronous sub-request that piggybacks an internal correlation id on the 'async' param.
    // That id must not be treated as a real async task id, so this bypasses
    // handlePotentiallyAsynchronousTask entirely, matching the original SplitOp behavior.
    if (Boolean.TRUE.equals(body.getRanges)) {
      handleGetRanges(coreName, response);
      return response;
    }
  ...
  /**
   * Splits the core's index, either into the directory paths in {@code body.path} or by merging
   * pieces into the cores named in {@code body.targetCore}.
   */
  private void doSplit(String coreName, SplitCoreRequestBody body) throws Exception {
    String splitKey = body.splitKey;
    String[] newCoreNames = body.targetCore != null ? body.targetCore.toArray(new String[0]) : null;

    List<DocRouter.Range> ranges = null;

    String[] pathsArr = body.path != null ? body.path.toArray(new String[0]) : null;
    String rangesStr = body.ranges; // ranges=a-b,c-d,e-f
    if (rangesStr != null) {
    ...
    // ensure either path or targetCore specified
    ...
    log.info("Invoked split action for core: {}", coreName);
    String methodStr =
        body.splitMethod != null
            ? body.splitMethod
            : SolrIndexSplitter.SplitMethod.REWRITE.toLower();
    SolrIndexSplitter.SplitMethod splitMethod = SolrIndexSplitter.SplitMethod.get(methodStr);
    if (splitMethod == null) {
      throw new SolrException(
          SolrException.ErrorCode.BAD_REQUEST, "Unsupported value of 'splitMethod': " + methodStr);
    }
    SolrCore parentCore = coreContainer.getCore(coreName);
    List<SolrCore> newCores = null;
    SolrQueryRequest splitReq = null;
    ...
    try {
      ...
      if (coreContainer.isZooKeeperAware()) {
        log.trace("SplitCoreAPI: Determine which router is associated with the shard for core");
      ...
      if (pathsArr == null) {
        log.trace("SplitCoreAPI: Create array of paths for sub-shards of core");
      ...
      splitReq = new SolrQueryRequestBase(parentCore, req.getParams());
      ...
      if (coreContainer.isZooKeeperAware()) {
        log.trace("SplitCoreAPI: Create cloud descriptors for sub-shards of core");
      ...
      if (splitReq != null) splitReq.close();
   ...
   */
  private void handleGetRanges(String coreName, SplitCoreResponse response) throws Exception {
    SolrCore parentCore = coreContainer.getCore(coreName);
    if (parentCore == null) {

...
public class CombinedQueryComponent extends QueryComponent implements SolrCoreAware {
  ...
  public void inform(SolrCore core) {
    for (Map.Entry<String, ?> initEntry : initParams) {
      if ("combiners".equals(initEntry.getKey()) && initEntry.getValue() instanceof NamedList<?>) {
  ...
  public void prepare(ResponseBuilder rb) throws IOException {
    if (rb instanceof CombinedQueryResponseBuilder) { // see CombinedQuerySearchHandler
  ...
  private static Set<Object> extractIdsFromCombinedResponse(
    ...
    if (response instanceof SolrDocumentList) {
      var docList = (SolrDocumentList) response;

...
public class ZkDistributedCollectionLockFactory extends ZkDistributedLockFactory implements DistributedCollectionLockFactory {
  ...
  private String getLockPath(CollectionParams.LockLevel level, String collName, String shardId, String replicaName) {
    ...
    if (level == CollectionParams.LockLevel.COLLECTION) {
      ...
      return path.append(SUBNODE_PREFIX).append(shardId).append(DistributedCollectionConfigSetCommandRunner.ZK_PATH_SEPARATOR).append(LOCK_NODENAME).toString();
    } else if (level == CollectionParams.LockLevel.REPLICA) {
      return path.append(SUBNODE_PREFIX).append(shardId).append(DistributedCollectionConfigSetCommandRunner.ZK_PATH_SEPARATOR).append(SUBNODE_PREFIX).append(replicaName).toString();
    } else {

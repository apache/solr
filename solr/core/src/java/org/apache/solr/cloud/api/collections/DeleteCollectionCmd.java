...
public class DeleteCollectionCmd implements OverseerCollectionMessageHandler.Cmd {
  ...
  public AddReplicaCmd.Response call(ClusterState clusterState, ZkNodeProps message, @SuppressWarnings({"rawtypes"}) NamedList results) throws Exception {
    ...
    try {
      ...
      clusterState = ClusterStateMutator.deleteCollection(clusterState, collection);
  ...
  private static List<String> checkAliasReference(ZkStateReader zkStateReader, String extCollection, boolean followAliases) throws Exception {
    Aliases aliases = zkStateReader.getAliases();
  ...
  private static void checkNotColocatedWith(ZkStateReader zkStateReader, String collection) throws Exception {

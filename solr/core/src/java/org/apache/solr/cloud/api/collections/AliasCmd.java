...
abstract class AliasCmd implements CollApiCmds.CollectionApiCommand {
  ...
  static NamedList<Object> createCollectionAndWait(
    ...
    try {
      ...
      new CreateCollectionCmd(ccc).call(adminCmdContext.subRequestContext(CollectionParams.CollectionAction.CREATE), createMessage, results);
    ...
    CollectionsHandler.waitForActiveCollection(
        createCollName, ccc.getCoreContainer(), new OverseerSolrResponse(results));
    CollectionProperties collectionProperties =
        new CollectionProperties(ccc.getZkStateReader().getZkClient());
    collectionProperties.setCollectionProperty(
        createCollName, ROUTED_ALIAS_NAME_CORE_PROP, aliasName);
    while (!ccc.getZkStateReader()

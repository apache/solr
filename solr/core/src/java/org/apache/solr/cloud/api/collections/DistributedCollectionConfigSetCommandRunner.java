...
public class DistributedCollectionConfigSetCommandRunner {
  ...
  /**
   * Best effort wait for termination of all tasks for a (short) while, then return. This method is called when the JVM
   * shuts down so tasks that did not complete are expected to be stopped mid processing.
   */
  ...
  private class CollectionCommandRunner implements Callable<OverseerSolrResponse> {
    ...
    /**
     * This method does always free the Collection API lock it grabs and updates the tracking of the {@code asyncId}

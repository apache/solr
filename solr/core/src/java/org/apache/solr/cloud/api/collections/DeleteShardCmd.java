...
public class DeleteShardCmd implements OverseerCollectionMessageHandler.Cmd {
  ...
  public AddReplicaCmd.Response call(ClusterState clusterState, ZkNodeProps message, @SuppressWarnings({"rawtypes"})NamedList results) throws Exception {
    ...
    if (state == Slice.State.RECOVERY)  {
      ...
      propMap.put("id", docCollection.getId());
    ...
    try {
      ...
      for (ZkNodeProps r : replicas) {
        ...
        try {
          ...
          AddReplicaCmd.Response resp = ((DeleteReplicaCmd) ocmh.commandMap.get(DELETEREPLICA)).deleteReplica(
      ...
      clusterState = CollectionMutator.deleteShard(clusterState, m);
      ...
      log.info("Successfully deleted shard: {} , shard: {}", collectionName, sliceId);
  ...
  private static List<ZkNodeProps> getReplicasForSlice(String collectionName, Slice slice) {

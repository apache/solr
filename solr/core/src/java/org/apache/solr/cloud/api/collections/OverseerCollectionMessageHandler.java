...
public class OverseerCollectionMessageHandler implements OverseerMessageHandler, SolrCloseable {
  ...
  public static final Map<String, Object> COLLECTION_PROPS_AND_DEFAULTS = Collections.unmodifiableMap(makeMap(
          ROUTER, CompositeIdRouter.DEFAULT_NAME,
...
//      new SolrNamedThreadFactory("OverseerCollectionMessageHandlerThreadFactory"));
  ...
  public OverseerCollectionMessageHandler(CoreContainer cc, Integer myId, LBHttp2SolrClient overseerLbClient, String adminPath, Stats stats, Overseer overseer) {
    ...
    commandMap = new ImmutableMap.Builder<CollectionAction, Cmd>()
            ...
            .put(MOCK_COLL_TASK, OverseerCollectionMessageHandler::mockOperation)
            .put(MOCK_SHARD_TASK, OverseerCollectionMessageHandler::mockOperation)
            .put(MOCK_REPLICA_TASK, OverseerCollectionMessageHandler::mockOperation)
  ...
  public OverseerSolrResponse processMessage(ZkNodeProps message, String operation, ZkStateWriter zkWriter) throws InterruptedException {
    ...
    try {
      ...
      clusterState = zkWriter.getClusterstate(collection);
...
//      }
      ...
      log.debug("action={}, command={}", action, command);

      if (command != null) {
        AddReplicaCmd.Response response = command.call(clusterState, message, results);
        if (response == null) {
          throw new SolrException(ErrorCode.SERVER_ERROR, "CMD did not return a response:" + operation);
        }

        if (log.isDebugEnabled()) log.debug("Command returned clusterstate={} results={}", response.clusterState, results);

        AddReplicaCmd.Response asyncResp = null;
        boolean needToWrite = false;
        if (response.clusterState != null) {
          DocCollection docColl = response.clusterState.getCollectionOrNull(collection);

          if (docColl != null) {

            zkWriter.enqueueStructureChange(docColl);
            needToWrite = true;

          }
          if (response.asyncFinalRunner != null) {
            asyncResp = response.asyncFinalRunner.call();
          }
        }
        ...
        if (asyncResp != null) {
          ...
          if (asyncResp.clusterState != null) {
            ...
            if (docColl != null) {
              zkWriter.enqueueStructureChange(docColl);
              Future writeFut2 = overseer.writePendingUpdates(collection);
              if (writeFut2 != null) {
                try {
                  writeFut2.get();
                  ...
                  log.error("Exception waiting for ZK state write to complete for collection {}", collection, e);
                }
              }
              needToWrite = false;
            }
        ...
        if (needToWrite) {
          Future writeFut = overseer.writePendingUpdates(collection);
          if (writeFut != null) {
            try {
              writeFut.get();
            } catch (InterruptedException ie) {
...
//          }
   //     }
      ...
      log.error("processMessage: operation={} collection={} caused exception", operation, collName, e);
  ...
  @SuppressWarnings({"unchecked"})
  private static AddReplicaCmd.Response mockOperation(ClusterState state, ZkNodeProps message, @SuppressWarnings({"rawtypes"}) NamedList results) throws InterruptedException {
  ...
  private static CollectionAction getCollectionAction(String operation) {
  ...
  private AddReplicaCmd.Response processReplicaAddPropertyCommand(ClusterState clusterState, ZkNodeProps message, @SuppressWarnings({"rawtypes"})NamedList results)
    ...
    // The property change is applied via the state update above; return an empty response (no structure
    // change to write) so the dispatcher in processMessage doesn't treat a null as "CMD did not return a response".
  ...
  private AddReplicaCmd.Response balanceProperty(ClusterState clusterState, ZkNodeProps message, @SuppressWarnings({"rawtypes"})NamedList results) throws Exception {
    ...
    Map<String, Object> m = new HashMap<>(message.size() + 1);
  ...
  AddReplicaCmd.Response deleteReplica(ClusterState clusterState, ZkNodeProps message, @SuppressWarnings({"rawtypes"})NamedList results)
          throws Exception {
    return commandMap.get(DELETEREPLICA).call(clusterState, message, results);
  }
  ...
  static DocRouter.Range intersect(DocRouter.Range a, DocRouter.Range b) {
  ...
  private AddReplicaCmd.Response modifyCollection(ClusterState clusterState, ZkNodeProps message, @SuppressWarnings({"rawtypes"})NamedList results)
    ...
    clusterState = CollectionMutator.modifyCollection(clusterState, message);

    // if switching to/from read-only mode reload the collection
    if (message.keySet().contains(ZkStateReader.READ_ONLY)) {
      // The reloaded cores read READ_ONLY from their watched cluster state during reload
      // (CoreContainer.reload). The modified collection props are written to ZK asynchronously
      // by the dispatcher AFTER this method returns, so we must persist the structure change and
      ...
      if (modified != null) {
        ...
        Future<?> writeFut = overseer.writePendingUpdates(collectionName);
        if (writeFut != null) {
          try {
            ...
            log.error("Exception waiting for READ_ONLY state write before reload for collection {}", collectionName, e);
          }
        }
      }
  ...
  private static NamedList<Object> waitForCoreAdminAsyncCallToComplete(String nodeName, String requestId, String adminPath, ZkStateReader zkStateReader, HttpShardHandlerFactory shardHandlerFactory,
    ...
    // The requestId here is a *core admin* async id (tracked in the target node's in-memory CoreAdminHandler
    // request-status map), NOT an overseer collection-API async id. It is therefore never present in the
    // overseer running/completed/failure maps, so we must poll the node directly via a CoreAdmin REQUESTSTATUS
    // request until the core admin task reports completed or failed. (The previous implementation only consulted
    // the overseer maps / overseer collection-map zk nodes, which never contain this id, so it immediately
    // returned NOT_FOUND and a failed core creation was silently treated as success — masking failed async
    // collection creates as COMPLETED.)
    ...
    int counter = 0;
    // poll up to ~45s (matches the overseer-side wait budget previously used here)
    while (counter < 15 && !overseer.isClosed()) {
      ModifiableSolrParams params = new ModifiableSolrParams();
      ...
      ShardResponse srGather = shardHandler.takeCompletedOrError();
      String coreAdminState = null;
      if (srGather != null) {
        if (srGather.getException() != null) {
          log.warn("Exception getting core admin async status for {} from {}", requestId, replica, srGather.getException());
        } else if (srGather.getSolrResponse() != null && srGather.getSolrResponse().getResponse() != null) {
          Object stObj = srGather.getSolrResponse().getResponse().get("STATUS");
          if (stObj != null) {
            coreAdminState = stObj.toString();
          }
        }
      }
      // drain anything else
      while (srGather != null) {
        srGather = shardHandler.takeCompletedOrError();
      }

      if (coreAdminState != null) {
        if (COMPLETED.toString().equalsIgnoreCase(coreAdminState)) {
          ...
          if (log.isDebugEnabled()) log.debug("The core admin task is COMPLETED, returning");
          return srsp.getValues();
        } else if (FAILED.toString().equalsIgnoreCase(coreAdminState)) {
          NamedList<String> status = new NamedList<>();
          status.add(STATE, FAILED.toString());
          status.add("msg", "found [" + requestId + "] in failed tasks");
          results.add(STATUS, status);
          if (log.isDebugEnabled()) log.debug("The core admin task is FAILED, returning");
          return srsp.getValues();
        }
        // RUNNING or notfound -> keep polling (notfound can be a transient pre-registration state)
      }
    ...
    // We never saw a terminal state; surface this as NOT_FOUND so the caller does not falsely treat it as success.
    NamedList<String> status = new NamedList<>();
    status.add(STATE, NOT_FOUND.toString());
    status.add("msg", "Did not find [" + requestId + "] in any tasks queue on " + replica);
    results.add(STATUS, status);
    return srsp.getValues();
  }
  ...
  public static class ShardRequestTracker{
    ...
    private final Map<String,String> shardAsyncIdByNode = new NonBlockingHashMap<>();
    ...
    public List<Replica> sliceCmd(ModifiableSolrParams params, Replica.State stateMatcher,
      ...
      log.info("replicas for cmd {}", slice.getReplicas());
      for (Replica replica : slice.getReplicas()) {
        // Use replica.getState() (the StateUpdates-aware accessor backed by the live state channel),
        // NOT getStr(STATE_PROP): the fork strips STATE_PROP from Replica, so the latter is null and
        // Replica.State.getState(null) returns null — which never equals the stateMatcher (e.g. ACTIVE),
        // silently dropping every replica so collectionCmd commands like RELOAD are never sent.
        if ((stateMatcher == null || replica.getState() == stateMatcher)) {
          if (zkStateReader.isNodeLive(replica.getNodeName())) {
            ...
            log.info("Send cmd for replica {}", replica.getName());
            sendShardRequest(replica.getNodeName(), cloneParams, shardHandler);
          } else {
            log.info("Replica is not live {}", replica);
  ...
  private static class WatchForResponseNode implements Watcher, Closeable {
    ...
    public void close() throws IOException {
      this.closed = true;
      try {
        zkClient.removeWatches(watchPath, this, Watcher.WatcherType.Any, true);

...
public class ClusterFileStore extends JerseyResource implements ClusterFileStoreApis {
  ...
  public SolrJerseyResponse getFile(String path, Boolean sync, String getFrom, Boolean meta) {
    ...
    // TODO NOCOMMIT Will this serialize weirdly because Jersey doesn't know how to handle the
    // 'FileDetails' objects in the list returned by FileStore.list()?  Or are they OK because
    // they're MapWriter implementations?
    if (typ == FileStore.FileType.DIRECTORY) {
      ...
      directoryListingResponse.files = Collections.singletonMap(path, fileStore.list(path, null));
    ...
    if (Boolean.TRUE.equals(meta)) {
      if (typ == FileStore.FileType.FILE) {
        ...
        final var fileMetaResponse =
            instantiateJerseyResponse(FileStoreDirectoryListingResponse.class);
        fileMetaResponse.files = Collections.singletonMap(path, l.isEmpty() ? null : l.get(0));

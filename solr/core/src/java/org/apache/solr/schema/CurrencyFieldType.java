...
public class CurrencyFieldType extends FieldType implements SchemaAware, ResourceLoaderAware {
  ...
  public void inform(IndexSchema schema) {
    ...
    if (null == fieldTypeAmountRaw) {
      ...
      if (field == null) {
        log.error("Could not find dynamic field {} in {}",  PARAM_FIELD_SUFFIX_AMOUNT_RAW + "=\"" + fieldSuffixAmountRaw + "\"", Arrays.asList(schema.getDynamicFields()));
  ...
  static class RawCurrencyValueSource extends ValueSource {
    ...
    private static class MyFunctionValues extends FunctionValues {
      ...
      public long longVal(int doc) throws IOException {
        ...
        if (!initializedCache) {
          Arrays.fill(fractionDigitCache, -1);

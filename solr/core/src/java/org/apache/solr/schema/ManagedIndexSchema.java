...
public final class ManagedIndexSchema extends IndexSchema {
   ...
   */
  ManagedIndexSchema(ManagedIndexSchemaFactory managedIndexSchemaFactory, String collection, SolrConfig solrConfig, String name, InputSource is,
      boolean isMutable, String managedSchemaResourceName, int schemaZkVersion, ReentrantLock schemaUpdateLock) {
    super(name, is, solrConfig.luceneMatchVersion, solrConfig.getResourceLoader(), solrConfig.getSubstituteProperties(), schemaUpdateLock);
    ...
    StopWatch timeInform = StopWatch.getStopWatch(name + "-informAfterSchemaRead");
  ...
  private ManagedIndexSchema(ManagedIndexSchemaFactory managedIndexSchemaFactory, String collection, Version luceneVersion, SolrResourceLoader loader, boolean isMutable,
      String managedSchemaResourceName, int schemaZkVersion, Properties substitutableProps, ReentrantLock schemaUpdateLock) {
    super(luceneVersion, loader, substitutableProps, schemaUpdateLock);
  ...
  public boolean persistManagedSchema(boolean createOnly) {
    ...
    try {
      ...
      try {
        ...
        final OutputStream out = Files.newOutputStream(managedSchemaFile.toPath());
  ...
  public static void waitForSchemaZkVersionAgreement(String collection, String localCoreNodeName, int schemaZkVersion, ZkController zkController, int maxWaitSecs, ConnectionManager.IsClosed isClosed) {
    ...
    for (GetZkSchemaVersionCallable call : concurrentTasks) {
      results.add(ParWork.submit("waitForSchemaZkVersionAgreement", call));
    }
  ...
  protected static List<String> getActiveReplicaCoreUrls(ZkController zkController, String collection, String localCoreNodeName) {
    ...
    private final int expectedZkVersion;
    ...
    private static boolean stoppingException(Throwable e) {
      if (e == null) {
        return false;
      }
      return e instanceof KeeperException.SessionExpiredException || e instanceof RejectedExecutionException || e instanceof InterruptedException
  ...
  public ManagedIndexSchema deleteFields(Collection<String> names) {
    ManagedIndexSchema newSchema;
    // Balance the unlock() in the finally below: deleteFields was missing the matching lock(), so a
    // SUCCESSFUL delete reached finally and called unlock() on a lock this thread never held, throwing
    // IllegalMonitorStateException (surfaced as a message-less 400). Mirror persistManagedSchema/addFields.
    schemaUpdateLock.lock();
    try {
      if (isMutable) {
        newSchema = shallowCopy(true);
  ...
  public ManagedIndexSchema replaceField
    ...
    if (isMutable) {
      ...
      if (dynamicCopyFieldsToRebuild.size() > 0) {
        newSchema.dynamicCopyFields = newDynamicCopyFields.toArray(IndexSchema.EMPTY_DYNAMIC_COPY_FIELDS);
  ...
  public ManagedIndexSchema addFieldTypes(List<FieldType> fieldTypeList, boolean persist) {
    ...
    newSchema.fieldTypes = (Object2ObjectOpenHashMap) ((Object2ObjectOpenHashMap) newSchema.fieldTypes).clone();
      ...
      if (persist) {
        ...
        if (success) {
          if (log.isDebugEnabled()) {
            StringBuilder fieldTypeNames = new StringBuilder(64);
  ...
  public ManagedIndexSchema replaceFieldType(String typeName, String replacementClassName, Map<String,Object> replacementArgs) {
    ...
    if (isMutable) {
      ...
      newSchema = shallowCopy(true);
      // clone data structures before modifying them
      newSchema.fieldTypes = new Object2ObjectOpenHashMap<>(fieldTypes);
      newSchema.copyFieldsMap = cloneCopyFieldsMap(copyFieldsMap);
      newSchema.copyFieldTargetCounts
          = (Map<SchemaField,Integer>)((Object2ObjectOpenHashMap)copyFieldTargetCounts).clone();
      newSchema.dynamicCopyFields = new DynamicCopy[dynamicCopyFields.length];
      ...
      // Rebuild affected dynamic copy fields
      if (dynamicCopyFieldsToRebuild.size() > 0) {
        newSchema.dynamicCopyFields = newDynamicCopyFields.toArray(EMPTY_DYNAMIC_COPY_FIELDS);
        for (DynamicCopy dynamicCopy : dynamicCopyFieldsToRebuild) {
          newSchema.registerCopyField(dynamicCopy.getRegex(), dynamicCopy.getDestFieldName(), dynamicCopy.getMaxChars());
        }
      }
  ...
  public void postReadInform() {
    postReadInform(false);
  }

  @Override
  public void postReadInform(boolean wait) {
    super.postReadInform(wait);
    if (wait) {
      try (ParWork work = new ParWork(this)) {
        fieldTypes.forEach((s, fieldType) -> work.collect("", () -> informResourceLoaderAwareObjectsForFieldType(fieldType)));
      }
    } else {
      fieldTypes.forEach((s, fieldType) -> ParWork.submitIO("postReadInform", () -> informResourceLoaderAwareObjectsForFieldType(fieldType)));
    }
  }
  ...
  protected void informResourceLoaderAwareObjectsForFieldType(FieldType fieldType) {
    ...
    if (queryAnalyzer != indexAnalyzer && queryAnalyzer instanceof TokenizerChain)
    ...
    if (fieldType instanceof TextField) {
      ...
      if (multiTermAnalyzer != indexAnalyzer && multiTermAnalyzer != queryAnalyzer && multiTermAnalyzer instanceof TokenizerChain)
  ...
  public FieldType newFieldType(String typeName, String className, Map<String, ?> options) {
    ...
    node = FieldTypeXmlAdapter.toNode(loader, options);

    // Include "class" in params so FieldType.getClassArg() returns the user-supplied short name
    // (e.g. "solr.TrieIntField") rather than falling back to getClass().getName() (the full FQN).
    // FieldType.setArgs already removes CLASS_NAME from initArgs before processing, so including
    // it here is safe.
    Map<String, String> params = Collections.unmodifiableMap(DOMUtil.toMapExcept(node.attributes(), NAME));
    String name =  DOMUtil.getAttr(node, NAME); //node, NAME, requireName ? type : null);
  //  String className = DOMUtil.getAttr(node, "class", type);
    FieldType plugin;
    try {
      plugin = typeLoader.create(loader, name, className, node, SolrResourceLoader.getXPath());
    } catch (Exception ex) {
      ParWork.propagateInterrupt(ex);
      throw new SolrException
          (ErrorCode.SERVER_ERROR, "Plugin init failure for " + name, ex);
    }


    typeLoader.loadSingle(name, plugin, params);
   ...
   */
   ManagedIndexSchema shallowCopy(boolean includeFieldDataStructures) {
     ManagedIndexSchema newSchema = new ManagedIndexSchema
         (managedIndexSchemaFactory, collection, luceneVersion, loader, isMutable, managedSchemaResourceName, schemaZkVersion, substitutableProperties, getSchemaUpdateLock());

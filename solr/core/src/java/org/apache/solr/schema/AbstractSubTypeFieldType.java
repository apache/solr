...
/**
 * An abstract base class for FieldTypes that delegate work to another {@link
 * org.apache.solr.schema.FieldType}. The sub type can be obtained by either specifying the
 * subFieldType attribute or the subFieldSuffix. In the former case, a new dynamic field will be
 * injected into the schema automatically with the name of {@link #POLY_FIELD_SEPARATOR}. In the
 * latter case, it will use an existing dynamic field definition to get the type. See the example
 * schema and the use of the {@link org.apache.solr.schema.PointType} for more details.
 */
public abstract class AbstractSubTypeFieldType extends FieldType implements SchemaAware {
  ...
  /**
   * Helper method for creating a dynamic field SchemaField prototype. Returns a {@link SchemaField}
   * with the {@link FieldType} given and a name of "*" + {@link FieldType#POLY_FIELD_SEPARATOR} +
   * {@link FieldType#typeName} and props of indexed=true, stored=false.
   *
   ...
   */
  static SchemaField registerPolyFieldDynamicPrototype(
      IndexSchema schema, FieldType subType, FieldType polyField) {

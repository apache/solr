...
public class DOMUtil {
  ...
  public static Map<String,String> toMapExcept(AttributeMap attrMap, String... exclusions) {
    ...
    outer: for (int j=0; j<attrs.size(); j++) {
      ...
      // Skip xml:* attributes (e.g. xml:base injected by XInclude processing)
      if (attrName.startsWith(XML_RESERVED_PREFIX + ":")) continue;
  ...
  private static String getAttribs(NodeInfo nd) {
    ...
    nd.attributes().forEach(attributeInfo -> {
      sb.append(attributeInfo.getNodeName()).append(":").append(attributeInfo.getValue());
    });
    return sb.toString();
  }
  ...
  public static String getText(NodeInfo nd) {
    ...
    if (type == Type.DOCUMENT) { /* fall through */
      return null;
    }

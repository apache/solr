 ...
 * of a {@link ResourceLoader}. Just plug this class on top of a {@link ResourceLoader} and pass it
 * as {@link EntityResolver} to SAX parsers or via wrapper methods as {@link URIResolver} to XSL
 * transformers or {@link XMLResolver} to STAX parsers. The resolver handles special SystemIds with
 * an URI scheme of {@code solrres:} that point to resources. To produce such systemIds when you
 * initially call the parser, use {@link #createSystemIdFromResourceName} which produces a SystemId
 * that can be included along the InputStream coming from {@link ResourceLoader#openResource}.
 *
 * <p>In general create the {@link InputSource} to be passed to the parser like:
 *
...
public final class SystemIdResolver implements EntityResolver, EntityResolver2 {
  ...
  URI resolveRelativeURI(String baseURI, String systemId) throws URISyntaxException {
    ...
    // special case for backwards compatibility: if relative systemId starts with "/" (we convert
    // that to an absolute solrres:-URI)
    if (systemId.startsWith("/")) {
      uri =
          new URI(RESOURCE_LOADER_URI_SCHEME, RESOURCE_LOADER_AUTHORITY_ABSOLUTE, "/", null, null)
              .resolve(systemId);
    } else {
      // simply parse as URI
    ...
    try {
      ...
      if (RESOURCE_LOADER_URI_SCHEME.equals(uri.getScheme())) {
        ...
        try {
          ...
          // unfortunately XInclude fallback only works with IOException, but openResource() never
          // throws that one
      ...
      throw new IOException(
          "An URI syntax problem occurred during resolving systemId: " + systemId, use);
    }
  }

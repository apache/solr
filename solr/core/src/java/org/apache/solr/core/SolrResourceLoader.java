...
//        }
  ...
  public void inform(SolrCore core) {
    while (true) {
      ...
      if (!(size > 0)) break;
      // Iterate sequentially so that exceptions from inform() propagate to the caller
      // and cause core init to fail (recorded in CoreContainer.coreInitFailures).
      // Using ignoreExceptions=true ParWork swallows them silently.
      for (SolrCoreAware aware : new java.util.ArrayList<>(waitingForCore)) {
        try {
          aware.inform(core);
        } catch (Exception e) {
          ...
          ParWork.propagateInterrupt(e);
          throw new SolrException(SolrException.ErrorCode.SERVER_ERROR,
              "Error informing SolrCore for " + aware.getClass().getName(), e);
        }

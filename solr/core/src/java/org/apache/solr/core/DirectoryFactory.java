...
public abstract class DirectoryFactory implements NamedListInitializedPlugin, Closeable {
  ...
  public void cleanupOldIndexDirectories(
    ...
    try (DirectoryStream<Path> oldIndexDirs =
      ...
      List<Path> dirsList =
          StreamSupport.stream(oldIndexDirs.spliterator(), false)
      ...
      int i = 0;
      if (afterCoreReload) {
        log.info("Will not remove most recent old directory after reload {}", dirsList.get(0));
        i = 1;
      }
  ...
  /**
   * Returns a default Update Log instance. UpdateHandler will invoke this method if the solrconfig
   * / plugin info does not specify any ulog class.
   */

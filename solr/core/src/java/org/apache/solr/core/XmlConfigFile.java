...
public class XmlConfigFile { // formerly simply "Config"
  ...
  public XmlConfigFile(SolrResourceLoader loader, String name, InputSource is, String prefix, Properties substituteProps)
    ...
    try {
      this.loader = loader;
      this.name = name;
      this.prefix = (prefix != null && !(!prefix.isEmpty() && prefix.charAt(prefix.length() - 1) == '/')) ? prefix + '/' : prefix;
     ...
     //   DocumentBuilder db = SolrResourceLoader.solrXMLConfigXpathExpressions.proc.getProcessor().newDocumentBuilder();
        doc = new XdmNode(this.tree);
  ...
  public static TinyDocumentImpl parseXml(SolrResourceLoader loader, InputSource is, Properties substituteProps) throws XPathException, SaxonApiException {
    TinyDocumentImpl docTree;

    SAXSource saxSrc = new SAXSource(is);
    PipelineConfiguration plc = SolrResourceLoader.getConf().makePipelineConfiguration();
    ParseOptions po = plc.getParseOptions();
    if (is.getSystemId() != null) {
      // Supply a fresh, XInclude-aware SAX parser per parse, with this loader's
      // SystemIdResolver bound as the EntityResolver. Saxon's shared static Configuration
      // (SolrResourceLoader.conf) pools its source parsers across the whole JVM, and a pooled
      // Xerces parser retains the EntityResolver wired into its XInclude handler from a previous
      // parse. Without supplying our own reader, a config parse performed earlier by a cloud test
      // through a ZkSolrResourceLoader would leave that (later-closed) Zk-backed SystemIdResolver
      // installed on the pooled parser; a subsequent non-cloud initCore() in the same JVM would
      // then resolve its XIncludes against the already-closed ZK connection (AlreadyClosedException).
      // Providing our own XMLReader makes Saxon use it directly instead of its pool, so the entity
      // resolver can never leak across parses/tests.
      try {
        SAXParserFactory spf = SAXParserFactory.newInstance("org.apache.xerces.jaxp.SAXParserFactoryImpl", null);
        spf.setNamespaceAware(true);
        spf.setXIncludeAware(true);
        spf.setValidating(false);
        XMLReader reader = spf.newSAXParser().getXMLReader();
        reader.setEntityResolver(new SystemIdResolver(loader));
        saxSrc.setXMLReader(reader);
      } catch (ParserConfigurationException | SAXException e) {
        throw new RuntimeException("Unable to create XInclude-aware XML parser for " + is.getSystemId(), e);
      }
      po.setEntityResolver(new SystemIdResolver(loader));
    }
    ...
    NodeInfo saxTreeInfo = SolrResourceLoader.conf.buildDocumentTree(saxSrc, po).getRootNode();
    return (TinyDocumentImpl) saxTreeInfo.getTreeInfo().getRootNode();
...
//    }
  ...
  public Object evaluate(TinyDocumentImpl tree, String path, QName type) {
    try {
      ...
      XPathExpression expression = ConfigXpathExpressions.CACHE.get(xstr);
      if (expression != null) {
        return expression.evaluate(tree, type);
      }

      log.error("COMPILE XPATH FOR PATH OR MANUALLY GRAPH THESE TINY NODES = {} QNAME={}", xstr, type);
...
//      }
      ...
      try {
        ArrayList<NodeInfo> nodes = (ArrayList) expression.evaluate(doc, XPathConstants.NODESET);
    ...
    public List<NodeInfo> getNodeList(String path, boolean errIfMissing){
      ...
      try {
        ...
        XPathExpression expression = ConfigXpathExpressions.CACHE.get(xstr);
        if (expression != null) {
          nodeList = (ArrayList) expression.evaluate(tree, XPathConstants.NODESET);
        } else {
          XPath xPath = SolrResourceLoader.getXPath();
          nodeList = (ArrayList) xPath.evaluate(xstr, tree, XPathConstants.NODESET);
        }
     ...
     */
    public static Set<String> getUnknownAttributes(Element element, String... knownAttributes){
  ...
  private static class ReceiverFilter implements Receiver {
    private final Receiver next;
    private final Properties substituteProps;

    public ReceiverFilter(Receiver next, Properties substituteProps) {
      this.next = next;
      this.substituteProps = substituteProps;
    }

    @Override public void setPipelineConfiguration(PipelineConfiguration pipe) {
      next.setPipelineConfiguration(pipe);
    }

    @Override public PipelineConfiguration getPipelineConfiguration() {
      return next.getPipelineConfiguration();
    }

    @Override public void setSystemId(String systemId) {
      next.setSystemId(systemId);
    }

    @Override public void open() throws XPathException {
      next.open();
    }

    @Override public void startDocument(int properties) throws XPathException {
      next.startDocument(properties);
    }

    @Override public void endDocument() throws XPathException {
      next.endDocument();
    }

    @Override public void setUnparsedEntity(String name, String systemID, String publicID) throws XPathException {
    ...
    public void startElement(NodeName elemName, SchemaType type, AttributeMap attributes, NamespaceMap namespaces, Location location, int properties)
      ...
      attributes = attributes.apply(attributeInfo -> {
        String attValue = attributeInfo.getValue();
        if (attValue.contains("${")) {
          return new AttributeInfo(attributeInfo.getNodeName(), attributeInfo.getType(), PropertiesUtil.substituteProperty(attValue, substituteProps),
              attributeInfo.getLocation(), attributeInfo.getProperties());
        }
    ...
    @Override public void endElement() throws XPathException {
      next.endElement();
    }

    @Override public void characters(CharSequence chars, Location location, int properties) throws XPathException {
      String val = chars.toString();
      if (val.contains("${")) {
        String sub = PropertiesUtil.substituteProperty(val, substituteProps);
        next.characters(sub, location, properties);
        return;
      }
      next.characters(chars, location, properties);
    }

    @Override public void processingInstruction(String name, CharSequence data, Location location, int properties) throws XPathException {
      next.processingInstruction(name, data, location, properties);
    }

    @Override public void comment(CharSequence content, Location location, int properties) throws XPathException {
      next.comment(content, location, properties);
    }

    @Override public void close() throws XPathException {
      next.close();
    }

    @Override public String getSystemId() {
      return next.getSystemId();
    }
  }
}

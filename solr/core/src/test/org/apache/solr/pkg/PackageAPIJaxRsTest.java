...
public class PackageAPIJaxRsTest extends SolrCloudTestCase {
  ...
  public void testAddAndDeletePackageVersion() throws Exception {
    String FILE1 = "/jaxrstestpkg/runtimelibs.jar";
    ...
    try (HttpSolrClient client =
      ...
      PackageApi.AddPackageVersion addRequest = new PackageApi.AddPackageVersion("jaxrstestpkg");
      ...
      assertNotNull(
          "Expected jaxrstestpkg in packages", listResponse.result.packages.get("jaxrstestpkg"));
      assertFalse(
          "Expected at least one version",
          listResponse.result.packages.get("jaxrstestpkg").isEmpty());

      // Verify GET /cluster/package/{name} returns only this package
      PackagesResponse getByNameResponse =
          new PackageApi.GetPackage("jaxrstestpkg").process(client);
      ...
      assertNotNull(
          "Expected jaxrstestpkg in get-by-name response",
          getByNameResponse.result.packages.get("jaxrstestpkg"));

      // Delete the package version via DELETE /cluster/package/{name}/versions/{version}
      new PackageApi.DeletePackageVersion("jaxrstestpkg", "1.0").process(client);
      ...
      List<?> versionsAfterDelete = listAfterDelete.result.packages.get("jaxrstestpkg");
      assertTrue(
  ...
  public void testRefreshNonExistentPackage() throws Exception {
    try (HttpSolrClient client =
      ...
      // Note: JacksonDataBindResponseParser doesn't expose errors as RemoteSolrException;
      // instead, the error is in the response body's 'error' field.
      var response = new PackageApi.RefreshPackage("nonexistentpkg_jaxrs").process(client);

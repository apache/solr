...
public class CollectionsAPIDistClusterPerZkTest extends SolrCloudTestCase {
  ...
  @Test
  // Fork adaptations: (1) AddReplicaCmd.assignReplicaDetails now reads the requested core name from
  // CoreAdminParams.NAME (SolrJ setCoreName()) with CORE fallback, so the blocker cores can collide
  // by name. (2) CoreContainer.create() now synchronously rejects a duplicate core name, so the
  // CoreAdmin CREATE fails, CreateCollectionCmd records the failure, and the client sees the error.
  public void testCreateShouldFailOnExistingCore() throws Exception {
    // Adapted to the fork: shards are auto-named "s1" (not "shard1") and replica cores are
    // auto-named "<collection>_<shard>_r_n<cnt>" (not "<collection>_shardN_replicaN").
    // To trigger the "existing core" conflict, the blocker cores must be named exactly what
    // the "halfcollection" create would auto-assign for its s1 replica: "halfcollection_s1_r_n1".
    // (The setCoreName CoreAdminParams.NAME-vs-CORE bug in AddReplicaCmd has been fixed so the
    // requested core name is now honored.)
    assertEquals(0, CollectionAdminRequest.createCollection("halfcollectionblocker", "conf", 1, 1)
        .setCreateNodeSet("")
        .process(cluster.getSolrClient()).getStatus());
    assertTrue(CollectionAdminRequest.addReplicaToShard("halfcollectionblocker", "s1")
        .setNode(cluster.getJettySolrRunner(0).getNodeName())
        .setCoreName("halfcollection_s1_r_n1")
    ...
    assertEquals(0, CollectionAdminRequest.createCollection("halfcollectionblocker2", "conf",1, 1)
        .setCreateNodeSet("")
        .process(cluster.getSolrClient()).getStatus());
    assertTrue(CollectionAdminRequest.addReplicaToShard("halfcollectionblocker2", "s1")

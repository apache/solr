...
public class HttpPartitionTest extends AbstractFullDistribZkTestBase {
  ...
  protected void testLeaderZkSessionLoss() throws Exception {
    ...
    while (System.nanoTime() < timeout) {
      ...
      try {
          Replica currentLeader = ZkStateReader.from(cloudClient).getLeaderRetry(testCollectionName, "shard1");
  ...
  protected void assertDocsExistInAllReplicas(List<Replica> notLeaders,
      ...
      throws Exception {
      Replica leader = ZkStateReader.from(cloudClient).getLeaderRetry(testCollectionName, "shard1", 10000);

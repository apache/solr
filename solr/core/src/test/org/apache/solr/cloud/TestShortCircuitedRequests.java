...
public class TestShortCircuitedRequests extends AbstractFullDistribZkTestBase {
  ...
  public void test() throws Exception {
    ...
    assertEquals(
        4,
        cloudClient.getClusterStateProvider().getCollection(DEFAULT_TEST_COLLECTION_NAME).getSlices().size());
    ...
    Replica shard3 =
        cloudClient.getClusterState().getCollection(DEFAULT_TEST_COLLECTION_NAME).getLeader("shard3");
    String nodeName = shard3.getNodeName();
    SolrClient shard3Client = getClient(nodeName);
    QueryResponse response =
        shard3Client.query(
            new SolrQuery("*:*")
                .add(ShardParams._ROUTE_, "a!")
                .add(ShardParams.SHARDS_INFO, "true"));

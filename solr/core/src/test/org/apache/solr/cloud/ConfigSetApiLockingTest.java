...
public class ConfigSetApiLockingTest extends SolrTestCaseJ4 {
  ...
  private void multithreadedTests(ConfigSetApiLockFactory apiLockingHelper) throws Exception {
    ...
    // (thread start could have been checked more reliably using another latch, and verifying the thread is in waitUntilAcquired done through that thread stacktrace, but that would be overkill compared to the very slight race condition of waiting 30ms, but a race that would not cause the test to fail since we're testing... that nothing happened yet).
    Thread.sleep(30);

...
public class PeerSyncReplicationTest extends AbstractFullDistribZkTestBase {
  ...
  public void test() throws Exception {
    ...
    assertTrue(
        "REPLICATION.peerSync.errors present", metrics.containsKey("REPLICATION.peerSync.errors"));
  ...
  private CloudJettyRunner forceNodeFailureAndDoPeerSync() throws Exception {
    ...
    CloudJettyRunner replicaToShutDown =
        nonLeaderJetties.get(random().nextInt(nonLeaderJetties.size())); // random non leader node
  ...
  private void bringUpDeadNodeAndEnsureNoReplication(CloudJettyRunner nodeToBringUp)
    ...
    String replicationProperties =
        nodeToBringUp.jetty.getSolrHome()
            + "/cores/"

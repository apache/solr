...
public class CollectionsAPISolrJTest extends SolrCloudTestCase {
  ...
  public void testCreateCollWithDefaultClusterPropertiesOldFormat() throws Exception {
    ...
    try {
      ...
      for (int i = 0; i < 300; i++) {
        Map<?, ?> m = ZkStateReader.from(cluster.getSolrClient()).getClusterProperty(COLLECTION_DEF, null);
        if (m != null) break;
        Thread.sleep(10);
      }
      Object clusterProperty = ZkStateReader.from(cluster.getSolrClient()).getClusterProperty(ImmutableList.of(DEFAULTS, COLLECTION, NUM_SHARDS_PROP), null);
      assertEquals("2", String.valueOf(clusterProperty));
      clusterProperty = ZkStateReader.from(cluster.getSolrClient()).getClusterProperty(ImmutableList.of(DEFAULTS, COLLECTION, NRT_REPLICAS), null);
      assertEquals("2", String.valueOf(clusterProperty));
      ...
      clusterProperty = ZkStateReader.from(cluster.getSolrClient()).getClusterProperty(ImmutableList.of(COLLECTION_DEF, NRT_REPLICAS), null);
      assertNull(clusterProperty);
      ...
      while (!timeOut.hasTimedOut()) {
        clusterProperty = ZkStateReader.from(cluster.getSolrClient()).getClusterProperty(ImmutableList.of(DEFAULTS, COLLECTION, NUM_SHARDS_PROP), null);
        if (clusterProperty == null)  break;
      }
      assertNull(clusterProperty);
      clusterProperty = ZkStateReader.from(cluster.getSolrClient()).getClusterProperty(ImmutableList.of(COLLECTION_DEF, NUM_SHARDS_PROP), null);
      assertNull(clusterProperty);
    } finally {
  ...
  public void testCloudInfoInCoreStatus() throws IOException, SolrServerException {
    ...
    try (HttpSolrClient coreclient = getHttpSolrClient(ZkStateReader.from(cluster.getSolrClient()).getBaseUrlForNodeName(nodeName))) {
  ...
  public void testAddAndDeleteReplica() throws Exception {
    ...
    ArrayList<String> nodeList = new ArrayList<>(ZkStateReader.from(cluster.getSolrClient()).getClusterState().getLiveNodes());
  ...
  private void checkCollectionProperty(String collection, String propertyName, String propertyValue, long timeoutMs) throws InterruptedException {
    ...
    while (!timeout.hasTimedOut()){
      Thread.sleep(10);
      if (Objects.equals(ZkStateReader.from(cluster.getSolrClient()).getCollectionProperties(collection).get(propertyName), propertyValue)) {
        return;
      }
    }
  ...
  public void testReadOnlyCollection() throws Exception {
    ...
    Replica leader = ZkStateReader.from(solrClient).getLeaderRetry(collectionName, "shard1", DEFAULT_TIMEOUT);
    ...
    CollectionAdminRequest.modifyCollection(collectionName,
        ...
        .process(cluster.getSolrClient());
    coll = ZkStateReader.from(cluster.getSolrClient()).getClusterState().getCollection(collectionName);
  ...
  public void testRenameCollection() throws Exception {
    ...
    ZkStateReader.from(cluster.getSolrClient()).aliasesManager.update();
  ...
  public void testDeleteAliasedCollection() throws Exception {
    ...
    RetryUtil.retryUntil("didn't get the new aliases", 10, 1000, TimeUnit.MILLISECONDS, () -> {
      try {
        ...
        return ZkStateReader.from(solrClient).getAliases()

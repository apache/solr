...
public class CollectionsAPISolrJTest extends SolrCloudTestCase {
  ...
  public void testCreateCollWithDefaultClusterPropertiesOldFormat() throws Exception {
    ...
    try {
      ...
      for (int i = 0; i < 300; i++) {
        Map<?, ?> m =
            ZkStateReader.from(cluster.getSolrClient()).getClusterProperty(COLLECTION_DEF, null);
        if (m != null) break;
        Thread.sleep(10);
      }
  ...
  public void testCloudInfoInCoreStatus() throws IOException, SolrServerException {
    ...
    try (HttpSolrClient coreclient =
        getHttpSolrClient(
            ZkStateReader.from(cluster.getSolrClient()).getBaseUrlForNodeName(nodeName))) {
  ...
  public void testCreateAndDeleteShard() throws Exception {
    ...
    ZkStateReader.from(cluster
        .getSolrClient()
        ).waitForState(collectionName, 30, TimeUnit.SECONDS, (l, c) -> c != null && c.getSlice("shardC") != null);

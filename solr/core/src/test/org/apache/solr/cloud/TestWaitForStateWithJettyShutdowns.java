...
public class TestWaitForStateWithJettyShutdowns extends SolrTestCaseJ4 {
  ...
  public void testWaitForStateBeforeShutDown() throws Exception {
    ...
    try {
          ...
          executor.submit(
              () -> {
                try {
                  ZkStateReader.from(cluster
                      .getSolrClient()
                      ).waitForState(col_name, 180, TimeUnit.SECONDS, new LatchCountingPredicateWrapper(latch,
                      clusterShape(1, 0)));
          } catch (Exception e) {
      ...
      if (!latch.await(120, TimeUnit.SECONDS)) {
        fail(
            "timed out Waiting a ridiculous amount of time for the waitForState latch -- did impl change?");
      }
  ...
  public final class LatchCountingPredicateWrapper implements CollectionStatePredicate {
    ...
    public LatchCountingPredicateWrapper(
        final CountDownLatch latch, final CollectionStatePredicate inner) {
      this.latch = latch;
      this.inner = inner;
    }
    ...
    public boolean matches(Set<String> liveNodes, DocCollection collectionState) {
      ...
      if (log.isInfoEnabled()) {
        log.info(
            "Predicate called: result={}, (pre)latch={}, liveNodes={}, state={}",
            result,
            latch.getCount(),
            liveNodes,
            collectionState);

...
public class NodeMutatorTest extends SolrTestCaseJ4 {
  ...
  public void downNodeReportsAllImpactedCollectionsAndNothingElse() throws IOException {
    ...
    try (ZkStateReader reader = ClusterStateMockUtil.buildClusterState("csrr2rDcsr2", 1, 1, NODE1, NODE2)) {
      ...
      DocCollection docCollection = clusterState.getCollection("collection1");
      DocCollection docCollection2 = clusterState.getCollection("collection2");
      assertEquals(docCollection.toString(), docCollection.getReplica("replica1").getBaseUrl(), NODE1_URL);
      assertEquals(docCollection.toString(), docCollection.getReplica("replica2").getBaseUrl(), NODE2_URL);
      assertEquals(docCollection2.toString(), docCollection2.getReplica("replica4").getBaseUrl(), NODE2_URL);
    }

...
public class RollingRestartTest extends AbstractFullDistribZkTestBase {
  ...
  public void restartWithRolesTest() throws Exception {
      String leader = OverseerCollectionConfigSetProcessor.getLeaderNode(ZkStateReader.from(cloudClient).getZkClient());
    assertNotNull(leader);
    log.info("Current overseer leader = {}", leader);

      ZkStateReader.from(cloudClient).getZkClient().printLayoutToStream(System.out);
    ...
    for (int i = 0; i < numDesignateOverseers; i++) {
      ...
      waitUntilOverseerDesignateIsLeader(ZkStateReader.from(cloudClient).getZkClient(), designates, MAX_WAIT_TIME);
    ...
    for (int i = 0; i < numRestarts; i++) {
      ...
      for (CloudJettyRunner cloudJetty : designateJettys) {
        ...
        chaosMonkey.stopJetty(cloudJetty);
          ...
          boolean liveDesignates = CollectionUtils.intersection(ZkStateReader.from(cloudClient).getClusterState().getLiveNodes(), designates).size() > 0;
        if (liveDesignates) {
          sawLiveDesignate = true;
            boolean success = waitUntilOverseerDesignateIsLeader(ZkStateReader.from(cloudClient).getZkClient(), designates, MAX_WAIT_TIME);
          if (!success) {
              leader = OverseerCollectionConfigSetProcessor.getLeaderNode(ZkStateReader.from(cloudClient).getZkClient());
            if (leader == null)
                log.error("NOOVERSEER election queue is : {}"
                    , OverseerCollectionConfigSetProcessor.getSortedElectionNodes(ZkStateReader.from(cloudClient).getZkClient(),
        ...
        if (!success) {
          ...
          ZkStateReader.from(cloudClient).updateLiveNodes();
          sawLiveDesignate = CollectionUtils.intersection(ZkStateReader.from(cloudClient).getClusterState().getLiveNodes(), designates).size() > 0;

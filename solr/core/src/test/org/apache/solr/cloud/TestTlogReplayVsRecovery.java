...
@AwaitsFix(
    bugUrl =
        "https://issues.apache.org/jira/browse/SOLR-13486;https://issues.apache.org/jira/browse/SOLR-14183")
public class TestTlogReplayVsRecovery extends SolrCloudTestCase {
  ...
  public void testManyDocsInTlogReplayWhileReplicaIsTryingToRecover() throws Exception {
    ...
    assertEquals(
        "Something broke our expected skipIndexWriterCommitOnClose",
        TEST_VALUE_FOR_SKIP_COMMIT_ON_CLOSE,
  ...
  private void addDocs(final boolean commit, final int numDocs, final int startId)
    ...
    try (SolrClient client =
        new HttpJettySolrClient.Builder(NODE0.getBaseUrl().toString()).build()) {
  ...
  private void assertDocsExistInBothReplicas(int firstDocId, int lastDocId) throws Exception {
    try (SolrClient leaderSolr =
        ...
        SolrClient replicaSolr =
            new HttpJettySolrClient.Builder(NODE1.getBaseUrl().toString()).build()) {

...
public class LeaderVoteWaitTimeoutTest extends SolrCloudTestCase {
  ...
  public void testMostInSyncReplicasCanWinElection() throws Exception {
    ...
    // All three replicas must be active (registered + in sync) before we index, so their shard terms
    // are bumped together by the first update. This fork's addReplicaToShard can return before the
    // replica is active; without this wait a just-added replica may register its term after the first
    // doc bumps the others, leaving it permanently a term behind and failing the term assertions below.
  ...
  private void addDoc(String collection, int docId, JettySolrRunner solrRunner) throws IOException, SolrServerException {
    try (Http2SolrClient solrClient = new Http2SolrClient.Builder(solrRunner.getBaseUrl().toString()).build()) {

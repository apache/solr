...
/**
 * Tests various permutations of SSL options with {@link MiniSolrCloudCluster}. <b>NOTE: This Test
 * ignores the randomized SSL &amp; clientAuth settings selected by base class</b>, instead each
 * method initializes a {@link SSLTestConfig} will specific combinations of settings to test.
 *
...
public class TestMiniSolrCloudClusterSSL extends SolrTestCaseJ4 {
  ...
  /**
   * Constructs a cluster with the specified sslConfigs, runs {@link
   * #checkClusterWithCollectionCreations}, then verifies that if we modify the default SSLContext
   * (mimicing <code>javax.net.ssl.*</code> sysprops set on JVM startup) and reset to the default
   * HttpClientBuilder, new HttpJettySolrClient instances will still be able to talk to our servers.
   *
  ...
  private void checkClusterWithNodeReplacement(SSLTestConfig sslConfig) throws Exception {
    ...
    final MiniSolrCloudCluster cluster =
        new MiniSolrCloudCluster(NUM_SERVERS, createTempDir(), config);
    try {
   ...
   *
   * <ol>
   *   <li>Upload a config set
   *   <li>verifies a collection can be created
   *   <li>verifies many things that should succeed/fail when communicating with the cluster
   *       according to the specified sslConfig
   *   <li>shutdown a server &amp; startup a new one in it's place
   *   <li>repeat the verifications of ssl / no-ssl communication
   *   <li>create a second collection
   * </ol>
   *
   * @see #CONF_NAME
   * @see #NUM_SERVERS
   */
  public static void checkClusterWithCollectionCreations(
   ...
   */
  private static void checkCreateCollection(
      final MiniSolrCloudCluster cluster, final String collection) throws Exception {
  ...
  private static void checkClusterJettys(
    ...
    for (JettySolrRunner jetty : jettys) {
      ...
      final String wrongBaseURL =
          baseURL.replaceFirst((ssl ? "https://" : "http://"), (ssl ? "http://" : "https://"));
   ...
   */
  private static HttpClient newSslAwareClientWithNoClientCerts() throws Exception {
    ...
    SslContextFactory.Client sslContextFactory =
        clientConfig.buildClientSSLConfig().createClientContextFactory();
    assertNotNull(sslContextFactory);

    var clientConnector = new ClientConnector();
    clientConnector.setSslContextFactory(sslContextFactory);

    var transport = new HttpClientTransportOverHTTP(clientConnector);

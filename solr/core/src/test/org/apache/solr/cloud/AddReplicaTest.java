...
public class AddReplicaTest extends SolrCloudTestCase {
  ...
  public void testAddMultipleReplicas() throws Exception {
    ...
    assertEquals(2, getReplicas(EnumSet.of(Replica.Type.NRT), docCollection).size());
    assertEquals(1, getReplicas(EnumSet.of(Replica.Type.TLOG), docCollection).size());
    assertEquals(1, getReplicas(EnumSet.of(Replica.Type.PULL), docCollection).size());
    ...
    assertEquals(5, getReplicas(EnumSet.of(Replica.Type.NRT), docCollection).size());
    assertEquals(2, getReplicas(EnumSet.of(Replica.Type.TLOG), docCollection).size());
    assertEquals(2, getReplicas(EnumSet.of(Replica.Type.PULL), docCollection).size());
  }
  ...
  public void test() throws Exception {
    ...
    CollectionAdminRequest.AddReplica addReplica =
        CollectionAdminRequest.addReplicaToShard(collection, sliceName);

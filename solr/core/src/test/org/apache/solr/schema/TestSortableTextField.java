...
public class TestSortableTextField extends SolrTestCaseJ4 {
  ...
  public static void create() throws Exception {
    ...
    for (String n :
        Arrays.asList(
            "keyword_stxt", "whitespace_stxt", "whitespace_f_stxt", "whitespace_l_stxt")) {

      FieldType ft = h.getCore().getLatestSchema().getFieldTypeByName(n);
      assertEquals(
          "type " + ft.getTypeName() + " should have docvalues - schema got changed?",
          true,
          ft.getNamedPropertyValues(true).get("docValues"));
    }
    for (String n :
        Arrays.asList(
            "keyword_stxt", "keyword_dv_stxt",
      ...
      assertTrue(
          "field " + sf.getName() + " should have docvalues - schema got changed?",
          sf.hasDocValues());
    }
  ...
  public void testSimple() {
    ...
    assertU(
        adoc("id", "2", "whitespace_stxt", "how now brown dog ?", "whitespace_f_stxt", "bbb aaa"));
    assertU(
        adoc("id", "3", "whitespace_stxt", "how now brown cat ?", "whitespace_f_stxt", "xxx yyy"));
    ...
    for (String sortf :
        Arrays.asList("whitespace_stxt", "whitespace_nois_stxt", "whitespace_plain_str")) {
      assertQ(
          req("q", "whitespace_stxt:cat", "sort", sortf + " asc"),
          "//*[@numFound='2']",
          "//result/doc[1]/str[@name='id'][.=4]",
          "//result/doc[2]/str[@name='id'][.=3]");
      assertQ(
          req("q", "whitespace_stxt:cat", "sort", sortf + " desc"),
          "//*[@numFound='2']",
          "//result/doc[1]/str[@name='id'][.=3]",
          "//result/doc[2]/str[@name='id'][.=4]");
      assertQ(
          req("q", "whitespace_stxt:brown", "sort", sortf + " asc"),
          "//*[@numFound='3']",
          "//result/doc[1]/str[@name='id'][.=3]",
          "//result/doc[2]/str[@name='id'][.=1]",
          "//result/doc[3]/str[@name='id'][.=2]");
      assertQ(
          req("q", "whitespace_stxt:brown", "sort", sortf + " desc"),
          "//*[@numFound='3']",
      ...
      assertQ(
          req("q", "whitespace_nodv_stxt:cat", "sort", sortf + " asc"),
          "//*[@numFound='2']",
          "//result/doc[1]/str[@name='id'][.=4]",
          "//result/doc[2]/str[@name='id'][.=3]");
    }
    ...
    assertQ(
        req("q", "*:*", "sort", "whitespace_f_stxt asc"),
        "//*[@numFound='4']",
        "//result/doc[1]/str[@name='id'][.=4]",
        "//result/doc[2]/str[@name='id'][.=1]",
        "//result/doc[3]/str[@name='id'][.=2]",
    ...
    assertQ(
        req("q", "*:*", "sort", "whitespace_f_stxt desc"),
        "//*[@numFound='4']",
        "//result/doc[1]/str[@name='id'][.=4]",
        "//result/doc[2]/str[@name='id'][.=3]",
        "//result/doc[3]/str[@name='id'][.=2]",
    ...
    assertQ(
        req("q", "*:*", "sort", "whitespace_l_stxt asc"),
        "//*[@numFound='4']",
    ...
    assertQ(
        req("q", "*:*", "sort", "whitespace_l_stxt desc"),
        "//*[@numFound='4']",
        "//result/doc[1]/str[@name='id'][.=3]",
        "//result/doc[2]/str[@name='id'][.=2]",
      ...
      for (String search :
        ...
        assertQ(
            req(
                ...
                "0",
                "json.facet",
                "{x:{ type: terms, field:'" + facet + "', mincount:0 }}"),
            "//*[@numFound='3']",
  ...
  public void testMaxCharsSort() {
    ...
    for (String searchF :
        Arrays.asList(
            "whitespace_stxt", "whitespace_plain_txt",
      ...
      for (String sortF :
          Arrays.asList(
              "whitespace_stxt", "whitespace_plain_str",
              "whitespace_max0_stxt", "whitespace_maxNeg_stxt")) {

        assertQ(
            req("q", searchF + ":ccc", "sort", sortF + " desc, id asc"),
            "//*[@numFound='2']",
            "//result/doc[1]/str[@name='id'][.=3]",
            "//result/doc[2]/str[@name='id'][.=1]");

        assertQ(
            req("q", searchF + ":ccc", "sort", sortF + " asc, id desc"),
            "//*[@numFound='2']",
            "//result/doc[1]/str[@name='id'][.=1]",
            "//result/doc[2]/str[@name='id'][.=3]");
      }
    ...
    for (String sortF : Arrays.asList("whitespace_stxt", "whitespace_nois_stxt")) {
      for (String dir : Arrays.asList("asc", "desc")) {
        assertQ(
            req("q", "whitespace_stxt:zzz", "sort", sortF + " " + dir + ", id asc"),
            "//*[@numFound='2']",
            "//result/doc[1]/str[@name='id'][.=5]",
            "//result/doc[2]/str[@name='id'][.=6]");
        assertQ(
            req("q", "whitespace_stxt:zzz", "sort", sortF + " " + dir + ", id desc"),
            "//*[@numFound='2']",
  ...
  public void testUseDocValuesAsStored() {
    ...
    for (Map.Entry<String, FieldType> entry :
        h.getCore().getLatestSchema().getFieldTypes().entrySet()) {
    ...
    assertEquals(
        "sanity check: wrong number of *_has_usedvs types found -- schema changed?",
        2,
        num_types_found);
    ...
    for (Map.Entry<String, SchemaField> entry :
        h.getCore().getLatestSchema().getFields().entrySet()) {
      if (entry.getKey().endsWith("_usedvs")) {
        ...
        assertTrue(
            "schema change broke assumptions: field must be '*_has_usedvs' or '*_negates_usedvs': "
                + name,
            usedvs ^ name.endsWith("_negates_usedvs"));
        final boolean max6 = name.startsWith("max6_");
        assertTrue(
            "schema change broke assumptions: field must be 'max6_*' or 'max0_*': " + name,
            max6 ^ name.startsWith("max0_"));

        assertEquals(
            "Unexpected useDocValuesAsStored value for field: " + name,
            usedvs,
            sf.useDocValuesAsStored());
        ...
        if (usedvs && max6) {
          ...
          for (String expect :
            ...
            assertTrue(
                "exception must mention " + expect + ": " + ex.getMessage(),
    ...
    assertEquals(
        "sanity check: wrong number of *_usedvs fields found -- schema changed?",
        6,
        num_fields_found);
  ...
  public void testRandomStrEquivalentBehavior() throws Exception {
    ...
    // we use embedded client instead of assertQ: we want to compare the responses from multiple
    // requests
    ...
    for (String f : test_fields) {
      ...
      final SolrDocumentList results =
          client
              ...
              .getResults();
      assertEquals(
          f + ": Query (" + magic + ") filtered by id: " + magicIdx + " ==> " + results,
          1L,
          results.getNumFound());
      final SolrDocument doc = results.get(0);
      assertEquals(
          f + ": Query (" + magic + ") filtered by id: " + magicIdx + " ==> " + doc,
          "" + magicIdx,

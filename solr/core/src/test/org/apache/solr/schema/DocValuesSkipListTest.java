...
public class DocValuesSkipListTest extends SolrTestCaseJ4 {

  private static final Logger log = LoggerFactory.getLogger(MethodHandles.lookup().lookupClass());

  private static final String[] DOC_VALUES_FIELDS = {
    "floatdv",
    "intdv",
    "doubledv",
    "longdv",
  ...
  public static void beforeTests() throws Exception {
    initCore("solrconfig-basic.xml", "schema-docValuesSkipList.xml");

    // sanity check our schema meets our expectations
    final IndexSchema schema = h.getCore().getLatestSchema();
    for (String fieldName : DOC_VALUES_FIELDS) {
      final SchemaField sf = schema.getField(fieldName);
      if (fieldName.endsWith("dvs")) {
        assertTrue(
            fieldName + " is not multiValued, test is useless, who changed the schema?",
            sf.multiValued());
      } else {
        assertFalse(
            fieldName + " is multiValued, test is useless, who changed the schema?",
            sf.multiValued());
      }
      if (!fieldName.contains("enum")) {
        assertFalse(
            fieldName + " is indexed, test is useless, who changed the schema?", sf.indexed());
      } else {
        assertTrue(
            fieldName + " is indexed, test is useless, who changed the schema?", sf.indexed());
      }
      assertTrue(
          fieldName + " has no docValues, test is useless, who changed the schema?",
          sf.hasDocValues());
      assertTrue(
          fieldName + " has no docValuesSkipList, test is useless, who changed the schema?",
  ...
  public void testDocValues() throws IOException {
    ...
    try (SolrCore core = h.getCoreInc()) {
      ...
      try {
        ...
        for (String f : DOC_VALUES_FIELDS) {
          assertEquals(DocValuesSkipIndexType.RANGE, infos.fieldInfo(f).docValuesSkipIndexType());
        }

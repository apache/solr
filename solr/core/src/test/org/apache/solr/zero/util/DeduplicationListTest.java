...
public class DeduplicationListTest extends Assert {
  ...
  public void mergePutGet() throws Exception {
    ...
    // Because we had a merge in the list, the merge result might not be equal to any of the merged
    // objects (well in this case it will given we only have the model and the year, but in general
    // it will not, and since we didn't implement equals() in DeduplicatableCar comparing to the
    // inserted object wouldn't work anyway)
    assertEquals(
        "expected the merged car to have the oldest of the two years",
        1999,
        dedupeList.removeFirst().year);
  }
  ...
  public void interleavedMerges1() throws Exception {
    ...
    // We expect Leon to be returned first (since it was inserted first) and be returned with 1999
    // since it's the older date
    // Then Dauphine should be returned second, with the older 1958 date as well
  ...
  /**
   * Tests 4 interleaved puts into the list that merge 2 by 2, inserted in a different order from
   * previous test.
   */
  @Test
  public void interleavedMerges2() throws Exception {
    // Note Dauphine 1958 inserted first and Dauphine 1963 second (difference with test
    // interleavedMerges1)
    dedupeList.addDeduplicated(new DeduplicatableCar("Leon", 2018), false);
    dedupeList.addDeduplicated(new DeduplicatableCar("Dauphine", 1958), false);
    dedupeList.addDeduplicated(new DeduplicatableCar("Leon", 1999), false);
  ...
  public void listFull() throws Exception {
    ...
    // We are allowed to merge into this additional entry even though we don't set isReenqueue to
    // true
    dedupeList.addDeduplicated(new DeduplicatableCar("Dauphine", 1958), false);
  ...
  /** Test consumers wake any blocked producers when the list is full */
  @Test
  public void listProducerConsumer() throws Exception {
    // set up a full list so producers can block when adding new tasks
    List<DeduplicatableCar> cars = new ArrayList<>();
    cars.add(new DeduplicatableCar("Leon", 2018));
    cars.add(new DeduplicatableCar("Dauphine", 1963));

    AtomicBoolean added = new AtomicBoolean(false);
    dedupeList = new DeduplicatingList<>(1, new OldCarMerger());
    dedupeList.addDeduplicated(cars.remove(0), false);

    // producer tries to add item and blocks
    ...
    // wait for the producer thread to begin processing
    startCdl.await();

    // ensure our producer is still waiting
    Thread.sleep(500);
    assertFalse(added.get());

    // consumer taking the first item should unblock the producer
    dedupeList.removeFirst();
    // block until the producer adds the next item
    dedupeList.removeFirst();

    try {
      // wait for the producer to complete then verify - if the thread doesn't complete then our
      // producer is interrupted and the test should fail
      producer.join(5000);
      assertEquals(0, cars.size());
      assertEquals(0, dedupeList.size());
    } finally {
      // if the producer completed, no-op
      producer.interrupt();
    }
  }
  ...
  /**
   * Given the {@link DeduplicatableCar}, this merger method keeps the oldest of the two cars. For
   * example we're collecting old cars and the older the better :)
   */

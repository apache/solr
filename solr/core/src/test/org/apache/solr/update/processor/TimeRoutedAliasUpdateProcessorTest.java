...
@LuceneTestCase.AwaitsFix(bugUrl = "https://issues.apache.org/jira/browse/SOLR-13059")
public class TimeRoutedAliasUpdateProcessorTest extends RoutedAliasUpdateProcessorTest {
  ...
  public void testMaxFutureMs() throws Exception {
    ...
    DateTimeFormatter SOLR_DATE_TIME_FORMATTER =
    ...
    final String dayB =
        DateTimeFormatter.ISO_INSTANT.format(
            DateMathParser.parseMath(new Date(), YYYY_MM_DD_b).toInstant());
    assertUpdateResponse(add(alias, Arrays.asList(sdoc("id", "2", "timestamp_dt", dayB)), params));

    final String dayC =
        DateTimeFormatter.ISO_INSTANT.format(
            DateMathParser.parseMath(new Date(), YYYY_MM_DD_c).toInstant());
    try {
      assertUpdateResponse(
          add(alias, Arrays.asList(sdoc("id", "3", "timestamp_dt", dayC)), params));
          ...
          ex.getMessage()
              .contains(
                  "The document's time routed key of "
                      + dayC
                      + " is too far in the future given router.maxFutureMs="
                      + maxFutureMs));
    }
  }

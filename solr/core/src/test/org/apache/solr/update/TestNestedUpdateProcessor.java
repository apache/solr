...
public class TestNestedUpdateProcessor extends SolrTestCaseJ4 {
  ...
  public void testDeeplyNestedURPGrandChild() throws Exception {
    ...
    assertJQ(
        req(
            "q",
            IndexSchema.NEST_PATH_FIELD_NAME + ":*/grandChild",
            "fl",
            "*, _nest_path_",
            "sort",
            "id desc",
  ...
  public void testNumberInName() throws Exception {
    ...
    // assertQ(req("qt", "/terms", "terms.fl", IndexSchema.NEST_PATH_FIELD_NAME), "false"); // for
    // debugging

    // find it
    assertJQ(
        req("q", "{!field f=" + IndexSchema.NEST_PATH_FIELD_NAME + "}/children/grandChild99"),
        "/response/numFound==1");
    // should *NOT* find it; different number
    assertJQ(
        req("q", "{!field f=" + IndexSchema.NEST_PATH_FIELD_NAME + "}/children/grandChild22"),
        "/response/numFound==0");
  }
  ...
  public void testDeeplyNestedURPChildren() throws Exception {
    ...
    assertJQ(
        req(
            "q",
            IndexSchema.NEST_PATH_FIELD_NAME + ":\\/children",
            "fl",
            "*, _nest_path_",
            "sort",
            "id asc",
    ...
    assertJQ(
        req(
            "q",
            IndexSchema.NEST_PATH_FIELD_NAME + ":\\/anotherChildList",
            "fl",
            "*, _nest_path_",
            "sort",
            "id asc",
  ...
  public void testDeeplyNestedURPSanity() throws Exception {
    SolrInputDocument docHierarchy =
        sdoc(
            "id",
            "1",
            "children",
            sdocs(
                sdoc("id", "2", "name_s", "Yaz"),
                sdoc("id", "3", "name_s", "Jazz", "grandChild", sdoc("id", "4", "name_s", "Gaz"))),
            "lonelyChild",
            sdoc("id", "5", "name_s", "Loner"));
    ...
    assertEquals(
        "SolrInputDocument(fields: [id=2, name_s=Yaz, _nest_path_=/children#0, _nest_parent_=1])",
        firstChild.toString());

    SolrInputDocument secondChild = (SolrInputDocument) children.get(1);
    assertEquals(
        "SolrInputDocument(fields: [id=3, name_s=Jazz, grandChild=SolrInputDocument(fields: [id=4, name_s=Gaz, _nest_path_=/children#1/grandChild#, _nest_parent_=3]), _nest_path_=/children#1, _nest_parent_=1])",
        secondChild.toString());

    SolrInputDocument grandChild =
        (SolrInputDocument) ((SolrInputDocument) children.get(1)).get("grandChild").getValue();
    assertEquals(
        "SolrInputDocument(fields: [id=4, name_s=Gaz, _nest_path_=/children#1/grandChild#, _nest_parent_=3])",
        grandChild.toString());

    SolrInputDocument singularChild =
        (SolrInputDocument) docHierarchy.get("lonelyChild").getValue();
    assertEquals(
        "SolrInputDocument(fields: [id=5, name_s=Loner, _nest_path_=/lonelyChild#, _nest_parent_=1])",
  ...
  public void testDeeplyNestedURPChildrenWoId() throws Exception {
    ...
    SolrInputDocument noIdChildren =
        sdoc(
            "id",
            rootId,
            "children",
            sdocs(sdoc("name_s", "Yaz"), sdoc("name_s", "Jazz", childKey, sdoc("name_s", "Gaz"))));
    UpdateRequestProcessor nestedUpdate =
        new NestedUpdateProcessorFactory().getInstance(req(), null, null);
    AddUpdateCommand cmd = new AddUpdateCommand(req());
    ...
    SolrInputDocument idLessChild =
        (SolrInputDocument) ((SolrInputDocument) children.get(1)).get(childKey).getValue();
    assertTrue("Id less child did not get an Id", idLessChild.containsKey("id"));
    assertEquals(
        "Id less child was assigned an unexpected id",
        expectedId,
        idLessChild.getFieldValue("id").toString());
  }
   ...
   * QParsers when a specific <code>_nest_path_</code> is desired
   *
  ...
  private static class RandomNestedDocModel {
    ...
    private synchronized SolrInputDocument buildRandomDoc(
        SolrInputDocument parent, List<String> test_path, int maxDepthAndBreadth) {
      ...
      final SolrInputDocument doc =
          sdoc(
              ...
              "num_direct_kids_s", "0",
              // conceptually matches _nest_path_ but should be easier to make assertions about (no
              // inline position #s)
      ...
      if (null != parent) {
        ...
        if (parent.containsKey(
            "ancestor_ids_ss")) { // sigh: getFieldValues returns null, not empty collection
      ...
      if (0 < maxDepthAndBreadth) {
        ...
        for (int i = 0; i < numDirectKids; i++) {
          ...
          if (kid.containsKey(
              "descendent_ids_ss")) { // sigh: getFieldValues returns null, not empty collection
    ...
    /**
     * Loops over the 'model' of every document we've indexed, asserting that parent/child queries
     * wrapping an '<code>id:foo</code> using various paths match the expected ancestors/descendents
     */
      ...
      for (String doc_id : allDocs.keySet()) {
        ...
        if (!doc_path.equals("/")) {
          ...
          final String ancestorId =
              allAncestorIds.get(random().nextInt(allAncestorIds.size())).toString();
          final String ancestor_path =
              allDocs.get(ancestorId).getFieldValue("test_path_s").toString();

          final Collection<Object> allOfAncestorsDescendentIds =
              allDocs.get(ancestorId).getFieldValues("descendent_ids_ss");

          assertTrue(
              "Sanity check " + ancestorId + " ancestor of " + descendentId,
     ...
     * only docs matched have the expected path, and at least one kid (since this is the "parents"
     * parser)
     *
     * <p>(using <code>*:*</code> as our inner query keeps the validation simple and also helps
     * stress out risk of matching incorrect docs if the 'which' param is wrong)
     *
     ...
     * @param inner_child_query the specific children whose ancestors we are looking for, must be
     ...
     *
     * <p>(using <code>*:*</code> as our inner query keeps the validation simple and also helps
     * stress out risk of matching incorrect docs if the 'of' param is wrong)
     *
     * @return total number of queries checked (assuming no assertion failures)
     */
     ...
     *
     * @param parent_path the nest path of the parents to consider
     * @param inner_parent_query the specific parents whose descendents we are looking for, must be
     *     simple string <code>*:*</code> or <code>id:foo</code>
     */

...
public class TestDocBasedVersionConstraints extends SolrTestCaseJ4 {
  ...
  /**
   * Sanity check that there are no hardcoded assumptions about the field type used that could byte
   * us in the ass.
   */
  public void testFloatVersionField() throws Exception {
    ...
    updateJ(
        jsonAdd(sdoc("id", "aaa", "name", "a1", "my_version_f", "10.01")),
    ...
    // skip low version delete against uncommitted doc from updateLog
    updateJ(
        jsonAdd(sdoc("id", "aaa", "name", "a2", "my_version_f", "10.02")),
        params("update.chain", "external-version-float"));
    deleteAndGetVersion(
        "aaa",
        params(
  ...
  public void testFailOnOldVersion() throws Exception {
    ...
    updateJ(
        jsonAdd(sdoc("id", "aaa", "name", "a1", "my_version_l", "1001")),
    ...
    updateJ(
        jsonAdd(sdoc("id", "aaa", "name", "a2", "my_version_l", "1002")),
        params("update.chain", "external-version-failhard"));
    ex =
        expectThrows(
            SolrException.class,
  ...
  public void testMultipleVersions() throws Exception {
    ...
    assertU(commit());
    // All variations of additional versions should fail other than my_version_l greater or
    // my_version_f greater.
    SolrException ex =
        expectThrows(
            SolrException.class,
            () -> {
    ...
    // update version 1
    updateJ(
        jsonAdd(sdoc("id", "aaa", "name", "Y1", "my_version_l", "2001", "my_version_f", "1.0")),
        params("update.chain", "external-version-failhard-multiple"));
    assertU(commit());
    ...
    // update version 2
    updateJ(
        jsonAdd(sdoc("id", "aaa", "name", "Y2", "my_version_l", "2001", "my_version_f", "2.0")),
        params("update.chain", "external-version-failhard-multiple"));
    assertU(commit());
  ...
  /**
   * Proof of concept test demonstrating how to manage and periodically cleanup the "logically"
   * deleted documents
   */
  ...
  /**
   * Constantly hammer the same doc with multiple concurrent threads and diff versions, confirm that
  ...
  public void testConcurrentAdds() throws Exception {
    ...
    ExecutorService runner =
        ExecutorUtil.newMDCAwareFixedThreadPool(
            MAX_CONCURENT, new SolrNamedThreadFactory("TestDocBasedVersionConstraints"));
    ...
    try {
      for (int id = 0; id < NUM_DOCS; id++) {
        ...
        for (int variant = 0; variant < numAdds; variant++) {
          ...
          final long version =
              (iShouldWin ? winnerVersion : TestUtil.nextInt(random(), 1, winnerVersion - 1));
        ...
        assertU(commit());
        assertJQ(
            req("q", "id:" + id, "fl", "id,name,my_version_l"),
            "/response/numFound==1",
  ...
  public void testMissingVersionOnOldDocs() throws Exception {
    ...
    updateJ(
        json("[{\"id\": \"b\", \"name\": \"b1\"}]"), params("update.chain", "no-external-version"));
    assertU(commit());
    ...
    updateJ(
        json("[{\"id\": \"b\", \"name\": \"b1\", \"my_version_l\": " + version + "}]"),
        params("update.chain", "external-version-support-missing"));
    assertU(commit());
    assertJQ(req("q", "*:*"), "/response/numFound==2");
    assertJQ(
  ...
  public void testTombstoneConfig() throws Exception {
    ...
    updateWithChain("tombstone-config", "id", "b!doc1", "my_version_l", "1");
    assertU(commit());
    ...
    deleteAndGetVersion("b!doc1", params("del_version", "2", "update.chain", "tombstone-config"));
    assertU(commit());
  ...
  public void testCanCreateTombstonesBasic() {
    DocBasedVersionConstraintsProcessorFactory factory =
        new DocBasedVersionConstraintsProcessorFactory();

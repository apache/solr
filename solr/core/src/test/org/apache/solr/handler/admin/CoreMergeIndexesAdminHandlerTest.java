...
public class CoreMergeIndexesAdminHandlerTest extends SolrTestCaseJ4 {
  ...
  @AfterClass
  public static void afterCoreMergeIndexesAdminHandlerTest() throws Exception {
    deleteCore();
  }
  ...
  public void testMergeIndexesCoreAdminHandler() throws Exception {
    ...
    try (CoreContainer cores = h.getCoreContainer()) {

      try (final CoreAdminHandler admin = new CoreAdminHandler(cores); SolrCore core = cores.getCore("collection1")) {
        DirectoryFactory df = core.getDirectoryFactory();
        FailingDirectoryFactory dirFactory = (FailingDirectoryFactory) df;

        try (SolrQueryRequest req = req(CoreAdminParams.ACTION, CoreAdminParams.CoreAdminAction.MERGEINDEXES.toString(), CoreAdminParams.CORE, "collection1",

...
public class CombinedQuerySolrCloudTest extends AbstractFullDistribZkTestBase {
  ...
  public void testQueriesWithFacetAndHighlights() throws Exception {
    ...
    assertEquals("[2 (3), 0 (2), 1 (2)]", rsp.getFacetFields().get(0).getValues().toString());
  ...
  public void testElevatedQueriesWithFacetAndHighlights() throws Exception {
    ...
    String jsonQuery =
        "{\"queries\": {"
            ...
            + "\"limit\": 4,"
            + "\"fields\": [\"id\", \"score\", \"title\"],"
            + "\"params\": {\"combiner\": true, \"elevateIds\": \"6,10\","
            + "\"combiner.query\": [\"lexical1\", \"lexical2\"],"
    ...
    assertEquals(
        ...
        rsp.getHighlighting().get("10").get("title").get(0));
    assertEquals(
        "title <em>test</em> for <em>doc</em> 5",
        rsp.getHighlighting().get("5").get("title").get(0));
  }
  ...
  public void testQueriesWithFacetAndHighlightsCollapse() throws Exception {
    ...
    String jsonQuery =
        "{\"queries\": {"
            + "\"lexical1\": {\"lucene\": {\"query\": \"id:(CO!2^=3 OR CO!3^=1 OR CO!6^=2 OR CO!5^=1)\"}},"
            + "\"lexical2\": {\"lucene\": {\"query\": \"id:(CO!8^=1 OR CO!5^=2 OR CO!7^=3 OR CO!10^=2)\"}}},"
            + "\"limit\": 3,"
            + "\"fields\": [\"id\", \"score\", \"title\"],"
            + "\"params\": {\"combiner\": true, \"facet\": true, \"facet.field\": \"id\","
            + "\"fq\": [\"{!collapse field=mod3_idv sort='id asc, score desc'}\"],"
    ...
    assertEquals(
        "title <em>test</em> for <em>doc</em> 2",
        rsp.getHighlighting().get("CO!2").get("title").get(0));

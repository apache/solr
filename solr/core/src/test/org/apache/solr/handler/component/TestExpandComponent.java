...
public class TestExpandComponent extends SolrTestCaseJ4 {
  ...
  private void _testExpand(String group, String floatAppend, String hint) {
    ...
    String[][] docs = {
      ...
      {
        "id", "3", "term_s", "YYYY", "test_i", "5000", "test_l", "100", "test_f", "200", "type_s",
        "other"
      },
      {
        "id", "4", "term_s", "YYYY", "test_i", "40", "test_l", "1000", "test_f", "2000", "type_s",
        "other"
      },
      {
        "id",
    ...
    assertQ(
        ...
        "/response/lst[@name='expanded']/result[@name='1"
            + floatAppend
            + "']/doc[2]/str[@name='id'][.='7']",
        "/response/lst[@name='expanded']/result[@name='0"
            + floatAppend
            + "']/doc[1]/str[@name='id'][.='5']",
        "/response/lst[@name='expanded']/result[@name='0"
            + floatAppend
    ...
    assertQ(
        req(params, "fq", "{!collapse field=" + group + hint + " nullPolicy=collapse}"),
        ...
        "/response/lst[@name='expanded']/result[@name='0"
            ...
            + "']/doc[2]/str[@name='id'][.='5']");
    assertQ(
        req(params, "fq", "{!collapse field=" + group + hint + " nullPolicy=expand}"),
    ...
    assertQ(
        req(
            ...
            "fq",
            "{!collapse field=" + group + hint + " nullPolicy=expand}"),
        "*[count(/response/result/doc)=4]",
        "*[count(/response/lst[@name='expanded']/result)=2]",
        "/response/result/doc[1]/str[@name='id'][.='3']",
        "/response/result/doc[2]/str[@name='id'][.='2']",
    ...
    assertQ(
        req(
            params,
            // limiting rows should cause null group to pop up since we now have a null doc not on
            // page...
            "rows",
            "2",
            "fq",
    ...
    assertQ(
        req(params, "fq", "{!collapse field=" + group + hint + "}"),
        "*[count(/response/result/doc)=2]",
        "/response/result/doc[1]/str[@name='id'][.='2']",
        "/response/result/doc[2]/str[@name='id'][.='6']",
        "*[count(/response/lst[@name='expanded']/result)=2]",
    ...
    assertQ(
        req(
            params,
            // same, but with collapsed nulls and a nullGroup, we should have our expanded null
            // group count
            "fq",
            "{!collapse field=" + group + hint + " nullPolicy=collapse}",
            "expand.nullGroup",
            "true"),
        "*[count(/response/result/doc)=3]",
        "/response/result/doc[1]/str[@name='id'][.='3']",
    ...
    assertQ(
        ...
        "*[count(/response/lst[@name='expanded']/result)=0]");
    assertQ(
        req(params, "expand.nullGroup", "true"),
        "*[count(/response/result/doc)=0]",
        "*[count(/response/lst[@name='expanded']/result)=0]");
  ...
  public void testErrorCases() {
    ...
    try (ErrorLogMuter missingExpandField = ErrorLogMuter.regex("missing expand field");
        ErrorLogMuter expectedIdentifier = ErrorLogMuter.regex("Expected identifier at pos 2");
        ErrorLogMuter sortOrder = ErrorLogMuter.regex("Can't determine a Sort Order");
        ErrorLogMuter expandNotSupported =
            ErrorLogMuter.regex("Expand not supported for fieldType:'text'")) {
      ...
      assertTrue(
          e.getMessage()
              .contains("Can't determine a Sort Order (asc or desc) in sort spec 'bleh'"));

...
public class TestUninvertingReader extends SolrTestCase {
  ...
  //            DirectoryReader.open(dir), Collections.singletonMap("foo",
  // Type.SORTED_SET_INTEGER));
  ...
  //
  //  /**
  //   * Tests {@link Type#SORTED_SET_INTEGER} using Integer based fields, with and w/o precision
  // steps
  //   */
  ...
  //
  //    { // (at least) one doc should have every value, so that at least one segment has every
  // value
  ...
  //        assertTrue(
  //            f
  //                + ": Expected no more then "
  //                + EXPECTED_VALSET_SIZE
  ...
  //          // tighter check on multi fields in single segment index since we know one doc has all
  // of
  //          // them
  //          assertEquals(
  //              f + ": Single segment LeafReader's value set should have had exactly expected
  // size",
  //              EXPECTED_VALSET_SIZE,
  //              valSetSize);
  //        }
  ...
  //      final long valSetSize = v.getValueCount();
  //      assertEquals(
  //          f + ": Composite reader value set should have had exactly expected size",
  //          EXPECTED_VALSET_SIZE,
  //          valSetSize);
  //    }
  //
  ...
  public void testSortedSetEmptyIndex() throws IOException {
    ...
    for (String f : UNINVERT_MAP.keySet()) {
      ...
      for (LeafReaderContext rc : ir.leaves()) {
        final LeafReader ar = rc.reader();
        assertNull(
            f + ": Expected no doc values from empty index (leaf)", ar.getSortedSetDocValues(f));
      }

      // check the composite
      assertNull(
          f + ": Expected no doc values from empty index (composite)",
  ...
  //    assertSame(
  //        "do not rebuild FieldInfo for unaffected fields",
  //        originalLeafReader.getFieldInfos().fieldInfo("id"),

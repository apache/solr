...
public class QueryEqualityTest extends SolrTestCaseJ4 {
  ...
  /**
   * The {@code name} local parameter is handled centrally in {@link QParser#getQuery()} and wraps
   * the parsed query with {@link NamedMatches}. Verify that the wrapping is consistent (equals +
   * hashCode) across the parsers.
   */
  public void testNameLocalParam() throws Exception {
    assertQueryEquals(
        "term", "{!term name=my_label f=foo_s}hello", "{!term name=my_label f=foo_s v=hello}");

    assertQueryEquals(
        "prefix", "{!prefix name=my_label f=foo_s}hel", "{!prefix name=my_label f=foo_s v=hel}");

    assertQueryEquals(
        "fuzzy", "{!fuzzy name=my_label f=foo_s}hello", "{!fuzzy name=my_label f=foo_s v=hello}");

    assertQueryEquals(
        "terms", "{!terms name=my_label f=foo_s}a,b", "{!terms name=my_label f=foo_s v='a,b'}");

    assertQueryEquals(
        "bool", "{!bool name=my_label must=foo_s:hello}", "{!bool name=my_label must=foo_s:hello}");
  ...
  public void testNamedBoolQuery() throws Exception {
    Query actual =
        QParser.getParser("{!bool name=my_bool must=name:foo should=name:bar}", req()).getQuery();
  ...
  public void testNamedBoolQueryWithExcludeTags() throws Exception {
    ...
    Query actual =
        QParser.getParser("{!bool name=my_ref must=$ref excludeTags=t2}", req).getQuery();
    BooleanQuery inner =
        new BooleanQuery.Builder()
            .add(new TermQuery(new Term("name", "foo")), BooleanClause.Occur.MUST)
            .build();

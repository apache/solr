...
public class CurrencyRangeFacetCloudTest extends SolrCloudTestCase {
    ...
    for (int id = 0; id < NUM_DOCS; id++) {
      ...
      assertEquals(
          ...
          (new UpdateRequest().add(sdoc("id", "" + id, "x_s", x, FIELD, val)))
              .process(cluster.getSolrClientForCollection(COLLECTION))
              .getStatus());
    }
    assertEquals(0, cluster.getSolrClientForCollection(COLLECTION).commit().getStatus());
  }
  ...
  public void testSimpleRangeFacetsOfSymmetricRates() throws Exception {
    for (boolean use_mincount : Arrays.asList(true, false)) {
      ...
      for (List<String> args :
          Arrays.asList( // default currency is USD
              Arrays.asList("4", "1.00", "11.0", "4.00,USD", "1.00,USD", "11.00,USD"),
              // explicit USD
              Arrays.asList("4,USD", "1,USD", "11,USD", "4.00,USD", "1.00,USD", "11.00,USD"),
              // Gap can be in diff currency (but start/end must currently match)
              Arrays.asList("4.00,USD", "000.50,GBP", "11,USD", "4.00,USD", ".50,GBP", "11.00,USD"),
              Arrays.asList("4.00,USD", "2,EUR", "11,USD", "4.00,USD", "2.00,EUR", "11.00,USD"))) {
        ...
        SolrQuery solrQuery =
            new SolrQuery(
                ...
                "all");
        QueryResponse rsp = cluster.getSolrClientForCollection(COLLECTION).query(solrQuery);
        try {
        ...
        solrQuery =
            new SolrQuery(
                ...
                "{ foo:{ type:range, field:"
                    ...
                    + args.get(2)
                    + "', other:all}}");
        rsp = cluster.getSolrClientForCollection(COLLECTION).query(solrQuery);
        try {
          assertEquals(NUM_DOCS, rsp.getResults().getNumFound());
          ...
          if (use_mincount) {
            ...
            for (int i = 0; i < 7; i++) {
              ...
              assertEquals(
                  "bucket #" + i, (i == 0 || i == 3 || i == 6) ? 3L : 0L, bucket.get("count"));
            }
          }
        } catch (AssertionError | RuntimeException ae) {
          throw new AssertionError(solrQuery + " -> " + rsp + " ===> " + ae.getMessage(), ae);
  ...
  public void testJsonRangeFacetWithSubFacet() throws Exception {
    ...
    //  - the 2nd term in each bucket may vary based on shard/doc placement, but the count will
    // always be '1'
    // ...and in many cases (based on the shard/doc placement) this will require refinement to
    ...
    for (boolean use_domain : Arrays.asList(true, false)) {
      ...
      final SolrQuery solrQuery =
          new SolrQuery(
              ...
              "0",
              "json.facet",
              "{ bar:{ type:range, field:"
                  + FIELD
                  + ", "
                  + domain
                  + "        start:'0,EUR', gap:'10,EUR', "
                  + end
                  + ", other:all "
      ...
      try {
        ...
        @SuppressWarnings({"unchecked"})
        final NamedList<Object> bar =
            ((NamedList<NamedList<Object>>) rsp.getResponse().get("facets")).get("bar");
        @SuppressWarnings({"unchecked"})
        ...
        for (int i = 0; i < 2; i++) {
          ...
          @SuppressWarnings({"unchecked"})
          final List<NamedList<Object>> foo_buckets =
              ((NamedList<List<NamedList<Object>>>) bucket.get("foo")).get("buckets");
          assertEquals("bucket #" + i + " foo num buckets", 2, foo_buckets.size());
          assertEquals(
              "bucket #" + i + " foo top term",
              (0 == i ? "x2" : "x0"),
        ...
        { // between...
          @SuppressWarnings({"unchecked"})
          final List<NamedList<Object>> buckets =
              ((NamedList<List<NamedList<Object>>>) between.get("foo")).get("buckets");
        ...
        { // after...
          @SuppressWarnings({"unchecked"})
          final List<NamedList<Object>> buckets =
              ((NamedList<List<NamedList<Object>>>) after.get("foo")).get("buckets");
  ...
  public void testJsonRangeFacetAsSubFacet() throws Exception {
    ...
    // filter out the first 5 docs (by id), which should ensure that 'x2' is the top bucket
    // overall...
    ...
    for (boolean use_domain : Arrays.asList(true, false)) {
      ...
      final SolrQuery solrQuery =
          new SolrQuery(
              ...
              "0",
              "json.facet",
              "{ foo:{ type:terms, field:x_s, refine:true, limit:1, overrequest:1, "
                  + domain
                  + "        facet: { bar:{ type:range, field:"
                  + FIELD
                  + ", other:all, "

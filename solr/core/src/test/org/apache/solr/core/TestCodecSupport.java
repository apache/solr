...
public class TestCodecSupport extends SolrTestCaseJ4 {
  ...
  private void reloadCoreAndRecreateIndex() throws InterruptedException {
  ...
  private void doTestCompressionMode(String propertyValue, String expectedModeString) throws IOException, InterruptedException {
  ...
  public void testCompressionModeDefault()
    ...
    assertFalse("Unexpected configuration of codec factory for this test. Expecting empty element",
        config.getNode("codecFactory", false).hasChildNodes());
    ...
    c.start();
    assertNull(coreContainer.registerCore(cd, c, true));

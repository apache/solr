...
public class TestCoreContainer extends SolrTestCaseJ4 {
  ...
  @BeforeClass
  public static void beforeTestCoreContainer() throws Exception {
  ...
  @AfterClass
  public static void afterTestCoreContainer() {
    if (oldSolrHome != null) {
  ...
  private CoreContainer init(SolrResourceLoader loader, Path homeDirectory, String xml) throws Exception {
    CoreContainer ret = new CoreContainer(new SolrXmlConfig().fromString(homeDirectory, xml));
    ret.load();
    return ret;
  }
  ...
  public void testSharedLib() throws Exception {
    ...
    final CoreContainer cc2 = init(loader, tmpRoot, "<solr><str name=\"sharedLib\">lib</str></solr>");
    try {
    ...
    final CoreContainer cc3 = init(loader, tmpRoot, "<solr><str name=\"sharedLib\">customLib</str></solr>");
    try {
    ...
    final CoreContainer cc4 = init(loader, tmpRoot, "<solr><str name=\"sharedLib\">customLib, customLib2</str></solr>");
    try {
  ...
  public void testCoreInitFailuresOnReload() throws Exception {
    ...
    // Fork: XmlConfigFile does not set systemId on the input stream, so getSystemId() would be null.
    // The SAXParseException itself confirms the corrupt solrconfig was correctly rejected.
    ...
    // Fork: XmlConfigFile does not set systemId on input stream; verify it's a parse error via the exception type.
    // (SAXParseException.getSystemId() is null in this fork, so we don't assert on the file name.)

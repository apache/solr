...
public class PluginBagTest extends SolrTestCaseJ4 {
    ...
    public void testOnlyInitsJerseyIfHoldingRequestHandlers() {
        final PluginBag<SearchComponent> nonRequestHandlerBag = new PluginBag<>(SearchComponent.class, null);
        assertNull("Jersey app should not be created for plugin bags that aren't managing RequestHandler's",
                nonRequestHandlerBag.getJerseyEndpoints());

        final PluginBag<SolrRequestHandler> handlerPluginBag = new PluginBag<>(SolrRequestHandler.class, null);
        assertNotNull("Jersey app should be created for plugin bags that manage RequestHandlers",
                handlerPluginBag.getJerseyEndpoints());
    }

    @Test
    public void testCreatesCoreSpecificJerseyAppIfCoreProvided() {
        final PluginBag<SolrRequestHandler> handlerPluginBag = new PluginBag<>(SolrRequestHandler.class, solrCore);
        assertEquals(JerseyApplications.SolrCoreApp.class, handlerPluginBag.getJerseyEndpoints().getClass());
    }

    @Test
    public void testCreatesContainerSpecificJerseyAppIfNoCoreProvided() {
        final PluginBag<SolrRequestHandler> handlerPluginBag = new PluginBag<>(SolrRequestHandler.class, null);
        assertEquals(JerseyApplications.CoreContainerApp.class, handlerPluginBag.getJerseyEndpoints().getClass());
    }

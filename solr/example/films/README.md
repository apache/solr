 ...
 * `film_vector` - The 10 dimensional vector representing the film, according to a toy example embedding model
 ...
 The `film_vector` is an embedding vector created to represent the movie with 10 dimensions. The vector is created by combining the first 5 dimensions of a pre-trained BERT sentence model applied on the name of the movies plus the name of the genres, followed by an item2vec 5-dimensions model of itemset co-ocurrence of genres in the movies, totalling 10 dimensions. Even tough it is expected that similar movies will be close to each other, this model is just a "toy example", so it's not guaranteed to be a good representation for the movies. 

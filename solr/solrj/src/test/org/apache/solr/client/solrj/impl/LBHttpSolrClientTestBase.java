 ...
 */
public abstract class LBHttpSolrClientTestBase<C extends HttpSolrClientBase> extends SolrTestCase {
     ...
     */
    public void testWithTheseParamNamesInTheUrl(String url, Set<String> urlParamNames, C delegate) {
        try (var h = buildLoadBalancingClient(delegate, new LBSolrClient.Endpoint(url))) {

            assertArrayEquals(
                    "Wrong urlParamNames found in lb client.",
                    urlParamNames.toArray(),
                    h.lbClient.getUrlParamNames().toArray());
            assertArrayEquals(
                    "Wrong urlParamNames found in base client.",
                    urlParamNames.toArray(),
                    h.delegate.getUrlParamNames().toArray());
        }
    }
    ...
    public void testSynchronous() throws Exception {
        ...
        try (MockHttp2SolrClient client = new MockHttp2SolrClient("http://base.url", b);
            ...
            for (int i = 0; i < 10; i++) {
                ...
                String expectedEndpoint = ep1.toString().equals(lastEndpoint) ? ep2.toString() : ep1.toString();
                assertEquals("There should be round-robin load balancing.", expectedEndpoint, response.server);
    ...
    public void testSynchronousWihFalures() throws Exception {
        ...
        try (MockHttp2SolrClient client = new MockHttp2SolrClient("http://base.url", b);
            ...
            for (int i = 0; i < 5; i++) {
                LBSolrClient.Req req = new LBSolrClient.Req(new QueryRequest(new MapSolrParams(Map.of("q", qValue))), endpointList);
                LBSolrClient.Rsp response = testClient.request(req);
                assertEquals("The healthy node 'endpoint two' should have served the request: " + i, basePathToSucceed, response.server);
                checkSynchonousResponseContent(response, qValue);
            }
            ...
            for (int i = 0; i < 5; i++) {
                LBSolrClient.Req req = new LBSolrClient.Req(new QueryRequest(new MapSolrParams(Map.of("q", qValue))), endpointList);
                LBSolrClient.Rsp response = testClient.request(req);
                assertEquals("The healthy node 'endpoint one' should have served the request: " + i, basePathToSucceed, response.server);
                checkSynchonousResponseContent(response, qValue);
            }
        }
    }
    ...
    private void checkSynchonousResponseContent(LBSolrClient.Rsp response, String qValue) {
        ...
        assertEquals("The response key 'response' should echo the query.", qValue, response.getResponse().get("response"));
    }
    ...
    protected abstract LBClientHolder buildLoadBalancingClient(C delegate, LBSolrClient.Endpoint... endpoints) ;

    protected static class LBClientHolder implements AutoCloseable {

        final LBHttpSolrClientBase<?> lbClient;
        final HttpSolrClientBase delegate;

        LBClientHolder(LBHttpSolrClientBase<?> lbClient, HttpSolrClientBase delegate) {

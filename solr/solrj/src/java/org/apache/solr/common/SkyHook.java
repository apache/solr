...
public class SkyHook {
  ...
  public static void register(SolrDocumentBase document) {
    String sId;
    try {
  ...
  private static void reg(String sId, String line) {

    if (Integer.parseInt(sId) > 400) {
      log.info("found high one {}", sId);
    }

    log.info("{} docid={} {}", cnt.incrementAndGet(), sId, line);
  ...
  public static void register(String sId, String additional) {
    try {
      ...
      StringBuilder sb = ObjectReleaseTrackerTestImpl.getThreadLocalStringBuilder();
      sb.setLength(0);

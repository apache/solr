...
public class SolrQueuedThreadPool extends ContainerLifeCycle implements ThreadFactory, ThreadPool.SizedThreadPool, Dumpable, TryExecutor, Closeable {
  ...
  private static final AtomicInteger NAME_CNT = new AtomicInteger();
  ...
  public SolrQueuedThreadPool(String name) {
    this(name, Integer.MAX_VALUE, Integer.getInteger("solr.minContainerThreads", 250), Integer.getInteger("solr.containerThreadsIdleTimeout", 600000), -1, null, -1, null,
  ...
  public SolrQueuedThreadPool(String name, @Name("maxThreads") int maxThreads, @Name("minThreads") int minThreads, @Name("idleTimeout") int idleTimeout, @Name("queue") BlockingQueue<Runnable> queue,
    ...
    this(name, maxThreads, minThreads, idleTimeout, 0, queue, queueOfferTimeout, threadGroup);
  }
  ...
  private SolrQueuedThreadPool(String name, @Name("maxThreads") int maxThreads, @Name("minThreads") int minThreads, @Name("idleTimeout") int idleTimeout, @Name("reservedThreads") int reservedThreads,
    ...
    this._name = name + '-' + NAME_CNT.incrementAndGet();
    ...
    if (queue == null) {
      queue = new BlockingArrayQueue<>(128, 64);
    }
    _jobs = queue;
  ...
  public SolrQueuedThreadPool(String name, StackTraceElement[] stackTrace) {
    this(name);
  ...
  public boolean isLowOnThreads() {
    return _maxThreads - getThreads() + getIdleThreads() - getQueueSize() <= _lowThreadsThreshold;
  }
  ...
  protected void startThread() {
    log.trace("Start a thread {}", _name);
    ...
    try {
      ...
      Future future = ParWork.submit(_name + (initStackTrace == null ? "" : "\n" + printStack(initStackTrace)), runnable);
      started = true;
    } finally {
  ...
  private static String printStack(StackTraceElement[] initStackTrace) {
    StringBuilder sb = new StringBuilder(128);
    sb.setLength(0);
    StringBuilderWriter sw = new StringBuilderWriter(sb);
    PrintWriter pw = new PrintWriter(sw);
    printStackTrace(initStackTrace, pw, 12);
    String stack = sw.toString().replaceAll("org\\.apache\\.solr", "o.a.s");
    return stack;
  }
  ...
  public static Runnable newRunnable(Runnable runnable) {
    ThreadGroup group;
  ...
  public String toString() {
    ...
    return String.format(Locale.ROOT, "%s[%s]@%x{%s,%d<=%d<=%d,i=%d,r=%d,q=%d}[%s]", getClass().getSimpleName(), _name, hashCode(), getState(), _minThreads, threads,
        _maxThreads, idle,
...
//      }
  ...
  public void close() {
    try {
      ...
      BlockingQueue<Runnable> jobs = _jobs;

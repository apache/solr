...
public class ParallelStream extends CloudSolrStream implements Expressible {
  ...
  /**
   * Sets the {@link StreamFactory} used to serialize the wrapped stream to an expression for the
   * worker nodes. Callers that build a ParallelStream programmatically (e.g. the SQL handler) must
   * supply a factory that knows every function in the wrapped stream; otherwise the factory is
   * taken from the StreamContext in {@link #setStreamContext}.
   */
  ...
  protected void constructStreams() throws IOException {
    // Validate that partitionKeys is set when numWorkers > 1
    if (workers > 1 && tupleStream instanceof CloudSolrStream) {
      ModifiableSolrParams innerParams = ((CloudSolrStream) tupleStream).params;
      if (innerParams == null || innerParams.get("partitionKeys") == null) {
    ...
    try {
      ...
      for(int w=0; w<workers; w++) {
        ...
        // When there are more workers than shard URLs, wrap around so multiple workers share a
        // shard URL. Each worker still reads a distinct slice of the data via its workerID and the
        // {!hash} partitionKeys filter, so this is correct and avoids an IndexOutOfBounds when
        // workers > number of shards (e.g. a 2-worker parallel() over a single-shard collection).
        String url = shardUrls.get(w % shardUrls.size());

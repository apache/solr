...
public class SolrStream extends TupleStream {
  ...
  **/
  ...
  public void open() throws IOException {
    ...
    try {
      ModifiableSolrParams requestParams = loadParams(params);
      if (!distrib) {
        requestParams.add("distrib","false");
      }
      assert tupleStreamParser == null;
      tupleStreamParser = constructParser(client, requestParams);
    } catch (Exception e) {
      ParWork.propagateInterrupt(e);
      // Include the root cause message so that exception propagation through ExceptionStream
      // preserves the original error (e.g. "sort param field can't be found: blah").
      Throwable root = e;
      while (root.getCause() != null) root = root.getCause();
      String rootMsg = root.getMessage();
      String msg = rootMsg != null && !rootMsg.equals(e.getMessage())
          ? "params " + params + ": " + rootMsg
          : "params " + params;
  ...
  **/
  ...
  private static Map mapFields(Map fields, Map<String,String> mappings) {
  ...
  public TupleStreamParser constructParser(SolrClient server, SolrParams requestParams) throws IOException, SolrServerException {
    ...
    // Determine the response writer type (wt). The fork's /stream handler always serializes the
    // result-set as JSON (the javabin writer is not reliably selected for /stream over a POST body
    // for HTTP/2 requests), so /stream is always read as JSON even if a stray wt=javabin was supplied
    // (e.g. randomly injected by test helpers). The /export handler always uses javabin explicitly.
    String wt = requestParams.get(CommonParams.WT);
    if (p != null && p.contains("stream")) {
      wt = "json";
    } else if (wt == null) {
      wt = CommonParams.JAVABIN;
    }
    ((ModifiableSolrParams) requestParams).set(CommonParams.WT, wt);

 ...
 * with the connection.
 *
 * <p>Crucially, a stall is only ever reported when the queue is NOT empty: a runner that has
 * drained the queue but is still finishing its in-flight request is making legitimate progress, not
 * stalling. The old queue-size-only heuristic produced false-positive stalls in that case (a queue
 * size that stayed constant at zero was reported as stalled), which is what this class fixes
 * (SOLR-17294).
 */

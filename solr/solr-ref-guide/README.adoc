...
One directory is an exception: `modules/deployment-guide/pages/cli/` holds the `bin/solr` command reference pages,
which are generated from the picocli annotations in the Java source by `./gradlew :solr:solr-ref-guide:generateCliDocs`.
Every `solr-*.adoc` file there is generated and must not be edited by hand; edit the annotations and regenerate.
Only `index.adoc` in that directory is hand-written.
See `../../dev-docs/ref-guide/antora.adoc` for details.

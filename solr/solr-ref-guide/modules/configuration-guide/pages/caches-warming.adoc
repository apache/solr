...
The setting aims to govern all prefix-based queries (e.g. `val_s:a*`, `{!prefix f=val_s}a`, `{!complexphrase}val_s:"a*"`) that target `string` or `text` type fields.

In the default configset, the minimum-prefix is set to '2' (or the value of the `solr.min.prefixLength` system property if specified).
If not specified in `solrconfig.xml`, no minimum-length is enforced.

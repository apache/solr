...
[source,bash]
----
curl http://localhost:8983/api/collections/collection1/shards -H 'Content-Type: application/json' -d '
  {
    "split": {
      "shard": "shard1",
      "async": "1000"
...
|===
+
v1 only. Set to `true` to clear all stored completed and failed async request responses.
The v2 API uses `DELETE /api/cluster/commands` for the same operation and does not take this parameter.

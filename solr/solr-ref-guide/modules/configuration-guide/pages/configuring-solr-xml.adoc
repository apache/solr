...
=== The <clusterPluginsSource> Element
The `<clusterPluginsSource>` element allows you to specify the class used to load cluster plugins.

[source,xml]
----
<clusterPluginsSource class="org.apache.solr.api.ZkClusterPluginsSource"/>
----

The default cluster plugin source is `ZkClusterPluginsSource`, which allows the cluster plugins API to be used to maintain plugins.

The `org.apache.solr.api.NodeConfigClusterPluginsSource` is also provided by solr, allowing cluster plugins to be declared in `solr.xml`.
If this implementation is used, the `/custer/plugin` API can't be used to add, update or remove cluster plugins.

The `NodeConfigClusterPluginsSource` only supports declaring `clusterSingleton` plugins in `solr.xml` at this time.


=== The <clusterSingleton> Element
If the `clusterPluginsSource` is set to `NodeConfigClusterPluginsSource` then one or more `clusterSingleton` elements can be declared.

...
<solr>
  ...
  <metrics enable="${metricsEnabled:true}"/>
...
The `<metrics>` element in `solr.xml` allows you to enable/disable metrics collected by Solr.

If you would like to customize metrics for your installation, see the xref:deployment-guide:metrics-reporting.adoc#metrics-configuration[Metrics Configuration] section.

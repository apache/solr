...
<solr>
  ...
  <solrcloud>
    ...
    <str name="stateCompressionProvider">${stateCompressionProvider:org.apache.solr.common.util.ZLibStateCompressionProvider}</str>
...
[%autowidth,frame=none]
|===
|Optional |Default:org.apache.solr.common.util.ZLibStateCompressionProvider
|===
+
Optional parameter to provide a compression implementation for state.json over the wire and stored in Zookeeper. The value provided is the class to use for state compression. This is only used if minimumStateSizeForCompression is set to a value above -1.

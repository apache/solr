...
<solr>
  ...
  <str name="allowZkHosts">${solr.allowZkHosts:}</str>
...
|===
+
Comma-separated list of ZooKeeper connection strings of other SolrCloud clusters that may be passed as a `zkHost` parameter to features that fetch data from another cluster, such as cross-collection join and streaming expressions.
...
+
The allow-list can also be configured with the `solr.allowZkHosts` system property in `solr.in.sh` / `solr.in.cmd`.

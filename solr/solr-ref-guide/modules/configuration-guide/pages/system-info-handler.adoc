...
"System"-level information for individual nodes can be fetched with the API below:
...
The `nodeInfo` object appears only in the V2 API response.  In the V1 API response, the keys listed below are available at the top level.

The keys in the result are:

`mode`::
Either `solrcloud` or `std`, indicating the mode Solr is running in standalone mode.

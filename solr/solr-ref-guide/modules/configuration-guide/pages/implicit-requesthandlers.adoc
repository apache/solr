...
NDJSON Updates:: Add newline delimited JSON (ND-JSON / JSON Lines) documents.
+
*Documentation*: xref:indexing-guide:indexing-with-update-handlers.adoc#newline-delimited-json[Newline Delimited JSON]
+
[cols="3*.",frame=none,grid=cols,options="header"]
|===
|API Endpoint |Class & Javadocs |Paramset
|`solr/<collection>/update/ndjson` |{solr-javadocs}/core/org/apache/solr/handler/UpdateRequestHandler.html[UpdateRequestHandler] |`_UPDATE_NDJSON`
|===

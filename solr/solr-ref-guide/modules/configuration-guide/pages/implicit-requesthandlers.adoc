...
v2: `api/node/properties` |v1: {solr-javadocs}/core/org/apache/solr/handler/admin/PropertiesRequestHandler.html[PropertiesRequestHandler]

v2: {solr-javadocs}/core/org/apache/solr/handler/admin/api/GetNodeProperties.html[GetNodeProperties] |`_ADMIN_PROPERTIES`
|===
+
...
Abbreviated JSON response with the default `json.nl=flat` format.
The thread dump alternates `"thread"` keys and thread objects:
...
----
{
  "system": {
    "threadDump": [
      ...
      "thread", {"id": 2, "name": "worker", "state": "WAITING"}
    ]
  }
}
----
...
The thread dump is an array of objects, each containing a `"thread"` property:
...
======
+
[cols="2*.",frame=none,grid=cols,options="header"]
|===
|Class & Javadocs |Paramset

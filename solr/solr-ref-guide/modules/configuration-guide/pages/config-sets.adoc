...
To upload a file to a configset already stored on ZooKeeper, you can use xref:deployment-guide:solr-control-script-reference.adoc#copy-between-local-files-and-zookeeper-znodes[`bin/solr zk cp`] or the xref:configsets-api.adoc#configsets-upload-file[`API`].
...
[source,bash]
----
curl -v 'http://localhost:8983/api/cores' -H 'Content-type: application/json' -d '{
  "create":[{
    "name": "mycore",

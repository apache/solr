...
|solr.admin.handler.systeminfo.dns.prevent.reverse.lookup.enabled|solr.dns.prevent.reverse.lookup|false|Controls whether reverse DNS lookups are prevented in the system info handler.
...
|solr.circuitbreaker.errorcode|solr.circuitbreaker.errorcode|503|Defines the HTTP error code returned when a circuit breaker is triggered.
...
|solr.configset.create.auth.checks.disabled|solr.disableConfigSetsCreateAuthChecks|false|Controls whether authentication checks are disabled when creating configsets.
...
|solr.index.replication.fingerprint.disabled|solr.disableFingerprint|false|Controls whether index fingerprinting for replication is disabled.
...
|solr.metrics.fieldcache.entries.disabled|disableSolrFieldCacheMBeanEntryList|false|Controls whether Solr FieldCache MBean entry list is disabled for metrics reporting.
...
|solr.tracing.requestid.disabled|solr.disableRequestId|false|Controls whether request ID generation is disabled.

...
Such an index may not fit in the memory available for the OS page cache, in which case files compete for finite space and preloading one that contributes little to query latency evicts one that matters.
`preloadExtensions` restricts preloading to the file extensions you name, prioritizing the files with the most impact on performance:

[source,xml]
----
<directoryFactory name="DirectoryFactory"
                  class="solr.MMapDirectoryFactory">
  <str name="preloadExtensions">vex,vec</str>
</directoryFactory>
----
...
Both parameters are honored by `solr.MMapDirectoryFactory`, `solr.StandardDirectoryFactory`, and the default `solr.NRTCachingDirectoryFactory`.
They have no effect when the underlying directory is not memory mapped, as with `solr.NIOFSDirectoryFactory`.

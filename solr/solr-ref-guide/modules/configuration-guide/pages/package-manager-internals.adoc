...
Start all your nodes with the system property `-Dsolr.packages.enabled=true` to use this feature.
...
* `GET /api/cluster/packages` Get the list of all packages
* `GET /api/cluster/packages/{name}` Get the details of a specific package
* `POST /api/cluster/packages/{name}/versions` Add a version of a package
* `DELETE /api/cluster/packages/{name}/versions/{version}` Delete a specific version of a package
* `POST /api/cluster/packages/{name}/refresh` Refresh a package on all nodes in the cluster

=== How to Upgrade?

Use the add version endpoint to add a version that is higher than the current version.

=== How to Downgrade?

Use the delete version endpoint to delete the highest version and choose the next highest version.
...
[source,bash]
----
curl -X POST http://localhost:8983/api/cluster/packages/mypkg/versions \
  -H 'Content-type:application/json' \
  -d '{"version":"1.0","files":["/mypkg/1.0/myplugins.jar"]}'
----
...
[source,bash]
----
curl http://localhost:8983/api/cluster/packages?omitHeader=true
----
+
[source,json]
----
...
curl -X POST http://localhost:8983/api/cluster/packages/mypkg/versions \
  -H 'Content-type:application/json' \
  -d '{"version":"2.0","files":["/mypkg/2.0/myplugins.jar"]}'
----

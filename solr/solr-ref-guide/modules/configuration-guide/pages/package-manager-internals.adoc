...
* `GET /api/cluster/package` Get the list of all packages
* `GET /api/cluster/package/{name}` Get the details of a specific package
* `POST /api/cluster/package/{name}/versions` Add a version of a package
* `DELETE /api/cluster/package/{name}/versions/{version}` Delete a specific version of a package
* `POST /api/cluster/package/{name}/refresh` Refresh a package on all nodes in the cluster
...
[source,bash]
----
curl -X POST http://localhost:8983/api/cluster/package/mypkg/versions \

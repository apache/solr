...
* FLUSH: We only flush explicitly the Solr internal FastOutputStream buffer to the underlying OutputStream, but this OutputStream is not explicitly flushed to the tlog file. This is less expensive but also less safe since if we have a crash before the OutputStream is also flushed, data from the buffer is lost.

...
This action is expensive and is intended for installations running Solr in user-managed (or standalone) mode only. It is currently not supported in SolrCloud mode.
...
[NOTE]
====
Indexes containing nested documents (parent-child document hierarchies) are not supported by this action.
If nested documents are detected in the index, the operation will fail with an error.
====

...
The `/export` handler offers several advantages over xref:pagination-of-results.adoc#fetching-a-large-number-of-sorted-results-cursors[cursor-based pagination] for streaming large result sets.
...
Cursors impose no constraints on the sort criteria accept that you must include a unique key, which isn't a real constraint.
...
If all requested fields are `docValues=true` then the data will be read only from docValues. This behavior applies to fields that are also `stored=true` and does not depend on the value of the `includeStoredFields` parameter.

...
Only after receiving this large payload can the client request the next batch, but in the interim Solr sits idle on this request.
...
Fields that have both docValues and stored values will always use docValues for optimal performance, regardless of this parameter setting.

...
There are serious performance implications that should be diligently examined before employing this component in a serious way.
...
Vectorising text using a hosted service may be slow so depending on your use case it could be a good idea to index first yur documents and then add vectors iteratively.
...
This has data traffic (you transfer your documents over the network twice) and processing (if you have other update request processors in your chain, those must be repeated the second time as we are literally replacing the indexed documents one by one) implications
====

...
[NOTE]
====
Partial updates are processed by the `DistributedUpdateProcessorFactory`. For this reason, it must be placed at the beginning of the UpdateRequestProcessorChain, before the `TextToVectorUpdateProcessorFactory`.
====

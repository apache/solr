...
[source,xml]
----
<queryParser name="text_to_vector" class="org.apache.solr.llm.texttovector.search.TextToVectorQParserPlugin"/>
----

== Text Vectorisation Lifecycle
...
*N.B.* the Solr vectorisation model specifies the parameters to access the APIs, the model doesn't run internally in Solr

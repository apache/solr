...
[NOTE]
====
Since Partial updates are processed by the `DistributedUpdateProcessorFactory`, this processor must be placed at the beginning of the UpdateRequestProcessorChain, before the `TextToVectorUpdateProcessorFactory`.
====

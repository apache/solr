...
|===
+
The UH scores each passage to select the best `hl.snippets` candidates; this option then controls the order in which those selected passages are returned.
The choices are `startOffset` (the passage's position in the field, the default), `endOffset`, and `score` (highest-scoring passage first).

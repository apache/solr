...
|===
+
The UH calculates a score for each passage, and it uses it to pick the best snippets defined by `hl.snippets`,
but the final sorting of these selected candidates is determined by this option.
The available choices are: `startOffset`, `endOffset`, and `score`.
+
The default value is `startOffset`, so even if a candidate has a lower score than others, it may rank higher if it appears earlier.
If this behavior does not align with the user’s intention, consider using `score` for sorting.

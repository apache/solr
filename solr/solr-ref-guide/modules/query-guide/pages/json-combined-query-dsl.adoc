...
[NOTE]
====
Consider setting `shards.rows` to at least your deepest expected `offset + limit` to enlarge the per-shard candidate pool used for fusion, giving more consistent ordering across pages.
====

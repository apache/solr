...
`combiner.method` | Default: `post`::
   There are two ways of combining the docs in case of sharded collection:
  - pre - The queries are executed at the shard local and combined there itself using the algorithm defined, returning the combined list to coordinator which finally merges the results basis score.
  - post - The queries are executed at the shard local and return the documents with original score to the coordinator to merge per query per shard responses basis the scores. Finally, each query results are combined using the algorithm defined.

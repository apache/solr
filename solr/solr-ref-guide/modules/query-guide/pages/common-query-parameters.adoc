...
If set to `false` or `true` then the response may contain a diagnostic `multiThreadedUsed` element; however the element will be absent if cached results were returned.

[source,json]
----
{
  "responseHeader": {
    "multiThreadedUsed": true,
    "status": 0,
    "QTime": 3,
    "params": {
      "q": "hello",

...
`auxIndexJoin` is completely experimental that's why it requires explicit configuration. So, far it's evalueated within single relation (from-to index pair) only, but it should support many ones in principle. It even might not guarantee compatibility with future versions.

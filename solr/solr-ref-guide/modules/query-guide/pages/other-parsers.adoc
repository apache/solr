...
|===
+
A label that tags every document matched by this query.
When xref:matched-queries-component.adoc[] is active (`mq=true`), the label appears in `matched_queries_per_hit` and `matched_queries_summary` in the response.
Inner sub-queries may carry their own `_name` labels independently.

...
Invoked with the syntax `{!intervals df=<field>}$<name>`, where `$<name>` is a required reference to an entry in the `json_queries` map supplied in the JSON request body.

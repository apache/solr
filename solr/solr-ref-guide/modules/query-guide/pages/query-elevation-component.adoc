...
If you want elevated documents to be included in the result set whether or not they match the provided filter queries, you can supply the request parameter `elevateFilteredDocs=true` (the default is `false`).
This option modifies the filter queries to include the elevated documents.
This option may have a performance impact when filter caching is enabled and the modified filters are not found in the cache.
...
With `logAll = true` all the features from the feature store are printed.
With `logAll = false` only the features used by the model are printed.

Below the tables showing the different configuration possibilities and the corresponding results in both no re-ranking
and re-ranking scenarios.

NO RE-RANKING
[cols=",,,"]
|===
2.2+h| 2+h|Logger store
h|Default store h|Defined store
.2+h|logAll h|Default |All the features logged |All the features logged
h|True |All the features logged |All the features logged
|===
...
|===
2.2+h| 2+h|Logger store
h|Default store h|Model store
.3+h|logAll h|Default |Model features logged |Model features logged
h|True |All the features of the model store logged |All the features logged
h|False |Model features logged |Model features logged
|===

[cols=",,"]
|===
2.2+h| h|Logger store
h|Different from model store
.2+h|logAll h|Default |All the features logged
h|True |All the features logged
|===

...
[NOTE]
Lucene91HnswVectorsFormat is not back-compatible with Lucene90HnswVectorsFormat which is no longer supported. Reindex is necessary when upgrading from Solr 9.0
...
|===
+
(advanced) This parameter is specific for the `Lucene91HnswVectorsFormat` codec format:
+

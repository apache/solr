...
The knn search returns the top-k nearest children documents that satify the filter on the parent.
+
...
[source,text]
?q={!knn f=vector topK=3 childrenOf=$someParents allParents=$allParents}[1.0, 2.0, 3.0, 4.0]
...
|===
|Optional |Default: none
|Mandatory if using 'childrenOf' parameter|Default: none
|===
+
A query that matches ALL parents.
It's required to work with the 'childrenOf' parameter.


Here is an example of a `knn` search using a `allParents`:

[source,text]
?q={!knn f=vector topK=3 allParents=$allParents}[1.0, 2.0, 3.0, 4.0]
&allParents=*:* -_nest_path_:*

The search results retrieved are the k=3 nearest documents to the vector in input `[1.0, 2.0, 3.0, 4.0]`, each of them with a different parent. The 'allParents' parameter must return all parents to guarantee the correct functioning of the query.
...
* `hnswEfConstruction` - hnswEfConstruction parameter passed to the fallback Lucene99HnswVectorsWriter

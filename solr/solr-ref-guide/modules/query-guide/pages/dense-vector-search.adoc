...
|===
+
Early termination is an HNSW optimization. Solr relies on Lucene’s implementation of early termination for kNN queries, provided by
https://lucene.apache.org/core/10_2_2/core/org/apache/lucene/search/PatienceKnnVectorQuery.html[org.apache.lucene.search.PatienceKnnVectorQuery].
+
When enabled (true), the search may exit early when the HNSW candidate queue remains saturated over a threshold (saturationThreshold) for more than a given number of iterations (patience).
+
...
[%autowidth,frame=none]
|===
|Optional |Default: `0.995d`
|===
+
(advanced) The early exit saturation threshold.
+
Accepted values: a double between 0.0 and 1.0 (exclusive).
+
...
|===
+
(advanced) The number of consecutive iterations the search will continue after the candidate queue is considered saturated.
+
Accepted values: any integer values > 7. The default is not a fixed value but a formula based on the topK parameter.
+
...
Here's an example of a `knn` search using the early termination with custom parameters:

...
`NOTE:` Because binary quantization is particularly lossy, it is recommended to use this in conjunction with `rerankOversample` param to improve recall
...
Recommended for use only with quantized dense vector types, particularly binary quantized.
+
Accepted values:
Any int >= 1

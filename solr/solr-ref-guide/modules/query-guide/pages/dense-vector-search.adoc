...
NOTE: `vectorEncoding="BYTE"` is not supported for `ScalarQuantizedDenseVectorField`. Scalar quantization operates on `FLOAT32` vectors.
...
The flat algorithm is supported by both `DenseVectorField` and `ScalarQuantizedDenseVectorField`. When used with `ScalarQuantizedDenseVectorField`, vectors are still quantized at index time.
...
For `ScalarQuantizedDenseVectorField`, flat fields also support the `{!knn}`, `{!knn_text_to_vector}`, and `{!vectorSimilarity}` query parsers, which score against the quantized vectors.

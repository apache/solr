...
Flat fields do not support the `{!knn}` query parser. Use `vectorSimilarity()` function queries to score and rank by similarity:

[source]
fl=id,sim:vectorSimilarity(vector,[1.0, 2.0, 3.0, 4.0])&sort=vectorSimilarity(vector,[1.0, 2.0, 3.0, 4.0]) desc

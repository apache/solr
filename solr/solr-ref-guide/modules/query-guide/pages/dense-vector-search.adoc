...
|===
+
A query seed to initiate the vector search, i.e. entry points in the HNSW graph exploration. Solr relies on the Lucene’s implementation of SeededKnnVectorQuery based on https://arxiv.org/pdf/2307.16779[Lexically-Accelerated Dense Retrieval (2023)].
+
...
[NOTE]
Currently seed and earlyTermination parameters (including saturationThreshold and patience) cannot be used together.
Solr must be upgraded to a Lucene version that includes this patch https://github.com/apache/lucene/pull/14688[Lucene #14688].

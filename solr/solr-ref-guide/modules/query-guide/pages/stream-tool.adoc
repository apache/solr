...
The Stream Tool allows you to specify `--execution local` to process the expression in the tools' JVM.
...
[,console]
----
$ bin/solr stream --execution local load_data.expr
----

Caveats:
 * You don't get to use any of the parallelization support locally.

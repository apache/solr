= Solr CLI Stream Command
:page-aliases: stream-tool.adoc
...
The Stream command allows you to run a xref:streaming-expressions.adoc[] in Solr and see the results from the command line.
...
NOTE: The Stream command is classified as "experimental".
...
== Using the bin/solr stream Command
...
List of options:
 ...
 -sc,--solr-connection <HOST>       Zookeeper or HTTP(S) connection string; unnecessary if SOLR_CONNECTION is defined in solr.in.sh;
 ...
 -z,--zk-host <HOST>                Zookeeper connection string; unnecessary if ZK_HOST is defined in solr.in.sh;
                                    otherwise, defaults to localhost:9983.
                                    Deprecated: use --solr-connection option
----
...
The Stream command allows you to specify `--execution local` to process the expression in the Solr CLI's JVM.
...
The Stream command adds some Streaming Expressions specifically for local use:

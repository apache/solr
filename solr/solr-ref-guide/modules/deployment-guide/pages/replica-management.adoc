...
Fetch the replicas of a collection shard.

This is a v2-only API. The closest v1 equivalent is xref:cluster-node-management.adoc#clusterstatus[CLUSTERSTATUS].

[tabs#listreplicas-request]
======
V2 API::
+
====
[source,bash]
----
curl -X GET http://localhost:8983/api/collections/techproducts/shards/shard1/replicas
----
====
======
...
The response includes a `replicas` object keyed by replica name (`core_nodeN`). Each entry uses the same replica fields CLUSTERSTATUS already returns. SolrJ provides `ReplicasApi.ListReplicas` for this request.

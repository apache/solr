...
An operator can make the cluster-wide default `true` without changing any client by setting the system property `solr.cloud.waitForFinalState.enabled=true` on each Solr node.
Unlike CREATE, CREATESHARD and SPLITSHARD -- whose replicas start empty -- a replica added here must recover from its leader, and that recovery has no bounded duration, so this command keeps the pre-SOLR-18367 default.
...
If `false`, the API will return the status of the single action, which may be before the new replica is online and active.
An operator can make the cluster-wide default `true` without changing any client by setting the system property `solr.cloud.waitForFinalState.enabled=true` on each Solr node.
Because the move runs ADDREPLICA on the destination, the new replica must recover from its leader, and that recovery has no bounded duration, so this command keeps the pre-SOLR-18367 default.

...
If the path is not absolute then given `location` will be computed relative to the xref:backup-restore.adoc#backuprestore-storage-repositories[Backup Repository's] default `location`.
+

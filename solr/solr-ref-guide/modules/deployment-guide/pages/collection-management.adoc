...
An operator can revert the cluster-wide default back to `false` without changing any client by setting the system property `solr.cloud.waitForFinalState.enabled=false` on each Solr node.

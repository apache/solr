...
+
The CrossDC Solr Module and CrossDC Manager bundle Kafka client library version 4.3.1.
Kafka 4.x clients are fully compatible with Kafka 3.x brokers, but only partially compatible with brokers in the 2.1-2.8 range, and are not compatible with brokers older than 2.1.
...
Additional configuration specific to the Kafka client library can be passed through using either
environment variables starting with `SOLR_CROSSDC_KAFKA_` or system properties starting with

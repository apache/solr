...
These gauges are registered under the `QUERY` category in the node metrics registry:

[cols="2,1,3",options="header"]
|===
| Metric Name Suffix | Type | Description
| `asyncPermits.max` | gauge | Configured maximum number of outstanding concurrent async HTTP requests (controlled by `solr.solrj.http.jetty.async_requests.max`, default 1000).
| `asyncPermits.available` | gauge | Number of async request permits currently available (i.e., not in use). When this approaches zero, new distributed requests will block waiting for a permit.
|===

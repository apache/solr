...
The `<metrics>` element in `solr.xml` supports one attribute `enable`, which takes a boolean value,
for example `<metrics enable="true">`.

The default value of this attribute is `true`, meaning that metrics are being collected, processed and available from the metrics APIs and/or OTLP.

The `false` value of this attribute (`<metrics enable="false">`) turns off metrics collection and processing.
Internally, all metrics are replaced by OpenTelemetry no-op
implementations, which effectively removes nearly all overheads related to metrics collection.
The metrics APIs stop reporting any metrics and will only return empty responses.

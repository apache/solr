...
Each merge metric is prefixed with `INDEX.merge` followed by components in the metric path indicating if the merge is a `major` or `minor` merge, and
if the metric is updating when the merge `started` or `finished`. The last component of the metric path indicates the property of the merge being counted:

* `merges` - number of total merge operations
* `docs` - number of total documents merged
* `segments` - number of total segments merged
* `deletedDocs` - number of total deleted documents merged

So for example, the metric for "total segments that have finished merging in major merges" would be `INDEX.merge.major.finished.segments`.
If you want to get the numbers for actively running merges, simply take the difference between any corresponding `started` and `finished` metric at any given time.

The default size for a "major" merge operation is 512k documents, but this can be configured to whatever you want as follows
...
Additionally, the following merge metrics are also reported:

* `INDEX.merge.flushes` - Meter for # of flush operations
* `INDEX.merge.errors` - Counter of errored merge operations
* `INDEX.merge.minor.mergeTimes` - Timer of minor merge operations
* `INDEX.merge.major.mergeTimes` - Timer of major merge operations

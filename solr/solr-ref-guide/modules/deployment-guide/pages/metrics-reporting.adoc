...
NOTE: These metrics are available only on HotSpot-based JDK distributions (Oracle JDK, Eclipse Temurin, Amazon Corretto, IBM Semeru, and others). On JVMs that do not provide `com.sun.management.OperatingSystemMXBean`, the metrics are silently absent.

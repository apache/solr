...
[NOTE]
====
For a complete Prometheus + Grafana + Alertmanager setup including a pre-built dashboard and alert rules, see xref:monitoring-with-prometheus-and-grafana.adoc[].
A docker-compose evaluation stack is also available in `solr/monitoring/dev/` in the source release.
====
...
For users who do not use or support pulling metrics in Prometheus format with the `/metrics` API, Solr also supports pushing metrics natively with https://opentelemetry.io/docs/specs/otlp/[OTLP], which is a vendor-agnostic protocol for pushing metrics via gRPC or HTTP.

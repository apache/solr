...
----
When query param disableDistribStats is set to true, distributed stats is turned off at query time to reduce the overhead.

[source,xml]
----
http://localhost:8987/solr/collection1/select?q=*%3A*&wt=json&fq={!terms f=id}id1,id2&disableDistribStats=true
----

...
The JVM's system properties are available from `GET /solr/admin/info/properties` (v1) or `GET /api/node/properties` (v2).
A single property can be requested with v1 `?name=java.version` or v2 `GET /api/node/properties/java.version`.

...
Circuit breakers can be configured globally using environment variables, e.g. in `solr.in.sh`, system properties, or via the v2 Cluster Properties API `set-circuit-breakers`.
...
==== Cluster Properties v2 API
Note: setting any circuit breaker through Cluster Properties will override all global circuit breakers set through environment variables or system properties.
...
|===
+
A map from CircuitBreaker canonical class name (i.e. `org.apache.solr.util.circuitbreaker.LoadAverageCircuitBreaker`) to a <<configs-parameters,CircuitBreaker configuration object>>.
...
|===
+
A map from host name to a <<configs-object, configs object>>. The `hostOverrides` map CircuitBreakers will be merged with any CircuitBreakers of the same class found in `configs`, with the values from this map taking precedence.

==== set-circuit-breaker `configs` Parameters [[configs-parameters]]
...
|===
+
If true, this circuit breaker will trip on its configured query and update thresholds. Otherwise, this circuit breaker will not be registered, and thus will not trip.
...
|===
+
The value above which this circuit breaker will trip for all query requests.
...
|===
+
The value above which this circuit breaker will trip for all update requests.
...
|===
+
If true, this circuit breaker will be registered, but will never trip. When the circuit breaker would have tripped, instead Solr will just log that it would have tripped.

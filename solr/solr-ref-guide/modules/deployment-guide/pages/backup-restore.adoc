...
Can also be set via the `SOLR_BACKUP_MAX_PARALLEL_UPLOADS` environment variable.
...
Can also be set via the `SOLR_BACKUP_MAX_PARALLEL_DOWNLOADS` environment variable.
For cloud storage repositories (S3, GCS), consider setting this to `8` or higher to improve restore performance.

TIP: When using cloud storage, increasing parallelism significantly improves backup and restore speed by overlapping network latency.
However, higher parallelism also increases memory usage.
Start with a value of `8` and adjust based on your available heap memory and network bandwidth.

...
This allow-list is automatically configured for SolrCloud but needs explicit configuration for standalone installations.
...
== Limitations to Standalone Distributed Search
...
When managing a standalone Solr installation, query requests should be load balanced across each of the shard followers.

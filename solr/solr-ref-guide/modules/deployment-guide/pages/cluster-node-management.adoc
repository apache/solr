...
This command can affect an arbitrary number of replicas cluster-wide, so unlike other Collections API commands it keeps the pre-SOLR-18367 default; an operator can opt in cluster-wide without changing any client by setting the system property `solr.cloud.waitForFinalState.enabled=true` on each Solr node.

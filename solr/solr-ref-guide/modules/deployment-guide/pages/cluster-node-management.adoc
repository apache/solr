...
This command can affect an arbitrary number of replicas cluster-wide, so like ADDREPLICA and MOVEREPLICA it keeps the pre-SOLR-18367 default; an operator can opt in cluster-wide without changing any client by setting the system property `solr.cloud.waitForFinalState.enabled=true` on each Solr node.

...
To fetch only shard-level status for one collection, without replica core or segment detail, use xref:shard-management.adoc#listshards[GET /api/collections/_collection-name_/shards]. That response reports the same GREEN / YELLOW / ORANGE / RED signal as `replicaHealth`.

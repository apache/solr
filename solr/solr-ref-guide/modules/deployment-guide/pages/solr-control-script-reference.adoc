...
+
NOTE: The `BasicAuthPlugin` itself defaults `blockUnknown` to `true` when not specified.
However, because `bin/solr auth enable` installs a best-practice template that includes a `RuleBasedAuthorizationPlugin` with fine-grained permissions, the template sets `blockUnknown` to `false`.
This means unauthenticated users can only access endpoints explicitly granted to the `null` role in the authorization configuration — by default these are the *health* and *metrics-read* endpoints, which are left open so that load balancers and monitoring tools can operate without credentials.

...
In order to set custom System Properties when running Solr tools, such as `bin/solr create`, `bin/solr auth` and `bin/solr package` (anything other than `start`, `stop` and `restart`),

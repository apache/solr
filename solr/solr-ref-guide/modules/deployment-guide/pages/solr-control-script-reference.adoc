...
The `create` command creates a Collection or Core depending on whether Solr is running in SolrCloud (collection) or standalone mode (core).
...
The `delete` command detects the mode that Solr is running in and then deletes the specified collection (SolrCloud) or core (standalone) as appropriate.

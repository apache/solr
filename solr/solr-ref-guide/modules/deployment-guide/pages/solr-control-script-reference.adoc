...
[NOTE]
====
This page documents the `bin/solr` script as it behaves by default.

An experimental re-implementation of the command line interface, built on picocli, is also included in this release.
It is disabled by default, and the options it accepts differ in places from those documented here.
See xref:cli/index.adoc[] if you would like to try it.
====

...
<3> A user called 'solr', with a password `'SolrRocks'`, in the encoded format detailed above, has been defined. See below for a bash script on how to calculate the encoded format.
...
The following bash script illustrates how to calculate the encoded password format:
[generate-salt-and-password.sh]
----
#!/bin/bash
...
# Generate a random 32-byte salt and encode it in base64
raw_salt=$(openssl rand -base64 32)
decoded_salt=$(echo -n "$raw_salt" | base64 -d)

# hash password and salt and base64 encode it
hash=$(echo -n "$decoded_salt$password" | openssl dgst -binary -sha256 | openssl dgst -binary -sha256 | base64)

# Format credentials for Solr security.json
echo "{"
echo "  \"credentials\": {"
echo "    \"$username\": \"$hash $raw_salt\","
echo "    ..."
echo "}"
----

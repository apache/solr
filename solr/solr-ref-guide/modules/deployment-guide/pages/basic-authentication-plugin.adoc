...
Usernames and passwords (as a base64 encoded `sha256(sha256(salt+password))` hash) could be added when the file is created, or can be added later with the Authentication API, described below.
...
<3> A user called 'solr', with a password `'SolrRocks'` in the format `base64(<password hash>) base64(<salt>)` has been defined.

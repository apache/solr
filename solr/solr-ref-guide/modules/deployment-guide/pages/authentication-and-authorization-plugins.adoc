...
When running Solr in standalone mode, you create the `security.json` file and put it in the `$SOLR_HOME` directory for your installation (this is the same place you have located `solr.xml` and is usually `server/solr`).

You can use the authentication and authorization APIs, but with a standalone cluster you will need to make the same API requests on each node separately.

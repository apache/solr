...
The ZK connection string must list all nodes, and every node must use `zookeeper_quorum:on`.
Run each command in a separate terminal (or on separate machines, replacing `localhost` with actual hostnames):

[source,bash]
----
export LH="localhost"
ZK="-z $LH:9983,$LH:9984,$LH:9985"
ROLES="-Dsolr.node.roles=data:on,overseer:allowed,zookeeper_quorum:on"

bin/solr start -p 8983 $ZK $ROLES
bin/solr start -p 8984 $ZK $ROLES
bin/solr start -p 8985 $ZK $ROLES
----

...
It can sometimes be noisy when `SolrCore` logs info on every response. Selective logging can be configured on SolrCore by using the `MarkerFilter` on request paths in `log4j2.xml`.

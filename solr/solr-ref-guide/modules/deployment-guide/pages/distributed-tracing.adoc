...
To use gRPC, add the required gRPC, OkHttp, and Protobuf JARs to the `opentelemetry` module's lib directory and set `OTEL_EXPORTER_OTLP_PROTOCOL=grpc`.
Look up the full list of runtime JARs needed at https://central.sonatype.com/artifact/io.opentelemetry/opentelemetry-exporter-sender-okhttp/1.56.0/dependencies[opentelemetry-exporter-sender-okhttp on Maven Central] (replace `1.56.0` with the OpenTelemetry version bundled in your Solr release).

...
The Metrics API provides a Prometheus Response Writer to output Solr metrics natively to be scraped. It is more efficient and drops the need of running the Prometheus Exporter but at the cost of a fixed output and not as flexiable in terms of configurability.  
...
The Prometheus Metrics API does not provide any configurability and the Prometheus output is fixed. Any metrics aggregations and/or filtering must be done on Grafana or the Prometheus server.

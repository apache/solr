...
$ ./bin/solr-exporter -p 9854 --solr-url http://localhost:8983/solr --config-file ./conf/solr-exporter-config.xml --num-threads 8
----
...
----
> cd prometheus-exporter
> .\bin\solr-exporter.cmd -p 9854 --solr-url http://localhost:8983/solr --config-file .\conf\solr-exporter-config.xml --num-threads 8
----
...
If neither the `-s` parameter nor the `-z` parameter are defined, the default is `--solr-url http://localhost:8983/solr`.
...
If neither the `--solr-url` parameter nor the `-z` parameter are defined, the `--solr-url` parameter default is used.
...
|===
+
A unique ID for the cluster to monitor. This ID will be added to all metrics as a label `cluster_id` and can be used as a filter in the Grafana dashboard if you operate multiple Solr clusters reporting to the same Prometheus instance. If this option is omitted, a hash of the `--solr-url` or `--zk-host` will be used as ID by default.

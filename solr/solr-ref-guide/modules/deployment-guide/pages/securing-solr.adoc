...
The bind host is controlled via the `SOLR_ZOOKEEPER_SERVER_EMBEDDED_HOST` value in your environment's "include script" (`solr.in.sh` or `solr.in.cmd`):

[source,bash]
 ----
 SOLR_ZOOKEEPER_SERVER_EMBEDDED_HOST="0.0.0.0"
 ----

...
Solr includes a Java agent that enforces runtime security controls — file access, outbound network connections, `System.exit()`, and process spawning — at the JVM level.
The agent activates automatically in "warn-only" mode to allow safe observation before enabling blocking enforcement.

See xref:security-agent.adoc[] for details on enforcement modes, policy configuration, and diagnosing violations.
...
This can be done by setting a `SOLR_HOST_BIND` value in your environment's "include script" (`solr.in.sh` or `solr.in.cmd`):

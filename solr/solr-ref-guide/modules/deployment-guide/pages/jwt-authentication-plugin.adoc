...
By default, `blockUnknown` is `true`, so all requests without a valid JWT token will be blocked.

If you need to configure the plugin using unauthenticated REST API calls, set `blockUnknown` to `false` as further described in section <<Editing JWT Authentication Plugin Configuration>>.
...
blockUnknown         ; Set to `false` if you need to perform configuration through REST API or if you use an Authorization Plugin and only want certain paths protected. By default all requests will require a token  ; `true`
...
<1> Plugin class
<2> Block anyone without a valid token (this is also the default)

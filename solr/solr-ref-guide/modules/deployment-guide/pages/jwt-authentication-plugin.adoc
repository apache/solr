...
With `blockUnknown` set to `false` as in the above example, unauthenticated requests are allowed through. To enforce JWT authentication for all traffic, set `blockUnknown` to `true`.
...
blockUnknown         ; Set to `true` to require a valid JWT for all requests, or `false` to allow unauthenticated requests through (e.g., when using an Authorization Plugin for path-specific protection). ; `false`

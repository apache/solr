...
This ID, whether generated or provided by the user, can then be used to track or cancel the task -- as a query parameter with the V1 API, or as part of the URL path with the V2 API; see the sections below for the specifics of each operation.
...
=== taskUUID Parameter
With the V1 API, the `taskUUID` request parameter is used to specify a task UUID whose status can be checked.

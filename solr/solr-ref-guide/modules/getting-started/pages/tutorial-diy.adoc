= Exercise 3: Index Your Own Binary Files
:experimental:
...
The tutorial assumes that you have Docker installed and running on your computer.

=== Start an external Apache Tika Server

Indexing binary files (PDF, DOCX, PPTX, etc.) with the Solr CLI requires the Solr extraction module to be enabled and an external Apache Tika Server to be running. See xref:indexing-guide:indexing-with-tika.adoc#tika-server[Tika Server] for background. We will be running Tika Server using Docker.
...
# Start Solr with extraction module
...
=== Enable the extraction handler

Add the `/update/extract` handler to your collection, pointing to the Tika Server you just started
...
}' 'http://localhost:8983/solr/localDocs/config'
----
...
If you have a local directory of files, the Solr CLI (`bin/solr post`) can index a directory of files.
We saw this in action in our first exercise.
+
We used only JSON, XML and CSV in our exercises, but the Solr CLI can also handle HTML, PDF, Microsoft Office formats (such as MS Word), plain text, and more.
+
...
Go ahead and edit any of the existing example data files, change some of the data, and re-run the Solr CLI (`bin/solr post`).
...
To stop Tika Server when done, run `docker stop tika` in a Terminal window.

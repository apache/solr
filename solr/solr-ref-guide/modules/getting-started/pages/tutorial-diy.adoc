...
Indexing binary files (PDF, DOCX, PPTX, etc.) with the Post Tool requires the Solr extraction module to be enabled and an external Apache Tika Server to be running. See xref:indexing-guide:indexing-with-tika.adoc#tika-server[Tika Server] for background. We will be running Tika Server using Docker.
...
[,console]
----
curl 'http://localhost:8983/solr/localDocs/config' -H 'Content-type:application/json' -d '{
  "add-requesthandler": {
    "name": "/update/extract",

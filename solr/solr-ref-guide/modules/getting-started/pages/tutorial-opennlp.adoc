...
[,console]
----
curl "http://localhost:8983/solr/sentiment/schema" -H 'Content-type:application/json' --data-binary '{
  "add-field":{
    "name":"name",
    "type":"string",
    "stored":true }
...
[,console]
----
curl "http://localhost:8983/solr/sentiment/config" -H 'Content-type:application/json' -d '{
  "add-updateprocessor": {
...
[,console]
----
curl "http://localhost:8983/solr/sentiment/update/json?processor=sentimentClassifier&commit=true" -H 'Content-type:application/json' -d '[
  {
    "id":"good",

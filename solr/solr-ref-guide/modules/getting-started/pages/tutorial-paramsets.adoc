...
[,console]
----
curl http://localhost:8983/solr/films/schema -H 'Content-type:application/json' --data-binary '{
  "add-field" : [
    {
      "name":"name",
      "type":"text_general",
...
[,console]
----
curl http://localhost:8983/solr/films/config/params -H 'Content-type:application/json' --data-binary '{
"set": {
    "algo_a":{
      "defType":"dismax",
      "qf":"name"

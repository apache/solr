...
+
Standalone installations use instead the concept of a <<core,Core>>.
"Collection" is most frequently used in the SolrCloud context, but as it represents a "logical index", the term may be used to refer to individual cores in a standalone installation as well.
+
[[defcommit]]Commit::
...
[[replication]]xref:deployment-guide:standalone-index-replication.adoc[Replication]::

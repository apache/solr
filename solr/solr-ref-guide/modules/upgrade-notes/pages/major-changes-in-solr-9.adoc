...
== Solr 9.2
=== Jetty Configuration
* Solr no longer duplicates certain Jetty "server" library dependencies between `server/lib` and `WEB-INF/lib`.
This is an improvement to our release process, but Jetty does not allow web-apps (Solr) to use these libraries by default.
The `server/contexts/solr-jetty-context.xml` now explicitly removes these restrictions, allowing Solr to use these "server" classpaths which live in `server/lib`.

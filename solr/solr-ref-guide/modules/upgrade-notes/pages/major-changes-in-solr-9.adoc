...
TIP: Users who need to use an older version of Docker engine have a few options. The first is to stay at Solr 9.1. Another option is to build your own Solr docker image, specifying Ubuntu 20 Focal as base image. Lastly, you may be able to run version 9.2 on older Docker engine by specifying the docker commandline option `--security-opt seccomp=unconfined` when starting the container.

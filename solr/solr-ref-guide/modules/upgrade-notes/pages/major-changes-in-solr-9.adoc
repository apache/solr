...
See the section https://solr.apache.org/guide/solr/9_9/deployment-guide/monitoring-with-prometheus-and-grafana.html#environment-variable-options[Environment Variable Options] for more details.
...
curl 'http://localhost:8983/api/cluster/autoscaling' \
  ...
  -d '{set-cluster-policy : [], set-cluster-preferences : []}'
+

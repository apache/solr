...
=== Language Detection Library Replaced
...
The new library supports a broader set of languages and uses a different statistical model, so detection results may differ from previous versions — particularly for short texts, texts that mix Latin characters with Cyrillic scripts (e.g., Russian technical writing that includes English brand names), or documents containing CJK characters alongside Latin-script content.
Users relying on exact language detection results should validate their pipelines after upgrading.

...
* `OpenTelemetry` libraries were removed and replaced with `OpenTracing` libraries. Any Java agents providing `OpenTelemetry` tracers will no longer work. Telemetry tags `http.status_code` and `http.method` have been deprecated, newer version of the tags have been added to the span data: `http.response.status_code`, `http.request.method`.

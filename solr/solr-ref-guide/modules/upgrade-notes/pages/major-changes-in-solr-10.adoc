...
* The deprecated 'solrcore.properties` configuration method has been removed.  The ability to configure a core via properties file via the `core.proprties` "property" setting remains.
...
* Use of `stream.file`, `stream.url`, and `stream.body` params in http requests have been removed.

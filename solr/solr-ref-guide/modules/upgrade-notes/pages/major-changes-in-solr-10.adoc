...
The `/update/json/docs` path, and the v2 `/update` and `/update/json` paths that map to it, currently accept a payload of several whitespace spearated JSON objects. In a future version, only valid JSON will be allowed for this endpoint (either a single JSON object or an array of objects). Use the `/update/json/ndjson` endpoint for JSON Lines (newline-delimited JSON) payloads.

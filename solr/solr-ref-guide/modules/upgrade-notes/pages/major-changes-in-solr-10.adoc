...
[IMPORTANT]
====
For security reasons, connections to external Solr clusters are disabled by default in internal server-side components. External cluster access can be explicitly enabled by setting `-Dsolr.allow-external-clusters=true`. Enabling this option may expose the server to SSRF risks and should only be used in trusted environments.
====

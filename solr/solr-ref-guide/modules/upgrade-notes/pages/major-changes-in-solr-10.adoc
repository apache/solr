...
The default export protocol has changed from `grpc` to `http/protobuf` (OTLP port `4318` instead of `4317`).

Use of gRPC can be restored by adding the required gRPC & Netty jars to the `opentelemetry` module's lib directory and setting `OTEL_EXPORTER_OTLP_PROTOCOL=grpc`.

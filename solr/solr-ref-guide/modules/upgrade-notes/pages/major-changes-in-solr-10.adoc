...
* A ZooKeeper connection string must match the local cluster's exactly (including any chroot), or be listed in the new xref:configuration-guide:configuring-solr-xml.adoc#allow-zk-hosts[`allowZkHosts`] setting in `solr.xml` (or the `solr.security.allow.zk.hosts` system property).
* An HTTP(S) connection string must refer to live nodes of the local cluster, or to hosts listed in xref:configuration-guide:configuring-solr-xml.adoc#allow-urls[`allowUrls`].
...
`ExecutorUtil.isSolrServerThread()` now tracks work that originated from Solr node startup or request handling (and work spawned from those), instead of any task running on an MDC-aware pool.
Plugin code that starts independent threads outside startup/request context and then makes inter-node requests may need to explicitly set server identity (for example with `PKIAuthenticationPlugin.withServerIdentity(true)` around that work).
...
To deploy an extra context, or to customize how the Solr webapp itself is deployed, edit `server/etc/jetty.xml` directly and add the context there.

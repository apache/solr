...
and beyond.
A Solr node will now fail to start if it's major version (e.g. 9) is *lower* than that of any existing Solr node in a SolrCloud cluster (as reported by info in "live_node").
What this

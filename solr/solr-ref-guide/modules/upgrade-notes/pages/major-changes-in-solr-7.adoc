...
** If you run a user-managed cluster or a standalone installation, the instanceDir will be created automatically, using the `_default` configset as its basis.
...
+
See also the section https://solr.apache.org/guide/7_7/metrics-reporting.html#the-metrics-reporters-element[The <metrics><reporters> Element], which describes how to set up Metrics Reporters in `solr.xml`.

...
|DateField |Date field. Represents a point in time with millisecond precision. This field type indexes both terms and Point values, providing efficient performance for both exact value queries and range queries. See the section xref:date-formatting-math.adoc[] for more details on the supported syntax. For single valued fields, `docValues="true"` must be used to enable sorting.
...
|DoubleField |Double field (64-bit IEEE floating point). This field type indexes both terms and Point values, providing efficient performance for both exact value queries and range queries. For single valued fields, `docValues="true"` must be used to enable sorting.
...
|FloatField |Floating point field (32-bit IEEE floating point). This field type indexes both terms and Point values, providing efficient performance for both exact value queries and range queries. For single valued fields, `docValues="true"` must be used to enable sorting.
...
|IntField |Integer field (32-bit signed integer). This field type indexes both terms and Point values, providing efficient performance for both exact value queries and range queries. For single valued fields, `docValues="true"` must be used to enable sorting.
...
|LongField |Long field (64-bit signed integer). This field type indexes both terms and Point values, providing efficient performance for both exact value queries and range queries. For single valued fields, `docValues="true"` must be used to enable sorting.
...
|StrFloatLateInteractionVectorField |Supports indexing dense "Multi-Vectors" of float values for use with Late Interaction Query Re-Ranking. See the section xref:query-guide:dense-vector-search.adoc[] for more information.
...
|TrieDateField |Use DateField instead.

|TrieDoubleField |Use DoubleField instead.

|TrieFloatField |Use FloatField instead.

|TrieIntField |Use IntField instead.

|TrieLongField |Use LongField instead.

|TrieField |This field takes a `type` parameter to define the specific class of Trie* field to use; Use an appropriate numeric field type instead (IntField, LongField, FloatField, DoubleField, or DateField).

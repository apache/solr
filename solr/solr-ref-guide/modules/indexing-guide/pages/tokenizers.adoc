...
NOTE: Starting with Lucene 10, this tokenizer produces sequential tokens (positionIncrement=1) instead of overlapping tokens (positionIncrement=0). This change can affect phrase queries and position-aware search logic that relied on the previous overlapping behavior.

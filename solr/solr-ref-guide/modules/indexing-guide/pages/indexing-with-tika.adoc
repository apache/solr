...
* Tika automatically determines the input document type (e.g., Word, PDF, HTML) and extracts the content appropriately.
You can explicitly specify a MIME type for Tika with the `stream.type` parameter.
See https://tika.apache.org/docs/{dep-version-tika}/formats.html for the file types supported.
* Tika Server synthesizes an XHTML document from the core content of the parsed document and returns it to Solr over HTTP (Solr specifically requests the XML output variant, since Tika Server's default output format is Markdown -- see <<Tika Server>> above).
Solr Cell then parses that response with its own SAX parser, feeding the resulting events into a http://www.saxproject.org/quickstart.html[SAX] ContentHandler to create one or more text fields from the content.
Tika Server exposes document metadata as well (apart from the XHTML).
* Tika produces metadata such as `dc:title`, `dc:subject`, and `dc:author` according to specifications such as the DublinCore.
Metadata that Tika itself generates (as opposed to metadata it reads from the document) is namespaced under a single `tk:` prefix, e.g. `tk:parsed-by`.
...
The "techproducts" example included with Solr already has Solr Cell configured.
If you are not using that example, see the section <<solrconfig.xml Configuration>> below.
...
Solr delegates content extraction to an external Apache Tika Server process. This provides operational isolation (crashes or heavy parsing do not impact Solr), simplifies dependency management, and allows you to scale Tika independently of Solr.

Solr's role is limited to sending each document to Tika Server and mapping the metadata and text it returns into Solr fields; Solr does not configure Tika Server's parsers on your behalf, so anything beyond how Tika Server behaves out of the box (parser options, OCR, password-protected documents, etc.) is configured directly on Tika Server itself -- see <<Parser-Specific Properties>> below and the https://tika.apache.org/[Apache Tika documentation].
...
First, start a Tika Server on port 9998 using Docker.
...
The commands below will start Solr, create a core/collection named `gettingstarted` with the `_default` configset, and enable the extraction module. Then the `/update/extract` handler is added to the `gettingstarted` core/collection to enable Solr Cell.

[source,bash]
----
bin/solr start -e schemaless -Dsolr.modules=extraction

# Configure the handler at runtime
...
Here is a closer look at the components of this command:
...
For optimum performance when loading many documents, do not call the commit command until you are done.
...
[NOTE]
====
The above example does not work as expected if you run it after already indexing the document one or more times.

The earlier example indexed the document without these parameters, so all fields were already added to the index at that time.
The `uprefix` parameter only applies to fields that are _undefined_, so these fields are not prefixed when the document is re-indexed.
...
To make the embedded XHTML tags easier to read on screen, set the `extractFormat` parameter to a response format other than XML.
+
...
+
Example: `fmap.content=text` causes the data in the `content` field generated by Tika to be moved to Solr's `text` field.
...
In some cases, there may be metadata available that will be indexed.
+
...
|===
+
Not currently supported by the `tikaserver` backend; setting it is rejected with `BAD_REQUEST`.
See <<Indexing Encrypted Documents>> below.
...
+
This parameter can only be configured in the request handler configuration (`solrconfig.xml`), not per request.
+
Example: In `solrconfig.xml`: `<long name="tikaserver.maxChars">1000000</long>`

`tikaserver.legacyFieldNames`::
+
[%autowidth,frame=none]
|===
|Optional |Default: false
|===
+
When enabled, Solr renames the metadata keys Tika 4.x emits back to their Tika 3.x equivalents (for example, `tk:parsed-by` becomes `X-TIKA:Parsed-By`), using Tika's own bundled key-migration table. A key with no Tika 3.x equivalent is dropped, and a migrated key replaces its Tika 4.x original rather than being added alongside it.
This can ease migration for a schema or downstream code written against a Solr/Tika 3.x deployment.
See xref:upgrade-notes:major-changes-in-solr-11.adoc#apache-tika-4[Apache Tika 4] for background on the Tika 4.x key renaming.
+
Only applicable for the `tikaserver` backend. Can only be set in `solrconfig.xml`, not per request.
+
Example: In `solrconfig.xml`: `<bool name="tikaserver.legacyFieldNames">true</bool>`
...
+
If your Tika Server is using HTTPS, it must use a verifiable SSL certificate. If using a self-signed or custom Certificate Authority, add it to Solr's truststore. The `tikaserver` backend does not currently support configuring custom certificates for Tika alone.
+
...
See https://tika.apache.org/docs/{dep-version-tika}/using-tika/server/index.html for details on Tika Server's output formats; the structure of the XHTML varies with the format being parsed.
...
Parser-specific properties for Tika (OCR settings, per-parser options, etc.) must be configured directly on your Tika Server instance, via its own JSON configuration file. Consult the https://tika.apache.org/[Apache Tika documentation] for details.

NOTE: In earlier versions of Solr Cell you could supply Tika configuration directly to Solr. This is no longer possible: Solr does not pass any per-request configuration to Tika Server.

=== Indexing Encrypted Documents

`resource.password` and `passwordsFile` are not currently supported by the `tikaserver` backend and are rejected with `BAD_REQUEST` if set: Tika Server has no simple per-request way to accept a password outside of its own JSON parser-configuration mechanism, which Solr does not use.
If you need to index encrypted documents, configure a password directly on your Tika Server instance -- see the https://tika.apache.org/[Apache Tika documentation] -- or decrypt the documents before sending them to Solr.
...
IMPORTANT: Use the `extractOnly` option before indexing to discover the values Solr will set for these metadata elements on your content.
...
To add your own metadata, pass the literal parameter along with the file:
...
The example below passes an XPath expression to restrict the XHTML returned by Tika:
...
This is useful if you are using Solr solely as an extraction server, or to test how Solr extracts content before indexing it.
...
The example below streams the file as the body of the POST request, so Solr has no information about the name of the file.
...
See xref:deployment-guide:solrj.adoc[] for more information on SolrJ.

The example below uses Solr Cell and SolrJ to add documents to a Solr index.

It uses SolrJ to create a new SolrClient, then constructs a request containing a ContentStream (essentially a wrapper around a file) and sends it to Solr:
...
The key class to use is the `ContentWriterUpdateRequest`, which makes sure the request content is written properly.
SolrJ takes care of the rest.

Note that the `ContentWriterUpdateRequest` is not just specific to Solr Cell.

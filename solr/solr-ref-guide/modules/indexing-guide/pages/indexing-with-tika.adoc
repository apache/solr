...
This command will simply start Solr and create a core/collection named `techproducts` with the `techproducts` configset.
...
[source,bash]
----
bin/post -c techproducts example/exampledocs/solr-word.pdf -params "literal.id=doc1"
----

Now you can execute a query and find that document with a request like `\http://localhost:8983/solr/techproducts/select?q=pdf`.
...
[source,bash]
----
bin/post -c techproducts example/exampledocs/solr-word.pdf -params "literal.id=doc1&uprefix=ignored_&fmap.last_modified=last_modified_dt"
----

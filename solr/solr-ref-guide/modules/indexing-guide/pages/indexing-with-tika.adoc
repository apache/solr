...
The ExtractionRequestHandler supports multiple backends, selectable with the `extraction.backend` parameter. The only backend currently supported is the `tikaserver` backend, which uses an external Tika server process to do the extraction.
...
Parser-specific properties for Tika must be configured directly on your Tika Server instance. Consult the https://tika.apache.org/[Apache Tika documentation] documentation of this.

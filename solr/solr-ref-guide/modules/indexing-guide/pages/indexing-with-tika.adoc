...
`resource.password` and `passwordsFile` are not currently supported by the `tikaserver` backend and are rejected with `BAD_REQUEST` if set: TikaServer has no simple per-request way to accept a password outside of its own JSON parser-configuration mechanism, which Solr does not use.

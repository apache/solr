...
IMPORTANT: The `local` backend using an in-process Tika 1.x library to extract content, used to be the default until and including Solr 9.10. It was removed in Solr 9.11 for security reasons. See release notes for more information.
...
|===
+
Specifies the backend to use for extraction. As of Solr 9.11, only `tikaserver` is supported.

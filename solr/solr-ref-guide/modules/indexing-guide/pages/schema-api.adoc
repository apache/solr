...
[IMPORTANT]
====
The v2 `PUT` is a replace, not an append: the destinations you send become the complete set of
copy-field rules for that source, and any rule left out is removed.
Sending the same request twice is therefore safe.

To add a destination while keeping the existing ones, list them all, or use `add-copy-field`
through the bulk endpoint, which only ever appends.
Note that `add-copy-field` does not check for duplicates, so appending a rule that already exists
will cause the source to be copied twice.
====
...
[source,bash]
----
curl -X DELETE "http://localhost:8983/api/collections/techproducts/schema/copyfields/shelf?destination=location"
----
====
======
+
Repeat the `destination` parameter to remove several rules at once.
Omitting it entirely removes *every* rule with that source.

...
In the v2 API this attribute is named `destinations`; `dest` is still accepted as an alias.
...
}' http://localhost:8983/api/collections/techproducts/schema/copyfields/shelf
----
====
======
+
Copy-field rules have no name of their own, so v2 addresses them by their source field: the source
is the final path segment, and the body carries the destinations.
The v2 request attribute is `destinations`, though `dest` is accepted as an alias.
+
[IMPORTANT]
====
`PUT` is a replace, not an append: the destinations you send become the complete set of copy-field
rules for that source, and any rule left out is removed.
Sending the same request twice is therefore safe.
Sending an empty `destinations` list removes every rule with that source.
====
+
To add destinations while leaving the source's existing rules alone, `POST` to the same path:
+
[source,bash]
----
curl -X POST -H 'Content-type:application/json' --data-binary '
{
  "destinations":[ "catchall" ]
}' http://localhost:8983/api/collections/techproducts/schema/copyfields/shelf
----
+
Destinations that the source already copies to are ignored, so a repeated `POST` will not leave the
source copied twice over.
The `add-copy-field` command available through the bulk endpoint performs no such check.
...
[source,bash]
----
curl -X DELETE "http://localhost:8983/api/collections/techproducts/schema/copyfields/shelf/location"
----
====
======
+
Several destinations may be removed at once by giving them as a comma-separated list, and omitting
the destinations entirely removes *every* rule with that source:
+
[source,bash]
----
curl -X DELETE "http://localhost:8983/api/collections/techproducts/schema/copyfields/shelf/location,catchall"
curl -X DELETE "http://localhost:8983/api/collections/techproducts/schema/copyfields/shelf"
----
+
Deleting a source that has no copy-field rules returns a 404.
...
Bulk schema operations are also supported in Solr's v2 API, using a different endpoint and syntax.
The v2 bulk endpoint is `POST /api/collections/_collection_/schema/bulk`; the `/schema` path itself only supports `GET` in v2.
...
To list the rules for a single source field, prefer the v2 path form, which reads the same resource
that the `PUT`, `POST` and `DELETE` operations above address:

[source,bash]
----
curl -X GET "http://localhost:8983/api/collections/techproducts/schema/copyfields/shelf"
----

The `source.fl` query parameter described below is an alternative, and is the only option when
filtering by destination or by more than one source at a time.
A source with no copy-field rules returns an empty list rather than a 404.

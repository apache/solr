...
For general numeric needs, always use one of the `IntField`, `LongField`, `FloatField`, `DoubleField`, or `DateField` classes, depending on the specific values you expect.
These numeric classes index both terms and Point values, providing efficient performance for both exact value queries and range queries.
xref:docvalues.adoc[] on these fields are enabled by default, and are needed for sorting and/or faceting, it is not recommended to disable DocValues on numeric fields.

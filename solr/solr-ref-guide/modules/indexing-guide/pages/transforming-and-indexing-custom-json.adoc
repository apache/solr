...
The one-document-per-line payload shown first above is also valid ND-JSON, but `/update/json/docs` merely tolerates that shape: as the second example shows, it equally accepts an enclosing array, and it accepts a document spread over several lines or several documents on one line.

To have the format enforced instead, send the same content with `Content-Type: application/x-ndjson` or to the `/update/ndjson` path, described in xref:indexing-with-update-handlers.adoc#newline-delimited-json[].
...
The mapping parameters on this page apply there as well, with two exceptions: `split` must list the document root `/` first, since each line is already one document, and `srcField` is not supported.

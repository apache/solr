...
In V1, one or more valid JSON documents can be sent to `/update/json/docs` with the configuration parameters. In V2, use the document-only `/update/json` endpoint.
...
[source,bash]
----
curl http://localhost:8983/api/collections/techproducts/config/params \
  -H 'Content-Type: application/json' \
  -d '{
 "set": {
   "my_params": {
...
It is possible to send any JSON to the V1 `/update/json/docs` endpoint or the V2 `/update/json` endpoint.
Both use the document loader configuration shown below:

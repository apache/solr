...
The same parameters apply to xref:indexing-with-update-handlers.adoc#newline-delimited-json[newline delimited JSON], sent to `/update/ndjson` or to `/update` with `Content-Type: application/x-ndjson`.
...
+
With ND-JSON every line is already one document, so `split` serves only to declare nesting there, and its first path must be the root “`/`”.
...
This can only be used if `split=/` (i.e., you want your JSON input file to be indexed as a single Solr document), and it is not supported with ND-JSON.
...
Multiple documents can be sent in a single request as a JSON array, as in this example:
...
To send one document per line instead, use the xref:indexing-with-update-handlers.adoc#newline-delimited-json[ND-JSON] endpoints: `/update` with `Content-Type: application/x-ndjson`, or the `/update/ndjson` path.
Solr then requires exactly one JSON Object per line in UTF-8, and reports the offending line number when a line does not comply.
All of the mapping parameters above apply there, `f`, `df`, `mapUniqueKeyOnly` and `echo` unchanged, with the two differences noted under `split` and `srcField`.

...
The xref:post-tool.adoc[] sends `.jsonl` files in this format automatically.
...
This makes the format a good fit for large bulk loads.
Blank lines are ignored, and a line that is not a single JSON Object fails the request with the offending line number.

...
Solr also accepts https://jsonlines.org/[Newline Delimited JSON] (ND-JSON), also known as JSON Lines or JSONL: one document per line, as a JSON Object, with no enclosing array and no commas between lines.
...
`application/jsonl`, `application/jsonlines` and `application/x-jsonlines` are accepted as aliases, and the xref:post-tool.adoc[] sends `.jsonl` and `.ndjson` files automatically.
...
Every line is an implicit `add`; NDJSON has no way to express the other update commands, so `delete`, `commit`, `optimize`, and `rollback` are not available in the payload.
Use the `commit` or `commitWithin` request parameters to control when the added documents are committed.

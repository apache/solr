...
A single line may not exceed 16 MB, which catches input that is not in fact newline delimited -- a whole JSON array on one line, for example.
If you index unusually large documents, raise the limit with the `solr.ndjson.maxLineLength` system property (in characters), or per handler with a `maxLineLength` init argument:

...
`application/jsonl` and `application/x-jsonlines` are accepted as aliases, and the xref:post-tool.adoc[] sends `.jsonl` and `.ndjson` files automatically.
The format is UTF-8 only: a request declaring any other `charset` is rejected with a `415 Unsupported Media Type`.

Solr parses and indexes the request as it streams in, so peak memory is bounded by the largest single document rather than by the size of the request, which makes the format a good fit for large bulk loads.
Blank lines are ignored.
A line that is not a single JSON Object -- an enclosing array, a document spread over several lines, two documents on one line, or anything that is not an Object -- fails the request, reporting the line number.
Documents from earlier lines have already been submitted when a later line fails, so a failed load may be partially applied.

A single line is also bounded in length, by default a quarter of the max heap while parsing, or about 64 million characters on a 2 GB heap.
Set it with the `solr.ndjson.maxLineLength` system property (in characters) or a `maxLineLength` init argument:
...
NDJSON carries documents only; the `add`, `delete`, `commit`, `optimize`, and `rollback` commands are not recognized, so use request parameters such as `commit` and `commitWithin` instead.

Documents are mapped as on the `/update/json/docs` path, described in xref:transforming-and-indexing-custom-json.adoc[]: by default a nested JSON Object is *flattened* into dotted field names, so `{"id":"1","author":{"name":"Jan"}}` indexes `author.name`.
Use the `split` parameter to declare the paths that should become xref:indexing-nested-documents.adoc[child documents] instead (`|` is URL encoded as `%7C`):
...
The first `split` path must be the root `/`, since every line is already one document, and wildcards are not allowed in it.
`f`, `echo` and `mapUniqueKeyOnly` work as on `/update/json/docs`; `srcField` is not supported, and neither are xref:partial-document-updates.adoc[atomic updates].

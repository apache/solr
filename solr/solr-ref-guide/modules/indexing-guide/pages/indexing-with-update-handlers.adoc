...
NOTE: Solr also exposes update functionality via the xref:configuration-guide:v2-api.adoc[v2 API].
The v2 update endpoints are documented in the <<V2 Update API>> section below.
...
You can use the `curl` utility to perform any of the above commands, using its `-d` option to append the XML message to the `curl` command, and generating a HTTP POST request.
The same XML update can be sent through either API:

[tabs#xml-update-request]
======
V1 API::
+
====
[source,bash]
----
curl http://localhost:8983/solr/my_collection/update?versions=true -H "Content-Type: text/xml" -d '
<add>
  <doc>
    <field name="authors">Patrick Eagar</field>
    <field name="subject">Sports</field>
...
[source,bash]
----
curl http://localhost:8983/api/collections/my_collection/update/xml?versions=true -H "Content-Type: text/xml" -d '
<add>
  <doc>
    <field name="authors">Patrick Eagar</field>
    <field name="subject">Sports</field>
...
The approach above works well, but using the `-d` option causes `curl` to load the whole `myfile.xml` into memory before posting it to server.
...
[tabs#xml-file-streaming-request]
======
V1 API::
+
====

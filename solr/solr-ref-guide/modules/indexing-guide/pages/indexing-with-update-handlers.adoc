...
Because each document ends at a newline, Solr indexes the request one line at a time without parsing the whole request body first, and a producer can append documents to a file or stream without rewriting any enclosing structure.
Peak memory therefore depends on the longest single line rather than on the size of the request, which makes the format a good fit for large bulk loads.

Blank lines are ignored, and a line that is not a single JSON Object fails the request reporting the offending line number.
...
A single line is bounded so that input which is not in fact newline delimited -- a whole JSON array on one line, for example -- fails with a clear error rather than exhausting the heap.
...
A nested JSON Object is always indexed as a xref:indexing-nested-documents.adoc[child document], the same as on the `/update/json/docs` path.
NDJSON therefore does not support xref:partial-document-updates.adoc[atomic updates], which rely on a nested Object being read as a modifier such as `set` or `inc` instead.

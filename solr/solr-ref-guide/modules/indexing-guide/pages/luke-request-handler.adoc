...
In non-cloud mode, you can still distribute across shards by passing the `shards` parameter with explicit shard URLs.
...
For collection-wide term frequencies or cardinality estimates, Solr's xref:query-guide:faceting.adoc[faceting API] may cover some of these use cases.

...
Solr's date fields (`DateField`, `DatePointField`, `DateRangeField` and the deprecated `TrieDateField`) represent "dates" as a point in time with millisecond precision.
...
If you specify a date instance supported by DateField or DatePointField with date math truncating it, like `NOW/DAY`, you still get the first millisecond of that day, not the entire day's range.
...
`DateRangeField` is almost a drop-in replacement for places where `DateField` or `DatePointField` is used.

= Property Substitution in solrconfig.xml
...
// *TODO* merge this page with defining-core-properties.adoc?
...
If the configuration directory for a Solr core contains a file named `solrcore.properties` that file can contain any arbitrary user-defined property names and values using the Java https://en.wikipedia.org/wiki/.properties[properties file format].
Those properties can then be used as variables in other configuration files for that Solr core.
...
[WARNING]
====
`solrcore.properties` does not work in SolrCloud mode as it is not read from ZooKeeper.
This feature is likely to be removed in the future.
Instead, use another mechanism like <<Config API to Override solrconfig.xml,`configoverlay.json`>>.
====

...
* Every request handler in Solr now implements PermissionNameProvider to explicitly decide on what security permissions are required to access the handler. This means that you may need permission changes in `security.json` after upgrading.

...
The CSV response writer supports multi-valued fields, as well as <<transforming-result-documents.adoc#,pseudo-fields>>, and the output of this CSV format is compatible with Solr's <<indexing-with-update-handlers.adoc#csv-formatted-index-updates,CSV update format>>.

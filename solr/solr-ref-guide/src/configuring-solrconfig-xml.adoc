...
|===
| <<datadir-and-directoryfactory-in-solrconfig.adoc#,DataDir and DirectoryFactory in SolrConfig>>: Where and how Solr's indexes are stored.
| <<schema-factory-definition-in-solrconfig.adoc#,Schema Factory Definition in SolrConfig>>: Schema file formats.
| <<indexconfig-in-solrconfig.adoc#,IndexConfig in SolrConfig>>: Options for Lucene index writers, including segment management, merges, and locks.
| <<segments-info.adoc#,Segments Info screen>>: A visualization of the underlying Lucene index segments. *TODO* this placement is kind of odd unless this whole section is re-done in a different way.
| <<requesthandlers-and-searchcomponents-in-solrconfig.adoc#,RequestHandlers and SearchComponents in SolrConfig>>: *TODO*
| <<implicit-requesthandlers.adoc#,Implicit RequestHandlers>>: Request end-points automatically provided by Solr and how to configure them.
| <<initparams-in-solrconfig.adoc#,InitParams in SolrConfig>>: Default parameters for request handlers.
| <<updatehandlers-in-solrconfig.adoc#,UpdateHandlers in SolrConfig>>: Update requests and commit settings.
| <<query-settings-in-solrconfig.adoc#,Query Settings in SolrConfig>>: Caches, query warming, and query listeners.
| <<requestdispatcher-in-solrconfig.adoc#,RequestDispatcher in SolrConfig>>: Advanced settings for how parsing requests and HTTP cache headers.

...
|===
+
The S3 bucket to read and write all backup files to. Can be overridden by setting `BLOB_S3_BUCKET_NAME` environment variable.
...
Can be overridden by setting `BLOB_S3_REGION` environment variable.
...
|===
+
Explicit S3 endpoint. Not needed under normal operations (the S3 client can infer the endpoint from the `blob.s3.region`).
This parameter is helpful if using a mock S3 framework and want to explicitly override where S3 requests are routed.
Can be overridden by setting `BLOB_S3_ENDPOINT` environment variable when `blob.s3.mock` is set to true.


[NOTE]
====
You can use the `blob.s3.endpoint` option to use this BackupRepository with _s3-compatible_ endpoints.
...
Should be used in conjunction with `blob.s3.proxy.port` parameter.
Can be overridden by setting `BLOB_S3_PROXY_HOST` environment variable.
...
Should be used in conjunction with `blob.s3.proxy.host` parameter.
Can be overridden by setting `BLOB_S3_PROXY_PORT` environment variable.
...
|===
+
If this S3 backup repository should route requests to an mock S3 instance (for example, https://github.com/adobe/S3Mock[S3Mock]).
If true (and `blob.s3.endpoint` is not set), S3 requests will be sent to `localhost:9090`.
Can be overridden by setting `BLOB_S3_MOCK` environment variable.
...
<backup>
  <repository name="s3" class="org.apache.solr.s3.S3BackupRepository" default="false">
    <str name="blob.s3.bucket.name">my-s3-bucket</str>

...
|===
+
Explicit S3 endpoint. Not needed under normal operations (the S3 client can infer the endpoint from the `s3.region`).
This parameter is helpful if using a mock S3 framework and want to explicitly override where S3 requests are routed.
Can be overridden by setting `S3_ENDPOINT` environment variable when `s3.mock` is set to true.
...
If true (and `s3.endpoint` is not set), S3 requests will be sent to `localhost:9090`.
Can be overridden by setting `S3_MOCK` environment variable.

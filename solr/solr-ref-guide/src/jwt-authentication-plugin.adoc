...
trustedCerts         ; One or more X.509 SSL certificates in plaintext PEM format, that should be trusted when talking to IdPs. ; Defaults to Java truststore
trustedCertsFile     ; Path to a `.pem` file containing one or more X.509 SSL certificates that should be trusted when talking to IdPs. ; Defaults to Java truststore

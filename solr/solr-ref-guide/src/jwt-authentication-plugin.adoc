...
trustedCertsFile     ; Path to a file of type PEM, DER or PKCS#7, containing one or more X.509 SSL certificates that should be trusted when talking to IdPs. Please configure either this or the `trustedCerts`, not both. ; Defaults to Java truststore

...
The Scripting module provides a separate XSLT Update Request Handler that allows you to index any arbitrary XML by using the `<tr>` parameter to apply an https://en.wikipedia.org/wiki/XSLT[XSL transformation].
...
Learn more about adding the `modules/scripting/lib/*` files into Solr's <<libs.adoc#lib-directories,Lib Directories>>.

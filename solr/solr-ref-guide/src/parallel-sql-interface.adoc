...
<<<<<<< HEAD
Some operators that are not supported are BETWEEN, LIKE and IN.
However, there are workarounds for BETWEEN and LIKE.
...
The supported functions are `count(*)`, `min`, `max`, `sum`, and `avg`.

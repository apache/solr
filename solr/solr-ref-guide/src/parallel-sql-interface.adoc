...
* When performing a range query over a multi-valued field, Apache Calcite short-circuits to zero results if the `AND`'d criteria appears to be disjoint sets. For example, `b_is >= 5 AND b_is <= 2` appears to Calcite to be disjoint sets (from a single-valued field perspective) but with multi-valued fields, Solr might match documents. The work-around is to use Solr query syntax directly inside of an equals expression: `b_is = '(+[5 TO *] +[* TO 2])'`

...
A dense vector describes information as an array of elements, most of them different from zero.

In this context, "sparse" and "dense" refer to the number of zero vs. non-zero elements in the array. 

A sparse vector is one that contains mostly zeros and few non-zero entries. 
Sparse representation is at the base of traditional information retrieval approaches (such as the https://en.wikipedia.org/wiki/Bag-of-words_model[Bag-of-words model]).

A dense array contains mostly non-zeros.

Among other techniques and use cases, deep learning models such as https://en.wikipedia.org/wiki/BERT_(language_model)[BERT] are able to encode textual information as dense vectors, to be used for dense retrieval strategies.

...
The Prometheus exporter is included in Solr as a module, and is located in `modules/prometheus-exporter` in your Solr instance.
...
You can start `solr-exporter` by running `./bin/solr-exporter` (Linux) or `.\bin\solr-exporter.cmd` (Windows) from the `modules/prometheus-exporter` directory.
...
A default example configuration is in `modules/prometheus-exporter/conf/solr-exporter-config.xml`.
...
A Grafana sample dashboard is provided in the following JSON file: `modules/prometheus-exporter/conf/grafana-solr-dashboard.json`.

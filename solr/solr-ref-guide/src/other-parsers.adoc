...
*TODO* link to new page <<block-join-query-parser.adoc#>>.
...
Notice that it also supports leading wildcards "*a" as well with consequent performance implications. Applying <<filters.adoc#reversed-wildcard-filter,ReversedWildcardFilterFactory>> in index-time analysis is usually a good idea.
...
*TODO* link to <<join-query-parser.adoc#>>.

...
However, the `MultiAuthPlugin` combines the roles from all plugins together when determining all roles for a user.

Users should take special care to lock down the exact set of endpoints service accounts need access to when using Basic authentication.

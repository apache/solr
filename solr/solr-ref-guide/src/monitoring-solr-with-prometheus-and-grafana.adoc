...
The default is `contrib/prometheus-exporter/conf/solr-exporter-config.xml`.

`-n`, `--num-threads`, `$NUM_THREADS`::
The number of threads. The `solr-exporter` creates thread pools for requests to Solr.
...
`collection`::: The collection to issue the query against.
Only used with SolrCloud clusters.
`core`::: The core to issue the query against.
Only used with user-managed clusters or single-node installations.
`path`::: The path to the query endpoint where the request will be sent.

...
* `group.ngroups` and `group.facet` require that all documents in each group must be co-located on the same shard in order for accurate counts to be returned. <<solrcloud-shards-indexing.adoc#,Document routing via composite keys>> can be a useful solution in many situations.

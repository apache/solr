...
Alternatively, you can create the znode automatically when starting Solr with the `-z` parameter and `-DcreateZkChroot=true`.  Read below for an example.
...
If the znode does not exist, you can add `-DcreateZkChroot=true` with the `-z` parameter to create it automatically on start-up:

[source,bash]
----
bin/solr start -e cloud -z zk1:2181,zk2:2181,zk3:2181/solr -DcreateZkChroot=true
----

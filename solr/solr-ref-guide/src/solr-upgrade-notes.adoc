...
The new backup format supports incremental backups.
This is supported by default, simply by storing each backup file in the same location.

The old and new formats are not compatible, although backups in the old format can still be used to restore to Solr for the time-being.

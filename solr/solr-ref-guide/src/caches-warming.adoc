...
Caches can also be disabled on a query-by-query basis with the `cache` parameter, as described in the section <<common-query-parameters.adoc#cache-parameter,cache Parameter>>.

Details of each cache are described below.

=== Filter Cache

The `filterCache` is used by `SolrIndexSearcher` for filters (DocSets) for unordered sets of all documents that match a query.
The numeric attributes control the number of entries in the cache.
...
If this per-collection limit is greater than the <<format-of-solr-xml#global-maxbooleanclauses,global `maxBooleanClauses` limit>> specified in `solr.xml`, it will have no effect, as that setting also limits the size of user specified boolean queries.

...
Other policies available are the `LogByteSizeMergePolicy`, `LogDocMergePolicy`, and `UninvertDocValuesMergePolicy`.
...
See the section <<running-solr-on-hdfs.adoc#,Running Solr on HDFS>> for more details on using this feature.

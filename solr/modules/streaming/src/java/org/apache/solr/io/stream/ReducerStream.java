...
import org.apache.solr.io.ops.ReduceOperation;
...
/**
 * Iterates over a TupleStream and buffers Tuples that are equal based on a comparator. This allows
 * tuples to be grouped by common field(s).
 *
 * <p>The read() method emits one tuple per group. The fields of the emitted Tuple reflect the first
 * tuple encountered in the group.
 *
 * <p>Use the Tuple.getMaps() method to return all the Tuples in the group. This method returns a
 * list of maps (including the group head), which hold the data for each Tuple in the group.
 *
 * <p>Note: The ReducerStream requires that it's underlying stream be sorted and partitioned by the
 * same fields as it's comparator.
 *
 * @since 5.1.0
 */
public class ReducerStream extends TupleStream implements Expressible {
  ...
  public ReducerStream(TupleStream stream, StreamComparator comp, ReduceOperation op)
      throws IOException {
  ...
  public ReducerStream(StreamExpression expression, StreamFactory factory) throws IOException {
    ...
    if (operationExpressions != null && operationExpressions.size() == 1) {
      ...
      if (operation instanceof ReduceOperation) {
        ...
        throw new IOException(
            "The ReducerStream requires a ReduceOperation. A StreamOperation was provided.");
      }
    } else {
    ...
    init(
        ...
        factory.constructEqualitor(
            ((StreamExpressionValue) byExpression.getParameter()).getValue(), FieldEqualitor.class),
        reduceOperation);
  }

...
public interface StreamEvaluator extends Expressible, Serializable {
  ...
  /**
   * Execute the evaluator over lets stored within the StreamContext. This allows evaluators to be
   * executed over values calculated elsewhere in the pipeline and stored in the {@link
   * StreamContext#getLets() streamContext.lets}
   *
   * <p>Default implementation just creates a tuple out of all values in the context and passes that
   * to {@link StreamEvaluator#evaluate(Tuple)}.
   *

...
public class TestXLSXResponseWriter extends SolrTestCaseJ4 {
  ...
  public static void beforeClass() throws Exception {
    ...
    initCore("solrconfig-minimal.xml", "schema.xml", getFile("extraction/solr").getAbsolutePath());
    createIndex();
  ...
  public void testStructuredDataViaBaseWriters() throws Exception {
    ...
    XSSFSheet resultSheet =
        getWSResultForQuery(
            req("q", "id:1", "wt", "xlsx", "fl", "id,foo_s,foo_i,foo_l,foo_b,foo_f,foo_d,foo_dt1"));
  ...
  public void testPseudoFields() throws Exception {
    ...
    String txt =
        getStringFromSheet(
            getWSResultForQuery(
                req("q", "id:1", "wt", "xlsx", "fl", "XXX:id,YYY:[docid],FOO:foo_s")));
    ...
    String funcText =
        getStringFromSheet(
            getWSResultForQuery(
                req("df", "text", "q", "*", "wt", "xlsx", "fl", "XXX:id,YYY:exists(foo_s1)")));
    ...
    String singleFuncText =
        getStringFromSheet(
            getWSResultForQuery(
                req("df", "text", "q", "*", "wt", "xlsx", "fl", "exists(foo_s1),XXX:id")));
    ...
    txt =
        getStringFromSheet(
            getWSResultForQuery(
                req("df", "text", "q", "id:4", "wt", "xlsx", "fl", "*,YYY:[docid],FOO:foo_s1")));
    lines = txt.split("\n");
    assertEquals(2, lines.length);
    assertEquals(
        sortHeader(
  ...
  public void testForDVEnabledFields() throws Exception {
    ...
    String singleFuncText =
        getStringFromSheet(getWSResultForQuery(req("df", "text", "q", "id:5", "wt", "xlsx")));
    String sortedHeader =
        sortHeader(

...
public class TeamDraftInterleaving implements Interleaving {
  ...
  static {
    ...
    String seed = EnvUtils.getProp("tests.seed");
    if (seed == null) {
      RANDOM = new Random();
    } else {
  ...
  /**
   * Team Draft Interleaving considers two ranking models: modelA and modelB. For a given query,
   * each model returns its ranked list of documents La = (a1,a2,...) and Lb = (b1, b2, ...). The
   * algorithm creates a unique ranked list I = (i1, i2, ...). This list is created by interleaving
   * elements from the two lists la and lb as described by Chapelle et al.[1]. Each element Ij is
   * labelled TeamA if it is selected from La and TeamB if it is selected from Lb.
   *
  ...
  public InterleavingResult interleave(ScoreDoc[] rerankedA, ScoreDoc[] rerankedB) {
    ...
    while (interleavedResults.size() < topN
        && indexA < rerankedA.length

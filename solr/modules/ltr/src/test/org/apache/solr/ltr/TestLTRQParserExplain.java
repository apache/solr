...
public class TestLTRQParserExplain extends TestRerankBase {
  ...
  public void multipleAdditiveTreesScoreExplainMissingEfiFeatureShouldReturnDefaultScore()
    ...
    final String tree1 =
        "(weight=1.0,root=(feature=user_device_smartphone,threshold=0.5,missing=right,left=0.0,right=50.0))";
    final String tree2 =
        "(weight=1.0,root=(feature=user_device_tablet,threshold=0.5,missing=left,left=0.0,right=65.0))";
    ...
    assertJQ(
        "/query" + query.toQueryString(),
        "/debug/explain/7=='\n"
            + "115.0 = MultipleAdditiveTreesModel(name=external_model_binary_feature,trees="
            + trees
            + ") model applied to features, sum of:\n"
  ...
  public void interleavingModels_shouldReturnExplainForTheModelPicked() throws Exception {
    ...
    loadFeature("featureA1", SolrFeature.class.getName(), "{\"fq\":[\"{!terms f=popularity}1\"]}");
    loadFeature(
        "featureA2", SolrFeature.class.getName(), "{\"fq\":[\"{!terms f=description}bloomberg\"]}");
    loadFeature("featureAB", SolrFeature.class.getName(), "{\"fq\":[\"{!terms f=popularity}2\"]}");
    loadFeature("featureB1", SolrFeature.class.getName(), "{\"fq\":[\"{!terms f=popularity}5\"]}");
    loadFeature(
        "featureB2", SolrFeature.class.getName(), "{\"fq\":[\"{!terms f=title}different\"]}");
    ...
    */
    ...
    for (int i = 1; i <= 4; i++) {
      ...
      tests[i + 4] =
          "/debug/explain/"
              + expectedInterleaved[(i - 1)]
              + "=='"
              + expectedExplains[(i - 1)]
              + "'}";
    }
  ...
  @Test
  public void interleavingModelsWithOriginalRanking_shouldReturnExplainForTheModelPicked()
      throws Exception {
    TeamDraftInterleaving.setRANDOM(
        new Random(10)); // Random Boolean Choices Generation from Seed: [1,0]

      ...
      for (int topN = 1; topN <= 5; topN++) {
        ...
        for (int i = topN - 1, j = 0; i >= 0; i--, j++) {
          if (log.isInfoEnabled()) {
            log.info("doc {} in pos {}", searcher.storedFields().document(hits.scoreDocs[j].doc).get("id"), j);
          }
          assertEquals(i, Integer.parseInt(searcher.storedFields().document(hits.scoreDocs[j].doc).get("id")));

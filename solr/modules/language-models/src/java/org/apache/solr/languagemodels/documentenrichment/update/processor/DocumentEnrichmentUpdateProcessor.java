...
class DocumentEnrichmentUpdateProcessor extends UpdateRequestProcessor {
  ...
  public void processAdd(AddUpdateCommand cmd) throws IOException {
    ...
    try {
      ...
      Object returnValue = Utils.fromJSONString(largeLanguageModel.generate(injectedPrompt, responseFormat));
      // Guardrails for OllamaChatModel, since it doesn't support strict JSON structured output mode
      if (!(returnValue instanceof Map<?, ?> map) || !map.containsKey(DocumentEnrichmentUpdateProcessorFactory.JSON_FIELD_PROPERTY)) {
        throw new SolrException(
            SolrException.ErrorCode.SERVER_ERROR,
            "LLM was not able to format the response correctly: " + returnValue);
      }
      Object generatedFieldValue = map.get(DocumentEnrichmentUpdateProcessorFactory.JSON_FIELD_PROPERTY);
      ...
      boolean typeOk = switch (valueSchema) {
        ...
        case JsonIntegerSchema ignored -> generatedFieldValue instanceof Integer ||  generatedFieldValue instanceof Long;
      ...
      if (!typeOk) {
        throw new SolrException(
            ...
            "LLM returned wrong value type: expected " + valueSchema.getClass().getSimpleName()
                + " but got " + generatedFieldValue.getClass().getSimpleName());
      }

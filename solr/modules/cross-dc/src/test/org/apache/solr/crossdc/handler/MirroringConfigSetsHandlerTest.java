...
public class MirroringConfigSetsHandlerTest extends SolrTestCaseJ4 {
    ...
    private static SolrQueryRequest createRequest(SolrCore solrCore, ConfigSetParams.ConfigSetAction action, String configSetName, String zipResource) throws Exception {
        ...
        if (action == ConfigSetParams.ConfigSetAction.UPLOAD) {
            ...
            List<ContentStream> streams = List.of(new ContentStreamBase.ByteArrayStream(content, configSetName, "application/zip"));
    ...
    public void testList() throws Exception {
        SolrQueryRequest req = createRequest(solrCore, ConfigSetParams.ConfigSetAction.LIST, null, null);
        runCommand(req, false, true);
    }

    @Test
    public void testUpload() throws Exception {
        SolrQueryRequest req = createRequest(solrCore, ConfigSetParams.ConfigSetAction.UPLOAD, "testConf", "/configs/cloud-minimal.zip");
        runCommand(req, true, true);
    }

    private void runCommand(SolrQueryRequest req, boolean expectStreams, boolean expectResult) throws Exception {
        int initialMirroredCount = captor.getAllValues().size();
        MirroringConfigSetsHandler handler = Mockito.spy(new MirroringConfigSetsHandler(coreContainer, sink));
        Mockito.doNothing().when(handler).baseHandleRequestBody(Mockito.any(), Mockito.any());
        SolrQueryResponse rsp = new SolrQueryResponse();
        handler.handleRequestBody(req, rsp);
        if (expectResult) {
            assertEquals("should capture additional mirrored value", initialMirroredCount + 1, captor.getAllValues().size());
            ...
            if (expectStreams) {
                ...
                for (int i = 0; i < sourceStreams.size(); i++) {
                    ...
                    byte[] sourceContent = MirroredSolrRequest.ExposedByteArrayContentStream.of(source).byteArray();
                    byte[] mirroredContent = MirroredSolrRequest.ExposedByteArrayContentStream.of(mirrored).byteArray();

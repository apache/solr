  ...
  public CommitStream(StreamExpression expression, StreamFactory factory) throws IOException {
    ...
    if (batchSize < 0) {
      throw new IOException(
          String.format(
              Locale.ROOT,
              "invalid expression %s - batchSize cannot be less than 0 but is '%d'",
              expression,
              batchSize));
    }
  ...
  private void sendCommit() throws IOException {
    ...
    try {
      ...
      log.warn(
          String.format(
              Locale.ROOT,
              "Unable to commit documents to collection '%s' due to unexpected error.",
              collection),
          e);
      ...
      throw new IOException(
          String.format(
              Locale.ROOT,
              "Unexpected error when committing documents to collection %s- %s:%s",
              collection,
              className,
              message));
    }

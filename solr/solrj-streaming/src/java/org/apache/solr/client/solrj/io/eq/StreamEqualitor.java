...
public interface StreamEqualitor extends Equalitor<Tuple>, Expressible, Serializable {
  ...
  static StreamComparator deriveComparator(StreamEqualitor eq, StreamComparator comp)
    ...
    if (eq instanceof MultipleFieldEqualitor && comp instanceof MultipleFieldComparator) {
      MultipleFieldEqualitor multiEq = (MultipleFieldEqualitor) eq;
      MultipleFieldComparator multiComp = (MultipleFieldComparator) comp;
      ...
      return deriveComparator(eq, ((MultipleFieldComparator) comp).getComps()[0]);
    } else if (eq instanceof FieldEqualitor && comp instanceof FieldComparator) {
      FieldEqualitor fieldEq = (FieldEqualitor) eq;
      FieldComparator fieldComp = (FieldComparator) comp;
  ...
  static StreamEqualitor deriveRightEqualitor(StreamEqualitor eq) throws IOException {
    if (eq instanceof MultipleFieldEqualitor) {
      MultipleFieldEqualitor multiEq = (MultipleFieldEqualitor) eq;
    ...
    } else if (eq instanceof FieldEqualitor) {
      return new FieldEqualitor(((FieldEqualitor) eq).getRightFieldName());
    } else {

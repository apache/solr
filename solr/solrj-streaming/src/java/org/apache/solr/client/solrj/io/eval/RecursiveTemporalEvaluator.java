...
public abstract class RecursiveTemporalEvaluator extends RecursiveEvaluator
    implements OneValueWorker {
  protected static final long serialVersionUID = 1L;

  private String functionName;

  public RecursiveTemporalEvaluator(
      StreamExpression expression, StreamFactory factory, String functionName) throws IOException {
    ...
    if (1 != containedEvaluators.size()) {
      throw new IOException(
          String.format(
              Locale.ROOT,
              "Invalid expression %s - expecting one value but found %d",
              expression,
      ...
      try {
        ...
        throw new UncheckedIOException(
            new IOException(
                String.format(
                    Locale.ROOT,
                    "It is not possible to call '%s' function on %s",
                    getFunction(),

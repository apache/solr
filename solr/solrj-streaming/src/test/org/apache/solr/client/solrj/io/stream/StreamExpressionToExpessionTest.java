...
public class StreamExpressionToExpessionTest extends SolrTestCase {
  ...
  public void testCloudSolrStream() throws Exception {
    ...
    try (CloudSolrStream stream =
        new CloudSolrStream(
            StreamExpressionParser.parse(
                "search(collection1, q=*:*, fl=\"id,a_s,a_i,a_f\", sort=\"a_f asc, a_i asc\", aliases=\"id=izzy,a_s=kayden\")"),
            factory)) {
      expressionString = stream.toExpression(factory).toString();
  ...
  public void testFeaturesSelectionStream() throws Exception {
    ...
    try (FeaturesSelectionStream stream =
        new FeaturesSelectionStream(StreamExpressionParser.parse(expr), factory)) {
  ...
  public void testTextLogitStreamWithFeaturesSelection() throws Exception {
    String expr =
        "tlogit("
            ...
            + "outcome=\"out_i\", "
            + "maxIterations=100)";
    try (TextLogitStream logitStream =
        new TextLogitStream(StreamExpressionParser.parse(expr), factory)) {

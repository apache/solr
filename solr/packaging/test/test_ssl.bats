...
@test "use different hostname when not checking peer-name" {
  ...
  run ! solr api --debug --solr-url "https://localhost:${SOLR_PORT}/solr/admin/collections?action=CLUSTERSTATUS"
...
@test "start solr with mTLS needed" {
  ...
  (
    ...
    (
      ...
      run ! solr api --debug --solr-url "https://localhost:${SOLR_PORT}/solr/test/select?q=*:*&rows=0"
  ...
  run ! solr api --debug --solr-url "https://localhost:${SOLR3_PORT}/solr/test/select?q=*:*&rows=0"

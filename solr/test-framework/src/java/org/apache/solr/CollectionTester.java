...
class CollectionTester {
  ...
  private boolean handleSpecialString(String str) {
    ...
    if ("ignore".equals(code)) {
      ...
      if (!(val instanceof String)) {
        err = "mismatch: '" + expected + "'!='" + val + "', value is not a string";
        return false;
      }
      ...
      err = "mismatch: '" + expected + "'!='" + val + "', regex does not match";
      return false;
    }
    ...
    for (String expectedKey : keys) {
      ...
      if (ordered) {
        ...
        for (; ; ) {
          if (!iter.hasNext()) {
            popPath();
            err = "expected key '" + expectedKey + "' in ordered map";
            return false;
          }
          entry = iter.next();
        ...
        if (!entry.getKey().equals(expectedKey)) {
          popPath();
          err = "expected key '" + expectedKey + "' instead of '" + entry.getKey() + "' in ordered map";
          return false;
        }
        val = entry.getValue();
      } else {
  ...
  public boolean seek(List<String> seekPath) {
    while (true) {
      ...
      seekPath = seekPath.subList(1, seekPath.size());
    }
  }

...
final class TestRuleSetupAndRestoreClassEnv extends AbstractBeforeAfterRule {
  private static final Similarity DEFAULT_SIM = new BM25Similarity();
  private static final TimeZone DEFAULT_TZ = new SimpleTimeZone(0, "simple");
  ...
  volatile Similarity similarity = DEFAULT_SIM;
  Codec codec;
  ...
  protected void before() throws Exception {
    ...
    final Random random = SolrTestCase.random();
    ...
    if ("default".equals(TEST_CODEC)) {
      ...
      codec = new MyAssertingCodec(format, dvFormat);
    ...
    TimeZone randomTimeZone = randomTimeZone(SolrTestCase.random());
    timeZone = testTimeZone.equals("random") ? randomTimeZone : TimeZone.getTimeZone(testTimeZone);
    TimeZone.setDefault(timeZone);
    similarity = new AssertingSimilarity(new RandomSimilarity(SolrTestCase.random()));
  ...
  private static class MyAssertingCodec extends AssertingCodec {
    private final PostingsFormat format;
    private final DocValuesFormat dvFormat;

    public MyAssertingCodec(PostingsFormat format, DocValuesFormat dvFormat) {
      this.format = format;

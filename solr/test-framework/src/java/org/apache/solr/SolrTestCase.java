...
public class SolrTestCase extends Assert {
   ...
   */
  private final static long STATIC_LEAK_THRESHOLD = 1200; // MRM TODO: I dropped this down hard and enabled it again, but then raised it a bit
  ...
  private static TestRuleSetupTeardownChained parentChainCallRule;
...
//  })
  ...
  @Rule
  public TestRule solrTestRules = RuleChain.outerRule(threadAndTestNameRule = new TestRuleThreadAndTestName()).around(parentChainCallRule = new TestRuleSetupTeardownChained());
  ...
  public static Random random() {
    ...
    //return RANDOM.updateAndGet(random -> random == null ? SolrTestCase.random() : random);
    return new Random();
  }
  ...
  public static void beforeSolrTestCase() throws Exception {
    ...
    // Capture this suite's live RandomizedContext so test analyzers (MockTokenizer) built later
    // on orphaned shared-pool threads can resolve a context. Test-only; no production impact.
    org.apache.solr.analysis.MockRandomizedContextAnchor.capture();

    Random random = random();

    if (random.nextInt(10) < 7) {
      System.setProperty(MockDirectoryFactory.SOLR_TESTS_USING_MOCK_DIRECTORY_WRAPPER, "false");
    }
    ...
    Thread.setDefaultUncaughtExceptionHandler((t, e) -> {
      try {
        ...
        log.error("Exception in the UncaughtExceptionHandler", e);
      }
    });
    ...
    System.setProperty("solr.rootSharedThreadPoolCoreSize", "32");
    System.setProperty("solr.skipCommitOnClose", "false");
    System.setProperty("solr.createCollectionTimeout", "180000");
   ...
   // System.setProperty("solr.tests.mergePolicyFactory", "org.apache.solr.index.NoMergePolicyFactory");
    ...
    if (!LuceneTestCase.TEST_NIGHTLY) {
      ...
      System.setProperty("solr.waitForState.looptimeout", "500");
   ...
   //   System.setProperty("solr.per_thread_exec.min_threads", "1");
      ...
      System.setProperty("zookeeper.nio.numSelectorThreads", "8");
      System.setProperty("zookeeper.nio.numWorkerThreads", "12");
      System.setProperty("zookeeper.commitProcessor.numWorkerThreads", "5");
      System.setProperty("zookeeper.skipACL", "true");
      System.setProperty("zookeeper.nio.shutdownTimeout", "500");
...
//      System.setProperty("solr.recoveryThrottle", "1000");
     ...
     // System.setProperty("lucene.cms.override_core_count", "3");
      ...
      SolrTestCase.useFactory("org.apache.solr.core.RAMDirectoryFactory");
      System.setProperty("solr.lock.type", "single");
...
//  }
  ...
  public static void afterSolrTestCaseClass() throws Exception {
    ...
    try {
      ...
      ObjectReleaseTracker ort = ObjectReleaseTracker.getInstance();

      String orr = ort.checkEmpty(object);

      if (orr != null) {
        Thread.sleep(250); // SolrCores can take a moment to close and might get rejected and close by caller thread
        orr = ort.checkEmpty(object);
      }
     ...
     // ParWork.shutdownParWorkExecutor();
      ...
      ObjectReleaseTracker.getInstance().clear();
     ...
     // ZkSolrResourceLoader.CONFIG_CACHE.clear();
      SolrMetricManager.REGISTRIES.clear();
...
//      }
  ...
  private static void checkForInterruptRequest() {
    try {
      interruptThreadsOnTearDown(true, interuptThreadWithNameContains.toArray(Utils.EMPTY_STRINGS));
    } catch (Exception e) {
  ...
  public static void interruptThreadsOnTearDown(boolean now, String... nameContains) {
   ...
   // System.out.println("DO FORCED INTERRUPTS");
  ...
  //  System.out.println("test group:" + tg.getName());
    ...
    for (Map.Entry<Thread,StackTraceElement[]> threadEntry : threadSet) {
      ...
      if (threadGroup != null && !(thread.getName().startsWith("SUITE") && !thread.getName().isEmpty()
          && thread.getName().charAt(thread.getName().length() - 1) == ']') && !thread.getName().equals("main")) {
      ...
      while (true) {
        boolean cont =
            threadGroup != null && threadGroup.getParent() != null && !(thread.getName().startsWith("SUITE") && thread.getName().endsWith("]")) && !thread
                .getName().equals("main");
        if (!cont) break;
  ...
  private static boolean interrupt(Thread thread, String... nameContains) {
    ...
    if (thread.getName().startsWith("Reference Handler") || thread.getName().startsWith("Signal Dispatcher") || thread.getName().startsWith("Monitor")
        || thread.getName().startsWith("YJPAgent-RequestListener")) {
      return false;
    }

    if (thread.getName().startsWith("ForkJoinPool.") || thread.getName().startsWith("Log4j2-") || thread.getName().startsWith("SessionTracker")) {
      return false;
    }
    ...
    if (thread.getName().startsWith(ParWork.ROOT_EXEC_PREFIX) && thread.getState() != Thread.State.RUNNABLE) {
      return false;
    }

    if (thread.getName().contains("-SendThread")) {
      if (log.isDebugEnabled()) {
        log.debug("interrupt on {}",  thread.getName());
      }
      thread.interrupt();
      return true;
    }
    if (((thread.getName().contains("ProcessThread(")) && thread.getState() != Thread.State.TERMINATED)) {
      if (log.isDebugEnabled()) {
        log.debug("interrupt on {}", thread.getName());
      }
      thread.interrupt();
    ...
    if (state == Thread.State.TERMINATED) {
      return false; // change to true if we go back to test framework thread checking
    }
    if (log.isDebugEnabled()) {
...
//  }
  ...
  public static void useFactory(String factory) throws Exception {
    ...
    if (factory == null) {
     // factory = "solr.StandardDirectoryFactory";
      factory = random().nextInt(100) < 75 ? "solr.StandardDirectoryFactory" : "solr.MMapDirectoryFactory"; // test the default most of the time
    }
  ...
  public static ExecutorService getTestExecutor() {
    synchronized (SolrTestCase.class) {
      ...
      testExecutor = (ParWorkExecutor) ParWork.getParExecutorService(
          "testExecutor", 5, 64, 10000);
      testExecutor.prestartAllCoreThreads();
      testExecutor.enableCloseLock();
      return testExecutor;
    }
  }
  ...
  private static IndexableField newTextField(String value, String foo_bar_bar_bar_bar, Field.Store no) {
    return SolrTestUtil.newTextField(value, foo_bar_bar_bar_bar, no);
  }

  protected static IndexableField newStringField(String value, String bar, Field.Store yes) {

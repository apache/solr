 ...
 */
public class SolrRandomIndexWriter implements Closeable {
  ...
  public SolrRandomIndexWriter(Random r, Directory dir) throws IOException {
    this(
        r, dir, SolrTestUtil.newIndexWriterConfig(r, new MockAnalyzer(r)), true, r.nextBoolean());
  }

  /** create a RandomIndexWriter with a random config */
  public SolrRandomIndexWriter(Random r, Directory dir, Analyzer a) throws IOException {
    this(r, dir, SolrTestUtil.newIndexWriterConfig(r, a));
  }

  /** create a RandomIndexWriter with the provided config */
  public SolrRandomIndexWriter(Random r, Directory dir, IndexWriterConfig c) throws IOException {
    this(r, dir, c, false, r.nextBoolean());
  }

  /** create a RandomIndexWriter with the provided config */
  public SolrRandomIndexWriter(Random r, Directory dir, IndexWriterConfig c, boolean useSoftDeletes)
      throws IOException {
    this(r, dir, c, false, useSoftDeletes);
  }

  private SolrRandomIndexWriter(
   ...
   */
  public <T extends IndexableField> long updateDocument(Term t, final Iterable<T> doc)
      throws IOException {
    final long seqNo;
    if (useSoftDeletes()) {
      if (r.nextInt(5) == 3) {
        ...
        seqNo =
            w.softUpdateDocument(
                t, doc, new NumericDocValuesField(config.getSoftDeletesField(), 1));
      }
    } else {
      if (r.nextInt(5) == 3) {
   ...
   * message. See also {@link SolrRandomIndexWriter#mockIndexWriter(Random, Directory,
   * IndexWriterConfig, TestPoint)}
   */

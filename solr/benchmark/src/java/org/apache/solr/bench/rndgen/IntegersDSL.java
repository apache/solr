...
import static org.apache.solr.bench.rndgen.SourceDSL.checkArguments;
  ...
  private static class IntegerMaxCardinalityRndGen extends RndGen<Integer> {
    ...
    public Integer gen(RandomnessSource in) {
      if (cardinalityStart == null) {
        cardinalityStart = Generate.range(0, Integer.MAX_VALUE - maxCardinality - 1).gen(in);
      }

      long seed = Generate.range(cardinalityStart, cardinalityStart + maxCardinality - 1).gen(in);
      return integers.gen(new SplittableRandomSource(new SplittableRandom(seed))).intValue();
    }
  }

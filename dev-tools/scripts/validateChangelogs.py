    ...
    def validate_versioned_folders_identical(self) -> bool:
        ...
        for folder in sorted(all_folders):
            ...
            if len(contents_by_branch) != len(self.branches):
                missing = set(self.branches.keys()) - set(contents_by_branch.keys())
                self.errors.append(f"Folder {folder} not found on branches: {missing}")
                continue

            if not first_contents:
                first_contents = next(iter(contents_by_branch.values()))
            for branch, contents in contents_by_branch.items():
                if contents != first_contents:
                    self.errors.append(f"Folder {folder} has different contents on branch {branch}")

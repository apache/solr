...
def prepare(root, version, gpg_key_id, gpg_password, gpg_home=None, sign_gradle=False):
  ...
  if not dev_mode:
    print('  ./gradlew --no-daemon -Dtests.badapples=false  -Pvalidation.sourcePatterns.failOnError=false clean check')
    run('./gradlew --no-daemon -Dtests.badapples=false -Pvalidation.sourcePatterns.failOnError=false clean check')
  else:
...
def checkDOAPfiles(version):
  # In Solr DOAP file, verify presence of all releases less than the one being produced.
  errorMessages = []
  distpage_lucene_solr = load('https://archive.apache.org/dist/lucene/solr')
  distpage_solr = load('https://archive.apache.org/dist/solr')
  releases = set()
  for regex in reVersion1, reVersion2:
    for tup in regex.findall(distpage_lucene_solr):
      if tup[0] in ('1', '2'):                    # Ignore 1.X and 2.X releases
        continue
      releases.add(normalizeVersion(tup))
    for tup in regex.findall(distpage_solr):
      if tup[0] in ('1', '2'):                    # Ignore 1.X and 2.X releases
        continue
      releases.add(normalizeVersion(tup))
  doapNS = '{http://usefulinc.com/ns/doap#}'
  xpathRevision = '{0}Project/{0}release/{0}Version/{0}revision'.format(doapNS)
  doapFile = "dev-tools/doap/solr.rdf"
...
def parse_config():
  ...
  parser.add_argument('--root', metavar='PATH', default='.',
                      help='Root of Git working tree for solr.  Default: "." (the current directory)')
  ...
  if os.system('git rev-parse') or 2 != len([d for d in ('dev-tools','solr') if os.path.isdir(d)]):
    parser.error('Root path "%s" is not a valid solr checkout' % config.root)
  os.chdir(cwd)
  global LOG
  if config.logfile:
    LOG = config.logfile

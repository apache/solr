...
./gradlew -p ui :webApp:wasmJsBrowserDevelopmentRun
----
...
`org.apache.solr.ui.views`. That includes all UI elements / composables. Prefer testing the
stateless composables, as they only need a UI state and callbacks. Integration tests
...
The unit tests mainly cover the state holders (`viewmodel/*StateHolder.kt`) and the use cases.
They use fake use cases or repositories (or an HTTP client with a mocked engine) for the API, and
test the behavior and state changes to confirm correct state transitions and emitted events with
specific inputs.

State holders receive their coroutine scope and dispatchers as parameters, which allows running
them with the test scheduler without any other setup:

[source,kotlin]
----
@Test
fun `WHEN initialized THEN system data fetched`() = runTest {
    val stateHolder = EnvironmentStateHolder(
        scope = this,
        loadSystemDataUseCase = fakeLoadSystemDataUseCase,
        loadJavaPropertiesUseCase = fakeLoadJavaPropertiesUseCase,
        dispatchers = TestDispatchers(UnconfinedTestDispatcher(testScheduler)),
    )
    advanceUntilIdle()

    assertEquals(expectedSolrHome, stateHolder.uiState.value.solrHome)
}
----

State holders that collect flows for as long as they exist (like the authentication state
holder) should be created with `backgroundScope` instead, so that the test does not wait for them.
Events are `SharedFlow`s without replay. To test them, collect them in the `backgroundScope`
...
[source,bash]
----
./gradlew -p ui :desktopApp:packageDistributionForCurrentOS
----

The distribution can be found at `ui/desktopApp/build/compose/binaries/main/[deb|msi|dmg]/`.

To build the JVM target for production, you can use:

[source,bash]
----
./gradlew -p ui :desktopApp:packageReleaseDistributionForCurrentOS
----
...
----
# for development
./gradlew -p ui :desktopApp:packageUberJarForCurrentOS

# for production
./gradlew -p ui :desktopApp:packageReleaseUberJarForCurrentOS
----

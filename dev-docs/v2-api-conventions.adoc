...
Note: this is distinct from "per-core" APIs generally (see the "API Types" section of link:apis.adoc[apis.adoc]), which are registered on individual `SolrCore` objects (e.g. `/select`, `/update`, `/admin/ping`). `CoreAdminAPIBase`-derived APIs are instead a "cluster" API registered on the `CoreContainer`, corresponding to the legacy `/admin/cores` handler.

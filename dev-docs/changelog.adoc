...
For the author section, the `name` key is the only one required. You may optionally want to link to either your github user (`nick`) or e.g. your ASF user (`url`) as shown below.
...
Pull requests that only touch documentation, tests, build files or CI configuration
are exempt and pass without an entry.  Subjectively, some minor changes don't need one either.

=== 4.1 Opting out of the changelog requirement

If your change does not require a changelog entry (refactoring, internal cleanup,
and so on), it is still possible to merge the PR. Mark it by adding the
`no-changelog` label, which you can set in either of two ways:

* Comment `/no-changelog` on the pull request, alone on its own line. This works
  for the PR author as well as for committers, so a contributor without write
  access does not need to ask anyone to set the label. Comment `/needs-changelog`
  to undo it. Use a regular comment in the "Conversation" tab; commands inside
  code review comments are not picked up.
* Add or remove the `no-changelog` label directly (committers only).

Either way the check re-runs automatically on the label change, so an already
failed "Check changelog entry" run turns green without pushing a new commit.

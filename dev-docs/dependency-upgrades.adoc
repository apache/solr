...
To further cut down on the number of tiny PRs, upgrades are *grouped*. There are a handful of logical groups for
libraries that must stay version-compatible (e.g. Admin UI, Web server stack, Logging), and then two large
catch-all groups - "all non-major dependencies" and "all non-major test dependencies" - that bundle the *minor and
patch* bumps of everything else. *Major* bumps are never grouped; each gets its own PR for individual review. See
`renovate.json` for the exact group definitions.

...
1. *Agree on Endpoint Syntax*: Before implementing, developers should generate consensus around what the new API will look like.  General guidelines and conventions for the v2 API syntax are described <<v2-api-conventions.adoc,here>>.
2. *Create POJO ("Plain Old Java Object") classes as needed to represent the API's request and response*:
...
3. *Find or create an interface to hold the v2 API definition*:
...
4. *Add a method to the chosen interface representing the API*:
...
5. *Futher JAX-RS Annotation*: The interface method in step (3) has specified its inputs and outputs, but several additional annotations are needed to define how users access the API, and to make it play nice with the code-generation done by Solr's build.
...
6. *Create a class implementing the API interface*: Implementation classes live in the `core` gradle project, typically in the `org.apache.solr.handler` package or one of its descendants.

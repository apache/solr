...
As these transitions complete, this complexity should simplify considerably.
But in the interim, this document can help guide developers who need to understand or modify APIs in Solr.

== API Types

APIs in Solr (regardless of whether v1 or v2) can typically be classified as either a "per-core" or an "admin" API.
...
Alternatively "admin" APIs potentially affect the entire Solr instance or cluster.

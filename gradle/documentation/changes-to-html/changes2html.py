...
class ChangelogParser:
    ...
    def parse(self, content):
        ...
        current_items = []
        self.preamble = None  # Store preamble text
        ...
        while i < len(lines):
            ...
            if not current_release and not self.preamble and line.strip() and not line.startswith('['):
                self.preamble = line.strip()
                i += 1
                continue
            ...
            match = re.match(r'^###\s+(\w+(?:\s+\w+)*)\s*(?:\(\d+\s+changes?\))?', line)
            if match and current_release is not None:
            ...
            if line.startswith('- ') and current_release is not None:
                # Collect multi-line item
                item_text = line[2:]  # Remove leading '- '
                i += 1

                # Collect continuation lines
                while i < len(lines):
                    next_line = lines[i]
                    if next_line.startswith('- ') or re.match(r'^###', next_line) or re.match(r'^\[', next_line):
                        break
                    if next_line.strip():
...
class HTMLGenerator:
    ...
    def extract_issue_from_text(self, text):
        ...
        Returns (issue_link_html, text_without_issue, authors)
        """
        # Extract JIRA issue: [SOLR-123](url)
        jira_match = re.search(r'\[([A-Z]+-\d+)\]\(https://issues\.apache\.org/jira/browse/\1\)', text)
        if jira_match:
            issue_id = jira_match.group(1)
            issue_html = f'<a href="{self.JIRA_URL_PREFIX}{issue_id}">{issue_id}</a>'
            text_before = text[:jira_match.start()]
            text_after = text[jira_match.end():]
            # Combine the text and remove markdown links from issue part
            clean_text = (text_before + text_after).strip()
            return issue_html, clean_text, None

        # Extract GitHub PR: [PR#123](url)
        pr_match = re.search(r'\[PR#(\d+)\]\(https://github\.com/apache/solr/pull/\1\)', text)
        if pr_match:
            pr_num = pr_match.group(1)
            issue_html = f'<a href="{self.GITHUB_PR_PREFIX}{pr_num}">PR#{pr_num}</a>'
            text_before = text[:pr_match.start()]
            text_after = text[pr_match.end():]
            clean_text = (text_before + text_after).strip()
            return issue_html, clean_text, None

        # Extract GITHUB: [GITHUB#123](url)
        gh_match = re.search(r'\[GITHUB#(\d+)\]\(https://github\.com/apache/solr/issues/\1\)', text)
        if gh_match:
            gh_num = gh_match.group(1)
            issue_html = f'<a href="https://github.com/apache/solr/issues/{gh_num}">GITHUB#{gh_num}</a>'
            text_before = text[:gh_match.start()]
            text_after = text[gh_match.end():]
            clean_text = (text_before + text_after).strip()
            return issue_html, clean_text, None
    ...
    def format_changelog_item(self, item_text):
        ...
        """
        # First extract the issue
        issue_html, text_after_issue, _ = self.extract_issue_from_text(item_text)

        if not issue_html:
            # No issue found, just linkify and return
        ...
        if authors_list:
            ...
            html += f'<br /><span class="attrib">({self.escape_html(authors_str)})</span>'
    ...
    def generate_releases(self, releases):
        ...
        for release in releases:
            ...
            if version:
                ...
                html.append(f'<a id="{relid}" href="javascript:toggleList(\'{relid}\')">')
                html.append('Release ' if version else '')
                html.append(f'{self.escape_html(version)} ')
                if date:
                ...
                for section in sections:
                    ...
                    if section_name:
                        ...
                        html.append(f'  <li><a id="{relid}.{sectid}" href="javascript:toggleList(\'{relid}.{sectid}\')">')
                        html.append(f'{self.escape_html(section_name)}</a>')
                        html.append(f'&nbsp;&nbsp;&nbsp;{num_str}\n')
                        licnt += 1
                        ...
                        for item in items:
                            ...
                            item_html = self.format_changelog_item(item)
